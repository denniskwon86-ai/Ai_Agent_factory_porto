...
async def run_vision_qa(state: Any) -> Dict[str, Any]:
    ...
    # ⚠️ [결함 #19 계열] UI 판정도 cacheable=False — 재설계된 UI 에 옛 소견이 재생되면 안 된다.
    output_str = await gateway.aexecute(state_obj, prompt, is_heavy=True, output_mode="json", light=True,
                                        cacheable=False)
