      ...
      <div className="flex flex-col gap-2">
        <div className="flex items-center justify-between gap-2">
          <div className="flex gap-1 items-center">
            <span className={`text-xs font-bold px-2 py-0.5 rounded border ${cat.color} ${cat.border}`}>
              {cat.label}
            </span>
            {data.is_start && <span className="text-xs font-bold bg-green-500/20 text-green-300 px-1.5 py-0.5 rounded" title="Start Point">▶ 시작</span>}
            {data.is_end && <span className="text-xs font-bold bg-red-500/20 text-red-300 px-1.5 py-0.5 rounded" title="End Point">■ 종료</span>}
          </div>
          <div className="flex gap-1">
            {data.hotl_after && <span className="text-xs font-bold bg-rose-500/20 text-rose-300 px-1.5 py-0.5 rounded" title="HOTL (전문가 개입)">✋</span>}
            {data.debate && <span className="text-xs font-bold bg-purple-500/20 text-purple-300 px-1.5 py-0.5 rounded" title="Debate">💬</span>}
          </div>
        </div>
        ...
        <div>
          ...
          <p className="text-xs font-mono text-gray-500 truncate" title={data.id}>
            {data.id}
          </p>
        </div>
      </div>
