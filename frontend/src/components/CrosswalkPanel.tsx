              ...
              {systems.map((s) => (
                <button key={s.system_id} onClick={() => setSel(s.system_id)}
                  className={`w-full text-left rounded-lg p-2.5 border transition-colors ${
                    sel === s.system_id ? 'border-sky-500 bg-sky-900/30' : 'border-[#2F3640] bg-[#0B0C10] hover:border-gray-500'}`}>
            ...
            {!selSys ? (
              ...
              <div className="flex-1 overflow-y-auto p-5 space-y-5">
                <div className="flex items-center justify-between">
                  <h3 className="text-base font-bold text-white">{selSys.name} <span className="text-xs text-gray-500 font-mono">({selSys.system_id})</span></h3>
                ...
                <div>
                  ...
                  <div className="space-y-1.5">
                    {pending.map((p) => (
                      <div key={p.id} className="flex items-center justify-between rounded-lg border border-[#2F3640] bg-[#0B0C10] px-3 py-2">
