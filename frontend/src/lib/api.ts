...
//
// ★★★ [WEB-1 / 2026-08-23] **«비워 둠» 과 «설정 안 함» 을 가른다.**
//
// ⚠️⚠️ 종전에는 `|| 'http://127.0.0.1:8080'` 이었다. 빈 문자열은 거짓이므로
//   `VITE_API_BASE_URL=` 로 빌드해도 **개발 주소가 번들에 박혔다** — 배포한 화면이
//   방문자의 **자기 PC** 를 부르고, 그 실패는 「서버가 죽었다」로 보인다.
//   실측으로 잡았다: same-origin 빌드 뒤 `dist/assets` 에 `127.0.0.1:8080` 이 남아 있었다.
//
// ★ 이제 **빈 문자열은 same-origin 을 뜻한다**(상대경로로 나간다). 도메인이 바뀌어도
//   프런트를 다시 빌드하지 않는다. 설정하지 않은 경우에만 개발 기본값으로 내려간다.
// ★ `??` 이지 `||` 가 아니다. 빈 문자열은 **설정된 값**(same-origin)이고, 설정하지
//   않은 경우에만 개발 기본값으로 내려간다.
