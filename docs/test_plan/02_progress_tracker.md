...
| P2 파일럿(관문) | **PASS** | **1/1** | ★ **2026-07-27 A-1 완주 달성** — 6/6 태스크 DONE · QA PASS · 수용검수 PASS · 게시 완료(`test_a1_v11_20260727_170122`). 소요 14분 |
...
| **P12 회사·조직·권한** | 진행 | **1/5** | H-5 PASS(자동). LLM 불필요 |
| **P13 업무 데이터 준비** | PENDING | 0/5 | LLM 불필요 |
| **P14 외부 데이터** | 진행 | **3/4** | J-2·J-3·J-4 PASS(자동) · J-1 COND(키 미실측). LLM 불필요 |
| **P15 계산·기준선·의사결정** | PENDING | 0/4 | |
| **P16 산출·전달 + 관통** | PENDING | 0/7 | ★ **Z-1·Z-2 를 가장 먼저** |

⚠️ **【2026-09-08 리비전】** 카탈로그가 07-29 이후 갱신되지 않아 그 뒤 구현분(core 신규
128파일·라우트 15개)이 **한 건도 검증 대상에 없었다.** 트랙 H~L·Z 를 추가했다.
기존 ID 는 하나도 바꾸지 않았다 — 이 표가 참조하기 때문이다.

    갱신 전   41건 중 12건 (29%)   ← 7월 시점 제품에 대한 것
    갱신 후   66건 중 16건 (24%)   ← 지금 제품에 대한 것

★ **비율이 «떨어진» 것이 정직한 상태다.** 분모가 커졌기 때문이고, 그 전 29% 는 이미 없는
  제품에 대한 숫자였다.
...
| A-1 | **PASS** | — | 2026-07-27 | ★ **완주 달성** — 6/6 태스크 DONE · QA PASS · 수용검수 PASS · 게시 완료(`test_a1_v11_20260727_170122`). 상세는 아래 「2026-07-27 A-1 관문 시나리오 완주」 절. ⚠️ **2026-09-08 정정**: 이 행은 07-26 의 COND(504 타임아웃 중단)로 남아 있었고 요약표만 PASS 로 갱신돼 있었다 — **상세행만 읽는 사람은 아직 실패로 읽었다.** 07-26 시도 기록: 기획 파이프라인 100% 통과·WBS 5건 분할 후 외부 LLM 504 로 중단, 504 방어(Tier/Circuit Breaker) 가동 실측 |
| A-2 | PENDING | — | | |
| A-3 | PENDING | — | | |
| A-4 | PENDING | — | | |
| A-5 | PENDING | — | | |
...
### 【2026-09-08 리비전】 신규 트랙 H ~ L · Z

> 상태 표기 추가: **`PASS(자동)`** = pytest 스위트로 검증됨. ⚠️ **제품 경로(HTTP)·화면까지
> 확인했다는 뜻이 아니다.** S-1~S-8 에 쓴 것과 같은 기준이며, 「배선됐다」까지만 주장한다.

| ID | 상태 | 실행일 | 근거 / 특이사항 |
|---|---|---|---|
| H-1 | PENDING | | 조직도 수립·범위 해석 |
| H-2 | PENDING | | ★ 가상 조직 «직접 생성 거부»가 핵심. 안 막히면 FAIL |
| H-3 | PENDING | | 경쟁사 근거 5종 필수·「확인 불가」 1급 상태 |
| H-4 | PENDING | | ⚠️ `org_enforce` 를 **켜고** 확인할 것. 끄면 200 이 통제 유무를 뜻하지 않는다 |
| H-5 | **PASS(자동)** | 2026-09-08 | `tests/test_route_authority_table.py` 외 314건. 유령 라우트·미봉인 쓰기 라우트 0건 |
| I-1 | PENDING | | 업무키트 9종(FOUNDATION·BK-01~08)과 계약 대조 |
| I-2 | PENDING | | 데이터셋 상태 ≠ 인스턴스 상태. 상태마다 「다음 행동·담당자」가 있는가 |
| I-3 | PENDING | | ⚠️ 실물은 `DEMO_CERTIFIED` 에 못 간다 — 그 경계가 **실제로 막는지** 본다 |
| I-4 | PENDING | | 원천 결속. 미지원을 «빈 결과»로 돌려주지 않는가 |
| I-5 | PENDING | | 표본이 「지금」에 대해 상하지 않았는가 |
| J-1 | **COND** | 2026-09-08 | Provider 5종 fixture 종단 완주(`tests/test_provider_*.py`). ⚠️ **실제 API 키로 한 번도 안 돌렸다** — 「실제 수집 완료」가 아니다 |
| J-2 | **PASS(자동)** | 2026-09-08 | 「자료 없음」 다섯 방언을 각각 올바른 상태로 보낸다. KOSIS 빈 배열 포함 |
| J-3 | **PASS(자동)** | 2026-09-08 | 비밀 누출·SSRF·리디렉션 재검사·DNS 사설망. ★ **차단기를 제거해 보고** 시험이 실제로 걸리는지 확인함 |
| J-4 | **PASS(자동)** | 2026-09-08 | 추천기 40건. 네트워크 0회(`socket`·`urlopen` 차단 후 실행) · 카탈로그 밖 생성 없음 · 정산 일치 |
| K-1 | PENDING | | ★ 시연 스크립트가 아니라 **제품이 부르는 경로**로 도는가 |
| K-2 | PENDING | | 미인증 데이터로 계산이 막히는가. 막는 이유가 응답에 있는가 |
| K-3 | PENDING | | 기준선 재계산 시 이전 판 보존 |
| K-4 | PENDING | | 결정이 근거 데이터의 «판»을 고정하는가 |
| L-1 | PENDING | | ⚠️ 담당 레인이 입력·수용 기준을 채울 것 |
| L-2 | PENDING | | ⚠️ 「운영 배포」가 어디까지 왔는지 담당 레인이 판정 |
| L-3 | PENDING | | ⚠️ 생성 앱 데이터와 플랫폼 데이터의 **격리**가 핵심 |
| L-4 | PENDING | | ⚠️ 「두 번째 비서를 만들지 않았는가」 |
| L-5 | PENDING | | ⚠️ 온톨로지 계약 |
| **Z-1** | PENDING | | ★★★ **관통 — 가장 먼저 돌 것.** 끊긴 곳을 찾는 것이 목적이다 |
| **Z-2** | PENDING | | ★★★ 실물 데이터로 **어디서 막히는지 기록.** 완주가 PASS 조건이 아니다 |

⚠️ **`PASS(자동)` 4건에 대한 정직한 경계** — 이것들은 pytest 로 검증됐고 그 시험이
  제품의 실제 클래스를 지나간다. 그러나 **화면에서 눌러 본 적은 없고**(로그인 뒤라 확인
  못 함), J-1 은 **실제 원천에 한 번도 접속하지 않았다.** 「통합 초록」으로 읽으면 안 된다.
...
| 17| A-1 | Critical | **LLM 산출물 키 편차가 스프린트 루프를 죽임**: Tech Lead 가 ADR 을 `{id,title,description}` 으로 냈으나 `ADR` 스키마는 `{id,decision,reason}` 필수 + `extra='forbid'` → `Sprint Loop Error: 8 validation errors for ProjectState` 로 파이프라인 사망. 형제 모델 `DebtItem` 은 이미 `extra='ignore'`+기본값+before-validator 로 같은 문제를 해결해 뒀는데 `ADR` 에만 그 조치가 없었다. 게다가 파싱 지점(`nodes/execution.py`)이 dict 를 **검증 없이** 상태에 넣어 오류가 발생 지점이 아니라 훨씬 뒤에서 터졌다 | **FIXED** (2026-07-26, `c3a31e175`) — `ADR` 에 동의어 흡수(title/summary/what/content→decision, description/rationale/why→reason) + 기본값 + `extra='ignore'`. 파싱 지점은 모델로 즉시 검증해 불량 항목만 폐기. 실제 실패 입력으로 검증 통과 |
| 18| A-1 | Critical | **Flash 재시도의 6k 프롬프트 축소가 유료 백스톱과 code 모드를 파괴**: `llm_gateway.py` 의 `if not is_heavy and retry_count >= 1: _budget = min(_budget, 6000)` 은 무료 소형 모델(Groq/Cerebras 6k)도 성공시키려는 조치로 무료 시대엔 타당했다. 그러나 ① 체인에 살아있는 모델이 유료(컨텍스트 200k)뿐인데도 6k 로 잘라 보내고 ② code 모드는 `_INCREMENTAL_GUARD` 가 기존 파일 전체 재출력을 요구하는데 그 원본이 절단돼 **모델이 볼 수 없는 것을 재현하라는 요구**가 되어 구조적으로 실패한다. 실측: `♻️ 유료 백스톱 재프로브` 직후 `✂️ 프롬프트 21,551자 → 15,000자 절단` → 실패. **결함 #16 을 고쳐 유료 백스톱에 도달하게 된 뒤에야 보이게 된 결함** | **FIXED** (2026-07-26) — 축소 대상에서 `output_mode=='code'` 와 '살아있는 유료 모델이 있는 경우'를 제외. 무료만 남은 비-code 호출은 기존 6k 축소 유지 |
| 19| A-1 | **Critical** | **심사·판정 성격 LLM 호출이 전부 캐시를 타서 재작업 루프가 수렴 불가**(결함 #13 의 근본 패턴). 재작업으로 산출물을 개선해도 프롬프트가 이전 회차와 같아지는 순간 **캐시된 옛 판정이 재생**되어 개선이 반영되지 않는다. 실측: `supervisor_hops` 1→2→3 으로 오르는데 `CODE_REVIEW` 점수는 **0.0 에 고정**, 같은 해시(`3d744629`) 반복 히트 → 상한(8) 도달 후 `best-effort 수용` 으로 **미해결 결함을 DONE 으로 위장 통과**. **전수 점검 결과 4곳**: `execution.py:652`(리뷰어 판정) · `scoring.py:119,134`(단계별 심판 채점) · `debate.py:143`(비평가) · `vision_qa.py:81`(UI 자문). ⚠️ `debate.py` 는 `:179`/`:202`(리비전·재작업)엔 이미 `cacheable=False` 를 걸어놨는데 **같은 파일 `:143` 비평가만 빠져 있었다** — #13·#17 과 동일하게 '한 곳에서 배운 교훈이 형제 코드에 전파되지 않는' 구조적 약점. 이 결함군은 **조용히 실패**한다(에러 없음, 오히려 '캐시 히트로 비용 절감'처럼 보임)는 점이 특히 위험하다. A-1 이 오래 완주하지 못한 진짜 이유일 가능성이 크다 | **FIXED** (2026-07-26) — 4곳 전부 `cacheable=False`. **원칙: 생성물은 캐시해도 되지만 판단(심사·채점·비평·검수)은 매번 새로 해야 한다.** 오염된 판정 캐시도 함께 비움 |
| 19-b| A-1 | Critical | **#19 전수 점검이 `core/supervisor_daemon.py` 를 누락**. `nodes/` 만 훑고 `core/` 의 감시 데몬을 놓쳤다(`:62`, `:117` — `is_heavy=False, output_mode="json"` 이라 `flash_router json` 경로). 이 데몬은 **파이프라인 일시정지(개입)를 시도**하므로 캐시된 옛 판정이 재생되면 **이미 해소된 결함으로 현재 스프린트를 멈춘다**. #19 수정 후에도 같은 해시(`3d744629`)가 계속 재발한 출처가 여기였다. ⚠️ 결함 #19 자체가 '교훈이 형제 코드에 전파되지 않는 약점'을 지적했는데, **그 수정조차 같은 방식으로 한 디렉터리를 빠뜨렸다** | **FIXED** (2026-07-26, `6c5034d12`) — `aexecute` 2곳에 `cacheable=False` |
| 20| A-1 | **Critical** | **렌더 검증 하네스가 거짓 실패를 만들어 정상 코드를 반려**. `frontend/scripts/render_check.mjs` 는 맨 Node 에서 `renderToString` 하는데 **브라우저 전역이 하나도 없었다**. 그래서 PRD 가 명시적으로 요구한 '변환 이력 저장'을 `localStorage` 로 **올바르게** 구현한 코드가 `초기 렌더 런타임 에러: localStorage is not defined` 로 실패 처리됐다. 실제 프리뷰(PreviewPanel)는 브라우저이므로 그 코드는 정상 동작한다 — 즉 **요구한 대로 만든 것을 감점**하고, 재작업 루프는 고칠 수 없는 것을 고치라고 8회 반복시켰다. #13·#19 를 고쳐 재작업 루프가 실제로 돌기 시작한 뒤에야 드러난 결함 | **FIXED** (2026-07-26, `6c5034d12`) — `localStorage`·`sessionStorage`·`window`·`document`·`navigator`·`matchMedia` 최소 스텁 주입. **스텁은 '브라우저라면 반드시 있는 것'으로만 제한**(과잉 스텁 시 진짜 깨진 코드가 통과해 검증 자체가 무의미해진다). `document.getElementById` 는 `null` 반환 유지 — 브라우저 초기 렌더와 동일하며 null 미확인 접근은 여기서 잡혀야 한다. 실측: 현재 `test_a1_v3` 산출물 → `ok=true, rendered=780`(이전엔 실패) |
| 21| A-1 | Major | **전체 파일 재출력 강제(`_INCREMENTAL_GUARD`)의 부작용 — 재작업 회귀**. 앞 회차에서 고친 결함(존재하지 않는 `./App.css` import)이 다음 회차 재출력에서 **되살아났다**(실측: 렌더 검증 `srv6` 통과 2건 → `srv7` 실패 1건, 같은 결함 재출현). 전체를 매번 새로 쓰게 하니 이전 수정이 유지되지 않는다. 또한 렌더 실패 지시가 "외부 import / null 안전성" 일반 문구뿐이어서 **가장 흔한 실패인 '미해결 상대 모듈'의 해결책을 알려주지 못했다** | **FIXED** (2026-07-26, `6c5034d12`) — (A) 가드에 **import 규칙**(상대경로 import 는 같은 응답에 파일을 함께 생성하거나 import 를 삭제)과 **회귀 금지 규칙**(앞 회차 수정 상태 유지) 추가. (B) 렌더 실패 메시지에서 누락 파일명을 추출해 **두 선택지를 명시**하고 이전 수정을 되돌리지 말라고 경고 |

---

## ★★ 2026-07-27 — A-1 관문 시나리오 완주 (PASS)

**프로젝트 `test_a1_v11` / 소요 14분 / `release_id: test_a1_v11_20260727_170122`**

### 완주 판정 기준 5개 — 전부 충족

| # | 기준 | 실측 |
|---|---|---|
| 1 | WBS 전 태스크 DONE | **6/6** (E2E-01~06) |
| 2 | QA 통합검수 | `qa_verdict=PASS` |
| 3 | 수용검수 | `supervisor_verdict=PASS` |
| 4 | 라이브러리 게시 | `library/test_a1_v11_20260727_170122/release.json` (69,251B) |
| 5 | 렌더 검증 실제 실행·통과 | 통과 10 / 실패 1 |

단계별 점수: RFP 1.0 · PLANNING 0.975 · UI_DESIGN 1.0 · VISION_QA 1.0 · ARCHITECTURE 0.9 · PMO 1.0 · TECH_SPEC 1.0 · CODE_REVIEW 1.0

### 산출물 독립 검증 (선언 전 별도 실행)

`{"ok": true, "errors": [], "rendered": 2002}`

| 파일 | 크기 |
|---|---|
| `src/App.tsx` | 5,187B |
| `src/components/ConverterSection.tsx` | 3,238B |
| `src/components/HistoryList.tsx` | 1,265B |
| `src/types.ts` | 536B |
| `src/utils/conversionUtils.ts` | 1,823B |

변환 계수 정확성 확인: **2.54**(inch↔cm) · **2.20462**(kg↔lb) · **32**(화씨 오프셋).
컴포넌트가 분리된 다중 디렉터리 React 앱이며 더미가 아니다.

### ⚠️ 이 PASS 가 신뢰할 수 있는 이유

**`best-effort PASS` 를 제거한 뒤의 결과**다(결함 B1). 이전에는 재작업 상한에 도달하면 리뷰어가
강제로 `PASS` 를 찍고 오케스트레이터가 모든 `END` 를 `DONE` 으로 마킹해, **미해결 결함을 안고
'완료'로 보이는 가짜 통과**가 구조적으로 가능했다. 지금은 `terminal_status` 가 `COMPLETED` 가
아닌 종결은 전부 실패로 기록되므로, 이 6건의 `DONE` 은 **리뷰어가 실제로 승인한 것**이다.

### 완주까지 제거한 병목 (8회 이동)

| 실행 | 막힌 지점 | 원인 |
|---|---|---|
| v3·v4 | 코드 생성 전량 llama | Pro 체인에 살아있는 Gemini 부재 |
| v5 | 체인이 유료 백스톱 하나로 붕괴 | 일시적 실패에 30분 배제 |
| v6 | `TECH_SPEC` 0.65 고정 | llama 역량 한계(`api_contract=0.4`) |
| v7 | 렌더 검증 9회 실패 | 하네스가 다중 디렉터리 구조를 거짓 반려 |
| v8 | 리뷰어가 없는 파일을 지적 | **리뷰어에게 코드가 주어지지 않음** |
| v9 | 아키텍처 혼재(React+바닐라) | **파이프라인에 파일 삭제 수단 부재** |
| v9 | 첫 실패에서 완주 포기 | 드라이버(테스트 하네스) 결함 |
| — | 에스컬레이션 미발동 | QA·Supervisor 가 리뷰어 PASS 뒤에만 존재 |

### 교훈

1. **하네스가 앱을 반려하면 하네스를 먼저 의심하라.** 제거한 결함 중 **3건이 하네스의 거짓 반려**였다
   (브라우저 전역 부재 / 스모크 cwd / 다중 디렉터리 지연 로딩). 공통 패턴은 *요구한 대로 만든 것을 감점한다* 였고,
   특히 마지막 것은 **모델이 코드를 잘 분리할수록 더 확실히 반려**되는 역설이었다.
2. **같은 지적의 반복은 '개발자가 게을러서'가 아니라 '그 지시를 수행할 수 없다'는 신호다.**
   Tech Lead 가 "삭제하라"를 8회 반복했는데 시스템에 삭제 기능이 없었다.
3. **가짜 통과를 제거해야 진짜 병목이 보인다.** B1 이전에는 모든 실패가 `DONE` 으로 위장돼 어디가
   문제인지 알 수 없었다.
