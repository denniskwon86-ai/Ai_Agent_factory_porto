...
### 8.2 MDM·카탈로그·제조 경영 온톨로지·Graph RAG의 역할
...
- **용어사전·Crosswalk**: 현업 표현·동의어·외부 시스템 코드와 표준 대상을 연결한다.
- **제조 경영 온톨로지**: ECM 문맥 안에서 기준 대상·업무 사건·조직 책임·KPI·정책·재무 영향의 의미와 관계 제약을 정의한다. 사용자에게는 `기업 경영 의미지도`로 설명한다.
- **지식 그래프**: 온톨로지를 따르는 관계 인스턴스와 근거·유효기간·버전·승인 상태를 보존한다.
- **Graph RAG**: 승인된 관계를 검색·설명하는 방식이다. 권한 필터와 증빙을 반드시 통과하며 온톨로지 자체를 대체하지 않는다.
...
### E2 — 프로필 기반 생성·권한 강제 🔶 **프로필 상속·바인딩 완료 (2026-07-28)**

> #### ✅ 완료 — 프로필 상속 병합 + 템플릿↔마스터 바인딩
>
> 구현: `core/enterprise_context/profile_resolver.py` · `api/routes/enterprise_context_control.py`
> (`GET /contexts/{scope_id}/resolved-profile`, `GET /templates/{id}/binding`) ·
> `core/context_engine.py`(바인딩 규칙 프롬프트 주입) · 테스트 29건(전체 648 통과)
>
> **§4.4 상속 규칙을 그대로 구현했다** — "충돌 시 가장 하위의 **승인된** 프로필이 이긴다".
> 두 단어를 다 지켰다: *가장 하위*(현장이 본사보다 자기 공정을 잘 안다 — §13 "업종 템플릿이 현장
> 예외를 삭제" 방지) · *승인된*(DRAFT 는 병합에 참여하지 않고 `skipped` 에 사유와 함께 남는다).
> **리스트는 병합하지 않고 교체한다** — 이어붙이면 그 공장에 없는 공정이 프롬프트에 들어가
> 환각의 근거가 된다. `inheritance_mode='replace'` 는 상위를 통째로 버린다.
> 응답에 `sources`(적용 순서)·`skipped`·`chain` 을 함께 담는다 — "이 값이 어디서 왔나"에 답할 수
> 없으면 프로필 기반 추천을 신뢰할 수 없다(§5.2).
>
> **감사 [ENTERPRISE-01] Action 3 반영** — 템플릿별 필수 마스터 바인딩으로 LLM 환각을 차단한다.
> ⚠️ 기준정보를 주입하는 것만으로는 부족하다는 것이 감사 지적의 핵심이었다. 그래서 `ContextEngine`
> 이 기준정보 본문 **바로 뒤에** "이 수치는 계산 결과이니 지어내지 말고, 없으면 '계산할 수 없음'
> 이라 밝히라"는 규칙을 함께 주입한다(`render_binding_block`). 5개 템플릿의 `required_master_sections`
> 가 실제 마스터 JSON 루트 키와 일치함을 테스트로 고정했다.
> 감사 권고에 대한 대안 2건은 `.agents/DECISIONS.md` [R-001](마스터 본문은 MDM 에 두고 바인딩만) ·
> [R-002](바인딩 단위를 시나리오 ID → **템플릿**으로 정규화)로 제출했고 R-002 는 이미 반영했다.
>
> **수용 기준 3 달성** — "같은 요청이라도 선택 조직에 따라 다른 추천이 나온다".
> 실측: 같은 경영계획 플레이북에서 **동제련 사업부**는 현금흐름 요구를 제외하고, **배터리소재
> 사업부**는 제품 마스터를 필수로 올리며 환경 배출 허가를 추가한다.
> ⚠️ 단 **조직이 플레이북의 `required` 를 지우거나 선택으로 내릴 수 없다.** 조용히 지우면 준비도가
> 부풀려지고 결손 안내도 뜨지 않는다(M0-a 에서 같은 유형의 결함을 겪었다) — `blocked` 로 돌려
> 사람이 보게 한다.
>
> **미완(E2 잔여)**: ~~MDM/카탈로그/크로스워크/MCP 에 범위 키 도입과 권한 강제~~ **✅ 완료
> (2026-07-29)** — MDM=R-001 바인딩 / 카탈로그·용어사전·계약=D-013 / **크로스워크·MCP=D-016**.
> 남은 것: 에이전트팩 바인딩, 공유서비스·연결집계의 도메인별 접근 검증,
> `scope_assignments`(ECM 전용 권한 테이블). 감사 Action 1 은 R-001 확인 후 진행한다.
>
> ⚠️ **크로스워크·MCP 에서 실재한 누출을 닫았다**(D-016): `mcp_broker.get_live_context()` 가
> 활성 연계 시스템을 **전부** 순회해 실측값을 프롬프트에 병기했다 — 배터리소재 프로젝트의
> 프롬프트에 동제련 시스템 값이 들어갔다. 상태 모델에는 조직 문맥이 이미 있었고 이 경로만
> 그것을 보지 않았다. **프롬프트 유출은 화면 유출보다 늦게 발견된다**는 것이 이 건의 교훈이며,
> 앞으로 프롬프트에 무언가를 주입하는 경로를 만들 때는 조직 문맥 배선을 먼저 확인한다.
