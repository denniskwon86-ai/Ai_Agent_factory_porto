...
> 작성: 2026-07-22 | 상태: **설계 초안(검토 대기 — 열린 질문 다수)** | 선행: M1·M2(구현 완료)
...
## 9. 열린 질문(검토 시 결정 — M3 는 미정 항목이 많음)
- **(a) MCP 연결 방식**: (1) 처음부터 실제 MCP 프로토콜 클라이언트 구현 vs (2) **어댑터 인터페이스 +
  목(mock) 어댑터로 시작**(실 MCP 는 후속). 완주·검증 기조상 (2) 추천 — 로직/캐시/보안을 먼저 굳히고
  실 커넥터는 교체 가능하게.
- **(b) 캐시 저장 위치**: `master.db` 확장 vs 별도 `mcp_cache.db`(런타임 성격이 더 강함). gitignore 대상.
- **(c) 조회 트리거**: 순수 온디맨드(pull) vs 활성 매핑 사전 프리페치. v1 은 온디맨드만 추천.
- **(d) ContextEngine 실측 병기 기본값**: off(명시 요청 시만) 추천 — 매 LLM 호출에 외부 지연·쿼터·신뢰
  리스크를 얹지 않기 위해.
- **(e) 쓰기(read-write) 로드맵 포함 여부**: v1 읽기전용 확정, 쓰기는 별도 설계로 분리 추천.
