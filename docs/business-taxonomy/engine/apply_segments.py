...
for r in uni:
    ...
    # **지주회사 자신은 인스턴스로 세운다 — 단위 「묶음」으로.** 설계는 「만들되
    # 정렬·합계에서 제외」였는데(README 묶음 노드 · D-12) 구현이 아예 빼고 있었다.
    # 그래서 1,748 건이 롱리스트에서 통째로 사라졌다 — SK(주) 3.6조·LG 8,849억도
    # 안 보인다. 지주회사도 하는 일이 있고(그룹 경영관리·브랜드 로열티·투자심의)
    # 업무키트 대상이 될 수 있으므로 **보이기는 해야** 한다.
    # 셀 집계·매출 합계에서는 계층 T1u 와 같은 방식으로 뺀다 (D-38)
    # **사업지주는 자기 사업 인스턴스를 만든다** — D-12 가 그렇게 정했는데 지켜지지
    # 않고 있었다. SK이노베이션(정유 4.85조 · `C192`)·포스코홀딩스(철강 1.4조 ·
    # `C2411`)가 묶음으로 빠져 격자에서 사라졌다. **A세분류가 「지주·투자」가 아니면
    # 사업지주다** — KSIC 가 사업코드라는 뜻이고, `build_universe` 의 지주보정이
    # 이미 그렇게 판정한 것이다. 34 건이 여기 해당한다 (D-38)
    if r['묶음노드'] == 'Y' and r['A세분류'] and r['A세분류'] != '지주·투자':
        pass                                   # 아래 법인 경로로 내려간다
    elif r['묶음노드'] == 'Y':
        held += 1
        if r['A세분류']:
            inst.append({'단위': '묶음', '모법인': '', '이름': r['회사명'],
                         '계층': r['모수계층'], '소속그룹': r.get('기업집단명들', ''),
                         '종업원수': r.get('종업원수', ''),
                         'A대분류': r.get('A대분류') or A대분류.get(r['A세분류'], ''),
                         'A세분류': r['A세분류'], 'B1주업종': r['B1주업종'],
                         'B1_2단': r['B1_2단'], 'B1_3단': r.get('B1_3단', ''),
                         '매출': r['매출액'], '매출기준': r.get('매출기준', ''),
                         '판정근거': '묶음 노드 — 매출이 자회사와 겹쳐 집계에서 뺀다'})
        continue
    if sr:
...
print(f'세그먼트로 대체한 회사 {replaced}건 · 지주(묶음) {held}건은 단위 「묶음」으로 세운다')
if shell:
...
# **셀 분포는 T1·T2 의 사업 인스턴스만 센다.** 두 가지를 뺀다 —
# ① T1u(비상장 공시법인)는 매출을 모르고 SPC·펀드가 섞여 있다
# ② 단위 「묶음」(지주회사)은 매출이 자회사와 겹친다. 롱리스트에는 보이지만
#    셀 분포·합계에 넣으면 같은 사업이 두 번 세어진다 (D-38)
# 원래 주석: T1u(비상장 공시법인)는 매출을 모르고 SPC·펀드가
# 대량 섞여 있어 함께 세면 커버리지가 왜곡된다 — 롱리스트에는 남긴다
core = [x for x in inst if x.get('계층') != 'T1u' and x['단위'] != '묶음']
cells = collections.Counter((x['A세분류'], x['B1주업종']) for x in core)
tot = len(core)
print(f'  그중 집계 대상(T1·T2 사업) {tot}건 · 제외 {len(inst)-tot}건'
      f" (T1u {sum(1 for x in inst if x.get('계층') == 'T1u')} · 묶음 {sum(1 for x in inst if x['단위'] == '묶음')})")
print(f'\n관측 셀 {len(cells)}칸')
print('\n=== 커버리지 (누적) ===')
acc = 0
