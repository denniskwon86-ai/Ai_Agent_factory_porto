...
_비부문 = re.compile(
    ...
    # 표본 검증에서 새어 나온 것들. 「대손충당금」(HD현대일렉트릭)·「자기주식」(오리온)
    # ·「불리한 변동」(한국자산신탁)·「전기 초」(코리아써키트)·「신한울 3,4호기 원자로설비」
    # (두산)이 부문으로 뽑혔다. 앞의 「기초|기말」은 `^` 앵커라 「전기초」를 놓친다
    r'|대손|충당|주식|변동|호기|[당전]기[초말]'
    # 2차 표본에서 새어 나온 것들. 「순액」·「총액」은 표시기준 열이고(부문이 아니다),
    # 「재무상태표 표시」·「손익계산서 표시」는 표의 구획 제목이다
    r'|순액|총액|재무상태표|손익계산서'
    # **부문 수집을 420 → 715 개사로 넓히자 새 유형이 들어왔다** (D-45).
    # 「연결조정|연결후|연결전」을 따로 적어 둔 탓에 「연결장부가액」(이지홀딩스 3.4 조)·
    # 「연결제거분개」(서울도시가스)·「연결」 단독(대한제당 1.4 조)이 새어 나왔다.
    # 정상 부문명이 「연결」로 시작하는 경우는 없으므로 **`^연결` 을 통째로 막는다**
    r'|연결'
    # 회계정책 변경 표 — 「변경 전」과 「변경 후」가 **같은 금액으로 두 번** 들어가
    # 이중계상된다(서울반도체 8,714 억 × 2 · 무신사)
    r'|변경'
    r'|종목'                  # 투자 명세표의 「종 목」 열
    r'|USD|EUR|JPY|CNY|GBP|HKD|외화환산'   # 외화 표의 통화 열
    # 유형자산·부채 명세표. 「토 지」·「건 물」처럼 띄어 써도 공백을 지우고 보므로 걸린다
    r'|기계장치|토지|건물|구축물|비품|차량운반구|건설중인자산'
    r'|[장단]기차입금|차입금|사채|충당부채|리스부채|파생상품'
    r'|발생사고|미경과|보험부채|책임준비금'     # 보험 부채 명세(교보생명 「발생사고요소」)
    r'|^감소$|^증가$|^추가$|^처분$'            # 변동표의 증감 열(화신 「감소」)
    # **한 항목을 막으면 같은 표의 다른 행이 대신 들어온다.** 화신에서 「감소」를 막자
    # 「대여금·매입채무·미지급금」이 올라왔다 — 그 표 자체가 재무 명세표다.
    # 계정과목은 부문명이 될 수 없으므로 통째로 막는다
    r'|대여금|매입채무|매출채권|미지급|미수금|선급|선수|예수|이연법인세|퇴직급여'
    # 무형자산 명세표의 항목들. HMM 은 「총액·순액」 열을 막자 그 표 대신 무형자산
    # 표가 뽑혀 「사용권·탄소배출권·회원권」이 부문이 됐다.
    # **표 단위로 막으면 안 된다** — K-IFRS 부문 공시는 부문별 자산에 「영업권」·
    # 「무형자산」을 함께 싣는다. 표를 배제하자 동원산업·CJ·한화오션·삼성SDS 등
    # 14 개사의 정상 부문 38 건이 통째로 죽었다. 그래서 **항목명만** 막는다.
    # 「소프트웨어」는 빼 둔다 — SW 회사의 실제 부문명이다
    r'|사용권|배출권|회원권|영업권|개발비|특허권|상표권|산업재산권'
    # 채권 연령 분석표. SK네트웍스가 그랬다
    r'|정상채권|손상된)')
_재무행 = re.compile(
    r'(수익|매출|이익|손실|상각|자산|부채|자본|원가|비용|투자|증감|현금|법인세'
    # `_비부문` 은 `^` 앵커라 「자기주식」·「불리한 변동」처럼 **중간에** 오는 말을
    # 못 잡는다. 이쪽은 search 이므로 여기 둔다
    r'|주식|변동|호기|대손|충당'
    # 「탄소배출권」·「개별 손상된 채권」은 앞의 `_비부문` 이 `^` 앵커라 놓친다
    r'|배출권|손상'
    # 3차 표본에서 새어 나온 것들 — 「만기경과후 1~3년」(현대위아 채권 연령표) ·
    # 「종업원급여」(한솔아이원스) · 「사용수수료」(사조씨푸드). 「수수료」 단독은
    # 넣지 않는다 — 금융권의 「수수료수익」 부문을 죽인다
    # 「연체되지 않은 채권」·「차감 계」 — 현대위아의 채권 연령표가 끈질기다.
    # 「채권」 자체는 넣지 않는다 — 증권사의 「채권운용」 부문을 죽인다
    r'|만기|급여|사용료|사용수수료|연체|차감'
    # 「연결재무제표」(대신증권 5.1조) · 「공시재무제표」(삼성카드 4.4조)가 부문으로 섰다.
    # 앞의 `_비부문` 은 `^` 앵커라 「재무제표」가 가운데 오면 놓친다
    r'|재무제표|재무현황'
    # 세그먼트 3 차 표본에서 새어 나온 것들 — 신세계톰보이 「총장부금액」·「집합평가대상채권
    # 금액」(채권 평가 표) · 한솔아이원스 「복리후생비」·「지급수수료」(비용 명세).
    # 「금액」으로 끝나는 부문명은 없다
    r'|금액|복리후생|지급수수료|평가대상'
    # 접수번호 색인으로 되살린 32 건에서 새로 새어 나온 것들 — 웅진씽크빅
    # 「17차부여분」(주식매수선택권 부여 표) · 우리금융지주 「영업부문항목명」·「내부조정」
    r'|차부여분|부여분|항목명|내부조정'
    # 「부문간내부거래 조정」·「중단영업조정」·「중단영업 (내부거래제거전)」 — 앞의
    # `_비부문` 에 「내부거래」·「조정」이 있지만 `^` 앵커라 가운데 오는 것을 놓쳤다.
    # 셀트리온·신세계푸드·소노스퀘어의 조정 행이 부문으로 서 있었다
    r'|내부거래|중단영업)')
...
_수익제외 = re.compile(
    ...
    r'|기초|기말|증가|감소|환입|전입|이연|배분|평가|처분|환산|미수|스왑|파생)')
...
_지역 = re.compile(
    ...
    # 고려아연은 「국내 / 호주」로만 부문을 가른다 — 「국내」만 막혀 「호주」가 사업
    # 부문으로 남았다. 지역만 공시하는 회사는 **0 건이 정답**이다
    r'|호주|대만|태국|인도네시아|싱가포르|말레이시아|필리핀|멕시코|캐나다|독일|영국'
    r'|프랑스|이탈리아|스페인|폴란드|체코|터키|튀르키예|헝가리|슬로바키아|남아프리카'
    r'|칠레|페루|아르헨티나|사우디|아랍에미리트|카타르|이집트|나이지리아|케냐'
    # 국내 지역으로 가르는 회사도 있다 — 부동산·건설사가 그렇다. 「인천광역시」가
    # 사업 부문으로 남았다. 광역 지자체 17 곳을 접두사로 둔다
    r'|서울|부산|대구|인천|광주광역|대전|울산|세종|경기|강원|충청|충북|충남'
    r'|전라|전북|전남|경상|경북|경남|제주)'
    # 지역명은 단독으로 온다. 뒤에 붙어도 되는 건 괄호주석·「및~」·「외」·「지역」과
    # 행정구역 접미사뿐이다. 「사업」「부문」이 붙으면 지역이 아니라 부문이다 —
    # 이마트 「해외사업」이 그렇다. **접미사를 좁게 두는 게 중요하다**: 「부산은행」
    # 「대구은행」 「광주은행」 같은 지방은행 부문이 지역으로 오인되면 안 된다
    # 「경기도 안산시」처럼 광역 뒤에 기초 지자체가 붙는다(삼천리)
    r'(?:및[가-힣]+|\(.*\)|외|지역|특별시|광역시|특별자치[시도]|[시도])?'
    r'(?:\s*[가-힣]{2,4}[시군구])?$')
...
# **수익 원천 분해는 부문이 아니다.** K-IFRS 1115호 공시라 「상품의 공급」
# 「용역의 제공」처럼 수익 인식 유형을 열로 세운다 — 레인보우로보틱스가
# 이걸 3개 부문으로 내놓았다
_수익원천 = re.compile(
