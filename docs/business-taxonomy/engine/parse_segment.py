...
_수익우선 = [
    ...
    re.compile(r'(순매출액|순수익|외부매출)'),
    re.compile(r'(총부문수익|부문수익|총매출액|매출액|영업수익)'),
    re.compile(r'수익'),
]
_수익제외 = re.compile(r'(내부|부문간|사내|제거|조정|대체)')
...
def _revenues(tables) -> dict[str, int]:
...
def parse(xml: str) -> list[dict]:
    ...
    for sec in _sections(xml):
        if _단일.search(sec):        # 부문이 하나뿐이라고 밝힌 섹션은 표를 읽지 않는다
            continue
        got = _parse_one(sec)
        ...
        if not prod:
            for k, v in prods.items():
                if k in nm or nm in k:
                    prod = v
                    break
        out.append({'명칭': nm, '주요제품': prod, '매출': revs.get(nm)})
    return out
