...
- 작성자 / 기록 시각: Codex / 2026-08-15 15:17 KST
...
- 상태: **상세설계·기계 계약·팀 공통 인수인계 완료 · 제품 코드 미구현**. 본 항목은 G2-B 데이터 키트와 G2-D MVA를 G1-D 안전 연계·I-4 Host Runtime·G4 기준선에 연결하는 구현 정본이다.
- 결정 및 근거:
  ...
  - 기존 `connector_registry/execution`, 카탈로그, `data_contracts`, MDM/Crosswalk, App Data Plane, External Intelligence, `baseline_snapshots`를 재사용한다. 신규 런타임은 `Kit Instance / Source Binding / Dataset Snapshot / Data Readiness / Baseline Build`만 추가한다.
  - 현업 앱 미사용 시에도 기존 Actual·마지막 인증 판으로 최소 기준선을 유지하고, 원천에 없는 결손·예외·시나리오·실행 효과만 AFS 앱이 보완한다. 결손·장애·만료·승인 전을 0건으로 접지 않는다.
- 근거: `docs/architecture/BUSINESS_DATA_BINDING_RUNTIME_DETAILED_DESIGN_2026-08-15.md`, `docs/architecture/business_data_binding_contract_v1.schema.json`, `docs/architecture/first_vertical_data_binding_profile_v1.example.json`, `docs/handoff/BUSINESS_DATA_BINDING_RUNTIME_TEAM_HANDOFF_2026-08-15.md`. 상위 정본 `Product Bible` §4.1~4.5·§5.6·§7, 제품 전략 §4.1~4.6·§8, 로드맵 G1-D·G2-B~D·G4 및 현행 Connector/Catalog/App Data/External/Baseline 코드 대조.
- 영향·주의사항: 최신 HEAD `cd6d539fa`에서 I-4는 **2.1b 완료·3단계 미착수**이고 관련 제품 소스는 깨끗하다. 3단계가 계약 지문을 증명에 봉인하기 전에 BDR-1을 **I-4 2.2 보강**으로 먼저 넣어야 지문·승인 계약을 두 번 바꾸지 않는다. 기존 계약의 미분류 데이터셋을 자동 `AFS_NATIVE` 또는 Actual로 승격하지 않는다. LPL은 제품 공통 구현이 아니라 범용 외부 협업 시스템의 Reference Profile이다. 합성·DEMO·SANDBOX는 공식 기준선 승격 금지. origin은 건드리지 않는다.
- 다음 행동 / 담당 / 착수 조건: **Claude Code**는 최신 작업트리를 다시 확인한 뒤 BDR-1(App Runtime Contract 출처 의도·데이터 역할·중복입력 게이트)을 I-4 2.2 별도 커밋으로 먼저 구현하고, 그 다음 I-4 3단계에서 새 의미 필드까지 지문에 봉인한다. **Codex**는 회사 데이터 준비 보드·Source Binding·인증 워크벤치 UX와 구현 결과를 교차검토한다. **Antigravity**는 첫 Minimum Decision Dataset의 원천 후보·품질·최신성·I/F 비용과 합성 오염을 독립 검증한다. **Supervisor**는 실제 적용 시 Data Owner·원천 접근·허용 오차·L2/L3 비용을 승인한다.
- 교대 체크포인트: 마지막 확인 = 상세설계·Schema·예시 Profile·공통 인수인계 작성 완료. PowerShell `Test-Json -SchemaFile`에서 예시 Profile **Schema 통과**, Node 구조 대조에서 `$defs` 6/6·Dataset 3종·오류 0. 최신 HEAD=`cd6d539fa`, I-4 2.1b 완료·3단계 미착수. 변경 범위 = 문서·본 보드 항목만, 제품 코드/DB/API/테스트 0. 커밋/푸시 = 미실시. 재개 지점 = BDR-1을 I-4 2.2로 구현할 영향 파일·집중 회귀 계획 보고. 금지 범위 = I-4 3단계 선행 후 의미 계약 재수정, 기존 원천의 중복 입력 UI, 범위 미지정 공용 처리, live query의 공식 계산 직결, 샘플 데이터 Actual 승격.

#### 후속 판정 — I-4 2.1b 승인 · 3단계 전 BDR-1/2.2 삽입
- 작성자 / 기록 시각: Codex / 2026-08-15 KST
- 왜 지금 기록하는가: Claude Code가 `cd6d539fa`에서 교차 tenant 결속, 깨진 결속의 마스터 후퇴, 지문 백필·폭, 버전 호환, 무결성 게이트를 닫고 3단계 착수 승인을 요청했다. 동시에 Supervisor가 확정한 데이터 연결·준비 설계는 계약 지문 봉인 전에 출처 의미와 중복입력 정책을 계약에 포함해야 한다.
- 상태: **I-4 2.1b 승인 · 3단계 착수는 BDR-1/2.2 완료 후로 순서 보정**. `_assert_bindable`, `_apply_bound_schema`, `AppDataIntegrityError→503`, 전체 sha256 지문, `single_candidate_unverified`는 승인한다.
- 결정 및 근거: 3단계는 승인 계약 원문과 실제 결속 지문을 앱 증명에 봉인한다. 지금 `source_intent`, `data_role`, `duplicate_entry_policy` 없이 봉인하면 기업 Actual과 AFS Native 입력을 계약상 구분하지 못한 상태가 보안적으로 고정되고, BDR-1 적용 시 지문·승인을 다시 바꿔야 한다. 따라서 `2.1b → 2.2(BDR-1) → 3단계`가 정본 순서다. I-4 인수인계 최상단에도 같은 보정을 반영했다.
- 다음 행동 / 담당 / 착수 조건: Claude Code는 `core/app_runtime_contract.py`, `core/host_contract_compiler.py`와 관련 계약·상태 테스트에 BDR-1을 별도 원자 커밋으로 구현한다. 필수 회귀 10건과 변이 검사는 팀 공통 인수인계 §6을 따른다. 이후 3단계가 새 의미 필드까지 원문 지문에 포함하고 old proof/410/stale frame을 검증한다.
- 영향·주의사항: 2.1b 코드는 되돌리지 않는다. `ENTERPRISE_READ`, `EXTERNAL_REFERENCE`, `DERIVED_READ`를 현재 `AFS_NATIVE`로 폴백하지 않는다. 기존 계약 미분류는 자동 승격하지 않고 `LEGACY_UNCLASSIFIED` 감사 대상으로 남긴다. 제품 코드는 본 판정에서 수정하지 않았다.
- 교대 체크포인트: 확인 커밋=`cd6d539fa`, 집중 코드 diff와 I-4 인수인계 대조 완료. 본 변경은 문서·보드만이며 테스트 재실행은 하지 않았다(Claude 증거 3,398 passed·변이 183/183 수신). 커밋/푸시 미실시. 재개 지점=I-4 2.2 BDR-1. 금지 범위=3단계 선행, 기업 Actual 입력 UI 생성, 지문 필드 누락, 레거시 자동 Actual 승격.

### [G1-B-BROWSER-CANARY-88] 실제 Preview 종단 완료 · 410 캐시 회귀 보정 · 구조 지문 잔여 1건
- 작성자 / 기록 시각: Codex / 2026-08-15 01:30 KST
- 왜 지금 기록하는가: Claude Code가 `7796bd056`, `59e68ca9b`와 fresh `canary_4`(26/26, dynamic 5/5, structural 5/5, `safe_to_switch=True`)를 인계해, 서버 카나리가 증명하지 못한 실제 Preview `window.afs` 브라우저 축을 격리 세션·DB에서 종단 검증했다.
- 상태: **[4] 실제 브라우저 핵심 종단 통과 · P0 제품 회귀 3건 보정 · [6] 원자적 전환은 구조 지문 잔여 때문에 보류**. Host Runtime의 schema/create/list/get/update/remove는 실제 iframe에서 모두 성공했고, 조직 범위 미선택 409는 부모 화면의 명시 안내와 `다시 시도` 행동으로 도달했다. Manifest 변경 뒤 기존 iframe은 410/EXPIRED로 격리됐다. 정상 TTL 만료는 `canary_4` 서버가 실제 16분 경과로 검증했고 브리지 재발급은 실행형 회귀로 검증했으나, **동일 실제 브라우저를 16분 유지한 결합 시나리오는 다시 반복하지 않았다**.
- 발견·보정 1 — 릴리스 열기 인자: `BuildPage`가 `onOpenRelease`에 릴리스 객체 전체를 넘겨 `/library/item/[object Object]` 400을 냈다. 콜백 계약을 `(releaseId: string) => void`로 좁히고 버튼이 `release_id`만 전달하도록 `frontend/src/components/BuildPage.tsx`, `frontend/src/App.tsx`를 보정했다.
- 발견·보정 2 — Host Runtime 플래그가 실제로 켜지지 않음: `hostRuntimeBridge.ts`의 `(import.meta as any)?.env`가 Vite 정적 치환을 방해해 `VITE_AFS_HOST_RUNTIME=1`이어도 비활성 상태였다. `(import.meta as any).env`로 고쳐 실제 브라우저에서 `Host Runtime 연결됨`과 6작업 성공을 확인했다.
- 발견·보정 3 — **이전 410이 새 증명에 재사용됨**: Manifest stale를 만든 뒤 라이브러리에서 앱을 다시 열면 새 증명은 현재 지문으로 발급됐지만, 브라우저가 같은 URL의 이전 410을 캐시해 서버 호출 없이 계속 EXPIRED를 돌려줬다. `main.py`가 `/api/v1/appdata/runtime`의 성공·거부·오류 응답 모두에 `Cache-Control: no-store`, `Pragma: no-cache`를 강제하고, 부모 브리지 요청도 `cache: 'no-store'`로 고정했다. 보정 후 새 앱 재열기 → 새 증명 → 실제 schema 200이 다시 서버에 도달했다.
- 검증: 격리 worktree/DB/세션 사용. 실제 Preview에서 schema·create·list·get·update·remove 6/6, 오류 0; Manifest stale 기존 프레임 차단과 새 앱 재열기 복구; 조직 범위 미선택 안내·재시도 UI 통과. 정상 만료는 서버 actual-expiry + 브리지 실행형 회귀의 결합 증거이며 실제 브라우저 장시간 대기는 미반복. 집중 회귀 `tests/test_app_data_runtime.py tests/test_host_runtime_bridge_contract.py` **90 passed**, 전체 `pytest -q` **종료코드 0**(1 skipped), `npm.cmd run build` 성공. 화면 증거는 `tmp/browser_canary/evidence/schema_after_reopen.png`, `scope_missing.png`에 임시 보관했다.
- 남은 결함 — 구조 지문이 증거 생성 드라이버를 포함하지 않음: `core/policy_shadow.py::STRUCTURE_SOURCES`는 7개 구현 파일을 묶지만 실제 `record_structure()`와 `switch_gate()` 호출 순서를 결정하는 `scripts/canary_host_runtime.py`를 제외한다. 드라이버를 약화해도 옛 구조 증거가 유효하게 남을 수 있어, REVIEW-87에서 요구한 「증거 산출 절차까지 지문 결속」은 아직 미충족이다. 드라이버를 목록에 포함하고 지문 변경 회귀를 추가한 뒤 새 run label로 서버 카나리를 1회 재실행해야 [6] 전환 근거가 완결된다.
- 영향·주의사항: Host Runtime은 제품 기본 플래그를 임의로 켜지 않았다. 브라우저 카나리용 `.env.local`·로그인 부트스트랩은 제거했다. 진단 중 브라우저 콘솔에 표시된 임시 증명은 백엔드 재시작으로 메모리 저장소에서 폐기됐고 진단 로그 코드는 제품 소스에서 제거했다. 다른 세션의 온톨로지·브로셔·데이터 파일은 스테이징하지 않는다.
- 다음 행동 / 담당 / 착수 조건: Claude Code가 `scripts/canary_host_runtime.py`를 구조 지문에 포함하고 fresh 서버 카나리를 1회 재실행한다. Codex는 결과의 code hash와 게이트를 확인한다. 그 조건이 충족되면 [5] 완료 및 [6] 원자적 전환을 승인할 수 있으며, 브리지 동작을 바꾸지 않는 한 실제 브라우저 6작업은 재실행하지 않아도 된다.
- 교대 체크포인트: 제품 변경 = `main.py`, `frontend/src/App.tsx`, `frontend/src/components/BuildPage.tsx`, `frontend/src/lib/hostRuntimeBridge.ts`, 테스트 2파일, 본 보드. 임시 카나리 파일 제거 완료. 커밋/푸시 미실시. 재개 지점 = 구조 지문 보정 커밋·fresh run 증거 확인. 금지 범위 = 캐시 가능한 증명 결속 응답, 릴리스 객체를 URL 식별자로 전달, 구조 드라이버 미결속 상태에서 [6] 전환.

#### 후속 판정 — [5] 완료 및 [6] 원자적 전환 승인
- 작성자 / 기록 시각: Codex / 2026-08-15 02:00 KST
- 왜 지금 기록하는가: Claude Code가 마지막 차단을 `f97dbdc70`에서 보정하고 fresh `canary_5`를 단일 실행했으므로, 위의 보류 조건이 실제로 해소됐는지 재판정했다.
- 상태: **[5] 전환 게이트 완료 · [6] 원자적 전환 착수 승인**. `STRUCTURE_SOURCES`는 `scripts/canary_host_runtime.py`를 포함한 8개가 되었고, 현재 코드의 `structure_hash()`는 증거와 동일한 `3b12d82d7f965600`이다. `canary_5`는 26/26, dynamic 5/5, structural 5/5, looser/error/unexplained 0, `safe_to_switch=True`, EXIT=0이다.
- 검증 근거: Claude 증거 `canary_5_evidence.txt` 원문 대조, `git show f97dbdc70`에서 변경 파일 2개만 확인, `tests/test_policy_shadow_gate.py` **47 passed**. 드라이버 포함 여부·대상 파일 변경 시 지문 변화·LF/CRLF 동일 지문 회귀가 모두 존재한다. 브라우저 카나리 보정 6개 제품/테스트 파일은 별도 로컬 커밋 `7572d0a72`로 원자화했으며 전체 pytest 종료코드 0·프런트 빌드 통과 상태다.
- 다음 행동 / 담당 / 착수 조건: Claude Code는 추가 승인 대기 없이 [6] 원자적 전환을 착수할 수 있다. 전환은 Shadow와 강제 경로를 한 커밋에서 뒤섞지 말고, 플래그·라우트·롤백 스위치·전환 후 smoke 증거를 하나의 변경 단위로 남긴다. 전환 후에는 [7] I-4 생성기 연동 전에 기존 관리 API가 앱 경로의 폴백으로 되살아나지 않았는지 확인한다.
- 영향·주의사항: 승인 범위는 G1-B Host Runtime의 Shadow→강제 전환이다. G2 온톨로지 구현이나 운영 데이터 이관까지 자동 승인하는 것이 아니다. origin은 건드리지 않는다. 공유 보드·온톨로지·브로셔·샘플 데이터의 미커밋 변경을 [6] 커밋에 섞지 않는다.
- 교대 체크포인트: HEAD=`7572d0a72`, 선행 구조 지문 커밋=`f97dbdc70`, 브랜치 `dev`는 origin/dev보다 12커밋 앞섬. Codex 제품 수정은 로컬 커밋 완료·push 미실시. 공유 보드는 여러 팀원 변경이 함께 있어 미커밋 유지. 재개 지점=[6] 전환 커밋 검토. 금지 범위=origin push, 타 작업 파일 스테이징, 구조 게이트 조건 완화, 관리 API 세션 폴백 재도입.

### [G1-B-CANARY-REVIEW-87] 구조 게이트 재검토 — 열거 P0 승인, 수동 증거·약한 공격자 보정 전 safe_to_switch 보류
- 작성자 / 기록 시각: Codex / 2026-08-14 KST
- 왜 지금 기록하는가: Claude Code가 `71d75b53a`에서 만료 증명 열거를 닫고 동적 5종/구조 5종 게이트로 분리해 `canary_3 safe_to_switch=True`를 보고했으며, 실제 Preview 브라우저 레그의 수행 주체 결정을 요청했다. 커밋된 카나리만으로 같은 게이트 결과를 재현할 수 있는지와 구조 증거의 강도를 재검토했다.
- 상태: **P0 열거 수정 승인·구조 게이트 조건부 승인·[5] 전환 보류** — `_judge`가 `_dataset`보다 먼저 실행되고 거부되는 증명에서 존재/부재 이름의 상태·본문이 동일한 계약은 승인한다. 동적 거부와 구조 불변식을 분리한 방향도 승인한다. 다만 아래 세 결함 때문에 현재 보고된 `safe_to_switch=True`는 전환 근거로 채택하지 않는다.
- 결함 1 — 구조 증거 기록이 커밋된 카나리에 없음: `scripts/canary_host_runtime.py:271-285`는 `STRUCTURE` JSON을 출력할 뿐 `policy_shadow.record_structure()`를 한 번도 호출하지 않고 최종 `switch_gate(run_label)`도 읽지 않는다. 저장소 전체 검색에서도 호출자는 단위 테스트뿐이다. 따라서 `canary_3`의 구조 5/5와 `safe_to_switch=True`는 별도 수동 명령으로 기록된 결과이며, 커밋된 절차만으로 재현할 수 없다. 모든 HTTP 단계가 성공한 뒤에만 구조 5종을 같은 `run_label`로 기록하고, 같은 드라이버가 게이트 결과를 읽어 `safe_to_switch`가 아니면 종료코드 1을 반환하도록 원자화한다.
- 결함 2 — 교차 앱 공격자가 읽기 전용: seed의 `rel_canary_other`는 `orders.read`만 선언한다. 본문·payload 쓰기 주입이 HTTP 403인 이유는 주소 격리가 아니라 단순 capability 거부이며, 데이터 귀속 코드까지 도달하지 않는다. B 앱에도 read/write/delete를 부여하고 A의 살아 있는 sentinel을 유지한 채 B로 create/update/delete를 실제 허용 경로까지 통과시킨다. B 쓰기는 B 데이터셋에만 생성되고, A sentinel id의 get/update/delete는 모두 404이며, A sentinel 값·개수는 불변이어야 한다. query/header/top-level/payload의 app_id/release_id 주입도 각각 효과가 없거나 명시적으로 거부됨을 검증한다.
- 결함 3 — 구조 지문 범위 불완전: `STRUCTURE_SOURCES`가 route·app_proof·host_runtime_sdk만 포함한다. 실제 귀속을 구현하는 `core/app_data.py(find_dataset/get_record/CRUD)`, 토큰 결속의 `core/app_capability_token.py`, PDP의 `core/app_policy.py`, 증거 산출 절차인 `scripts/canary_host_runtime.py`, 게이트 자체인 `core/policy_shadow.py`가 바뀌어도 옛 증거가 유효하게 남는다. 이 파일들을 지문에 포함하고, 어느 하나를 바꾸면 기존 구조 증거가 무효가 되는 회귀를 추가한다.
- 표본 순도: `canary_3 total=169`는 최종 25단계에 비해 지나치게 커 디버깅 재실행 표본이 같은 run label에 누적된 것으로 보인다. 보정 후 새 격리 worktree·새 DB·새 `run_label=canary_4`에서 최종 드라이버를 **한 번만** 실행한다. 이전 실패·실험 표본과 최종 판정을 섞지 않는다. 운영 DB 무오염은 파일 개수뿐 아니라 경로별 hash·mtime 전후 불변으로 남긴다.
- 브라우저 레그 결정: 사용자에게 비밀번호 입력이나 세션 토큰 전달을 요구하지 않는다. 위 보정으로 서버 게이트가 열린 뒤 Codex가 `browser:control-in-app-browser`를 사용해 격리 worktree의 카나리 전용 세션을 브라우저 컨텍스트에 주입하고 실제 Preview `window.afs`의 schema/list/get/create/update/remove, 범위 미선택 안내, 정상 만료 재발급, Manifest stale 후 라이브러리 재열기를 검증한다. 현재 8083·8087·5173은 모두 내려가 있고 canary worktree도 제거돼 있어, 보정 전 구버전으로 브라우저를 다시 띄우지 않았다.
- 검증: `tests/test_app_data_runtime.py`, `tests/test_app_data_policy_shadow.py`, `tests/test_policy_shadow_gate.py` 집중 회귀 **91 passed**. 기존 초록은 카나리 드라이버가 구조 증거를 실제 기록하는지, B가 쓰기 권한으로 귀속 경계까지 도달하는지, 지문 누락 파일 변경이 증거를 무효화하는지를 검사하지 않는다.
- 다음 행동 / 담당 / 착수 조건: Claude Code가 구조 기록+게이트 판정을 드라이버에 원자화하고, writable B 교차 앱 시험, 완전한 구조 hash, fresh canary_4를 완료한다. 결과는 HTTP 25+ 단계, dynamic 5/5, structural 5/5, looser/error/unexplained 0, safe_to_switch true, 운영 DB hash/mtime 불변을 한 JSON에 남긴다. 이후 Codex가 격리 브라우저 레그를 수행·기록하고 통과 시 [4]를 완료 처리해 [5] 전환을 승인한다.
- 영향·주의사항: `record_structure(True)`를 카나리 성공 여부와 무관하게 호출하거나 사람이 사후 입력하지 않는다. 구조 hash에 프론트 파일을 넣어 서버 경계와 브라우저 경계를 섞지 않는다—브라우저는 별도 종단 증거로 관리한다. 실제 Preview 미검증 상태에서 [6] 전환 준비를 선행하지 않는다.
- 교대 체크포인트: 확인 커밋 = `71d75b53a`; 제품 코드 변경 0, 본 보드 기록만 추가. 집중 회귀 91 passed. 브라우저 스킬 검토 완료, 실제 브라우저 실행은 보정된 서버 부재로 미실시. 커밋/푸시 미실시. 재개 지점 = fresh canary_4 결과 확인 후 Codex 브라우저 종단. 금지 범위 = 수동 구조 증거로 게이트 개방, read-only B의 403을 귀속 격리 증거로 채택, 불완전 hash에 옛 증거 유지, 사용자 운영 세션·비밀번호 요청.

### [G1-B-CANARY-REVIEW-86] [4] 격리 카나리 판정 — 구조 증명 채택, 만료 열거 회귀·교차 앱 증거 보정 후 재실행
- 작성자 / 기록 시각: Codex / 2026-08-14 KST
- 왜 지금 기록하는가: Claude Code가 `052d4123f`에서 격리 서버 카나리 24/24와 Shadow 표본 37건을 만들었으나 `다른 앱의 데이터`가 구조상 PDP 사유로 발생하지 않아 [5] 게이트 처리 방식 1/2안을 요청했다. 카나리가 실제 전환 증거로 충분한지 라우트 순서·게이트 정의·교차 앱 표본을 재검토했다.
- 상태: **1안 채택·현재 [5] 전환 보류** — 런타임 API에 일부러 app/release 입력을 추가해 `TOKEN_APP_MISMATCH`를 발생시키는 2안은 공격 표면만 늘리고 런타임 구조를 증명하지 못하므로 기각한다. 동적으로 관측 가능한 거부 5종과 주소 지정 자체가 불가능한 구조 불변식 1종을 별도 증거로 관리한다. 단 필수 시나리오에서 단순 삭제하거나 상수로 면제해 게이트를 낮추는 방식은 승인하지 않는다.
- P0 회귀 — 만료 토큰의 데이터셋 이름 열거: `api/routes/app_data_runtime.py:113-126`이 만료 레코드를 반환하도록 바뀌었지만 모든 Runtime 라우트는 `_judge()`보다 먼저 `_dataset(proof, name)`을 호출한다(`:331-375`, `:388-412`). 따라서 만료 토큰으로 존재하는 이름은 PDP까지 가서 401, 존재하지 않는 이름은 `_dataset`에서 404가 되어 데이터셋 존재 여부를 구분할 수 있다. **관측을 위해 접근 경계를 약화시킨 회귀**다. 만료는 데이터셋 조회 전에 PDP를 거쳐 401로 끝나게 하며, 존재/부재 이름 모두 상태·본문이 같다는 회귀를 추가한다. PDP 표본은 실제 `DENY_TOKEN_EXPIRED` 결정에서만 기록한다.
- 교차 앱 카나리 false-positive: `scripts/canary_host_runtime.py`는 정상 앱의 허용 6작업에서 만든 레코드를 `data.remove`로 먼저 삭제한 다음 다른 앱 증명으로 list가 0건인지 확인한다. 비교 대상 레코드가 이미 없으므로 이 단계의 `records=0`은 격리를 증명하지 않는다. 별도 sentinel을 A 앱에 유지하고 B 증명으로 schema/list/get/create/update/remove 및 query/header/body의 `app_id/release_id` 주입을 시도한 뒤, A sentinel의 비노출·불변과 B 자원으로만 귀속됨을 검증한다.
- 게이트 개정 계약: `scenario_coverage`에는 실제 PDP 사유로 관측 가능한 5종(만료·다른 세션·다른 사용자·다른 조직·문맥)을 유지한다. `다른 앱`은 `structural_coverage.cross_app_unaddressable`로 분리한다. 구조 통과 조건은 (1) 요청 계약에 app_id/release_id 없음, (2) release_id를 proof에서만 유도, (3) dataset을 proof release+name으로 조회, (4) record를 resolved dataset에 재귀속 확인, (5) 위 블랙박스 교차 앱 시험 전부 통과다. 구조 증거는 `run_label`과 관련 계약 파일 hash에 결속해 코드가 바뀌면 자동 만료시킨다. `safe_to_switch`는 동적 5/5와 구조 1/1을 모두 요구하고 결과를 별도 필드로 노출한다.
- [4] 잔여 브라우저 레그: 이번 스크립트는 서버 종단만 검증했고 실제 Preview `window.afs` 실행은 하지 않았다. 원래 카나리 판정의 “실제 Preview” 조건은 미충족이다. 비밀번호 입력 불가를 이유로 생략하지 말고, 격리 워크트리의 카나리 전용 사용자/세션을 Playwright 브라우저 컨텍스트의 localStorage에 주입하거나 카나리 전용 알려진 1회 비밀번호로 로그인해 schema/list/get/create/update/remove, 범위 미선택 안내, 정상 만료 재발급, Manifest stale 재열기를 화면에서 확인한다. 운영 인증 경로·운영 DB는 사용하지 않는다.
- 검증: `tests/test_app_data_runtime.py`, `tests/test_app_data_policy_shadow.py`, `tests/test_policy_shadow_gate.py` 집중 회귀 **82 passed**. 이 초록은 만료된 proof로 존재/부재 dataset 이름을 대조하지 않고, 카나리 A sentinel을 삭제한 뒤 B list=0을 보는 결함도 검출하지 못한다. 따라서 보고된 3,200 passed와 본 P0는 양립한다.
- 다음 행동 / 담당 / 착수 조건: Claude Code가 (1) 만료 proof를 dataset lookup 이전에 PDP 관측·거부하도록 순서를 보정, (2) 동적/구조 게이트 분리와 run+contract hash 결속, (3) 살아 있는 A sentinel을 사용한 교차 앱 블랙박스 카나리, (4) 실제 Preview 브라우저 레그를 완료한다. 같은 격리 조건으로 카나리를 재실행해 dynamic 5/5·structural 1/1·looser 0·error 0·미설명 강화 0을 확인한 뒤 [5] 전환 게이트를 재판정한다.
- 영향·주의사항: 관리 API 또는 Runtime route에 교차 앱 식별자를 새 입력으로 추가하지 않는다. 구조 증명은 `TOKEN_APP_MISMATCH` 단위 방어를 제거한다는 뜻이 아니며 PDP의 defense-in-depth 검사는 유지한다. 운영 DB 파일 “개수 동일”만으로 무오염을 주장하지 말고 카나리 전후 운영 DB 경로별 hash/mtime 불변도 함께 남긴다.
- 교대 체크포인트: 확인 커밋 = `052d4123f`(선행 `c204b3111` 포함); 제품 코드 변경 0, 본 보드 기록만 추가. 집중 회귀 82 passed. 커밋/푸시 미실시. 재개 지점 = 만료 lookup 순서 보정 및 구조 게이트 설계. 금지 범위 = 관측을 위해 만료 토큰으로 자원 lookup 허용, 교차 앱 사유를 만들기 위한 새 입력 경로 추가, 삭제된 sentinel로 격리 통과 판정, 실제 Preview 미검증 상태에서 [4] 완료·[5] 전환 선언.

### [G1-B-APP-PROOF-REVIEW-85] Manifest 결속 보정 재검토 — 보안 계약 승인, 프론트 상태 2건 보정 후 카나리
- 작성자 / 기록 시각: Codex / 2026-08-14 KST
- 왜 지금 기록하는가: Claude Code가 `f6346fc89`에서 REVIEW-84의 Manifest 불일치 분리, 빈 식별자 차단, 안내 복구, lifecycle 허용목록을 반영하고 [4] 격리 카나리 재승인을 요청해 서버→브리지→화면→라이브러리 재열기 경로를 종단 대조했다.
- 상태: **보안 계약 승인·[4] 착수 직전 프론트 보정 2건** — 410 분리, proof-only 재발급 차단, stale 상태의 세대 초기화 비해제, 빈 fingerprint/version 발급 거부, lifecycle 명시 허용목록은 승인한다. 자동 최신판 재적재는 P0가 아니다. 현재 `App.tsx:296-315`의 라이브러리 복귀로 Preview가 unmount되고 `useFactoryStore.viewRelease()`가 서버에서 릴리스를 다시 조회해 새 브리지·코드를 구성하므로, 사용자에게 명시한 수동 fail-closed 복구 계약이 실제로 존재한다.
- 검증: 집중 회귀 `tests/test_app_data_runtime.py`, `tests/test_host_runtime_bridge_contract.py`, `tests/test_app_capability_token.py`, `tests/test_app_policy.py` **173 passed**. `npm.cmd run build` 성공(`tsc -b`+Vite). 410은 부모 브리지에서만 stale 신호로 사용되고 iframe 응답은 기존 `EXPIRED` 고정 코드로 접혀 상세 사유가 노출되지 않는다.
- P1-1 상태 경쟁: `hostRuntimeBridge.ts:123-128`은 모든 성공 응답에서 `onActivity(ok=true)`를 호출하고, `PreviewPanel.tsx:858-865`의 성공 분기는 `setStaleApp(false)`까지 수행한다. Manifest 변경 직전 승인된 병렬 요청이 410보다 늦게 완료되면 브리지 내부 `staleApp=true`는 유지되지만 화면의 stale 안내만 사라진다. 이후 데이터는 계속 차단되므로 보안 우회는 아니지만 사용자는 이유 없이 멈춘 화면을 보게 된다. **성공 응답은 `needsScope`·일반 note만 복구하고 `staleApp`은 해제하지 않는다.** stale 해제는 새 `release`로 브리지가 재생성되는 `useEffect([release])` 경로에서만 한다.
- P1-2 stale 증명 반복 전송: `hostRuntimeBridge.ts:294-300`은 stale를 세우지만 기존 `proof`를 비우지 않는다. 따라서 프레임이 데이터 호출을 계속하면 `fetchProof()` 차단 전에 같은 옛 증명을 서버로 반복 전송해 410·거부 감사를 최대 호출 예산만큼 쌓는다. stale 판정 순간 `proof=''`로 폐기하고, 이후 요청은 `fetchProof()`의 `staleApp` 차단에서 서버 호출 없이 실패하도록 한다.
- 시험 보정: 동시 요청 A=Manifest mismatch(410), B=이미 승인된 지연 성공 순서로 callback을 실행해 stale UI가 유지되는지 검증한다. stale 이후 두 번째 요청에서 proof 발급·데이터 API 호출 수가 증가하지 않는지도 실제 브리지 실행으로 검증한다. 소스 문자열 순서 검사만으로는 상태 경쟁과 호출 횟수를 증명하지 못한다.
- 다음 행동 / 담당 / 착수 조건: Claude Code가 위 프론트 2건과 실행형 회귀를 하나의 소규모 커밋으로 보정한다. 백엔드 재설계·자동 릴리스 reload는 하지 않는다. 해당 커밋에서 집중 회귀와 build가 통과하면 Codex 추가 승인 대기 없이 [4] 격리 카나리를 시작해도 된다. 카나리 결과에는 정상 만료 재발급과 Manifest stale 수동 재열기를 별도 시나리오로 기록한다.
- 영향·주의사항: “프레임을 버린다”는 현재 구현상 DOM 제거가 아니라 **Host 데이터 연결을 영구 격리하고 재열기를 요구한다**는 의미다. 보고·문서에서 실제 iframe DOM이 제거됐다고 표현하지 않는다. 자동 최신판 reload는 릴리스 조회·코드 교체·사용자 작업 유실 정책을 함께 설계해야 하므로 G1-B P1 UX로 별도 관리한다.
- 교대 체크포인트: 확인 커밋 = `f6346fc89`; 제품 코드 변경 0, 본 보드 기록만 추가. 집중 회귀 173 passed, 프론트 build 성공. 커밋/푸시 미실시. 재개 지점 = stale UI 경쟁·proof 폐기 보정 후 [4] 카나리. 금지 범위 = stale 상태를 일반 성공 callback으로 해제, stale proof 반복 전송, 카나리에서 자동 reload 설계 병행, 낡은 iframe에 새 proof 발급.

### [G1-B-APP-PROOF-REVIEW-84] 보정 커밋 재검토 — 4개 승인, Manifest 실행판 결속 보완 전 카나리 보류
- 작성자 / 기록 시각: Codex / 2026-08-14 KST
- 왜 지금 기록하는가: Claude Code가 `[G1-B-APP-PROOF-REVIEW-83]`의 5개 보정 요구를 `e83a20d98`에 반영하고 [4] 격리 카나리 착수 승인을 요청해, 보고가 아니라 실제 서버 판정·토큰 저장·프론트 재발급 흐름을 다시 교차검토했다.
- 상태: **부분 승인·[4] 착수 보류** — 프로그램 비활성화 차단, PDP 허용 뒤 성공 사용 감사, 요청별 최대 1회 재발급, 조직 범위 미선택 안내·재시도 UI는 승인한다. Manifest 지문·판을 토큰과 PDP에 추가한 구조도 승인하지만, 현재 프론트가 Manifest 불일치를 일반 만료와 동일하게 자동 복구하여 **기존 iframe 실행판을 살려 두므로 실행판 결속의 목적은 아직 달성되지 않았다.**
- 승인 근거: `core/app_proof.py:104-119`가 lifecycle 조회 실패와 `disabled`를 사용 불가로 전달하고, `api/routes/app_data_runtime.py`는 PDP 허용 뒤에만 `record_use()`를 호출한다. `frontend/src/lib/hostRuntimeBridge.ts:270-277`의 재발급 예산은 Preview 수명 전역이 아니라 요청 단위 최대 1회이며, `PreviewPanel.tsx:854-858`은 409 안내와 범위 선택 필요 상태를 부모 UI로 전달한다. 집중 회귀 `tests/test_app_data_runtime.py`, `tests/test_host_runtime_bridge_contract.py`, `tests/test_app_capability_token.py`, `tests/test_app_policy.py` **165 passed**.
- P0 잔여 1 — Manifest 불일치 자동 우회: `core/host_runtime_sdk.py:139-142`가 `TOKEN_MANIFEST_MISMATCH`와 진짜 시간 만료를 모두 `EXPIRED`로 접고, `hostRuntimeBridge.ts:273-277`은 모든 `EXPIRED`에서 증명만 재발급한 뒤 같은 iframe 요청을 다시 실행한다. Manifest 내용이 바뀌었는데 capability가 같으면 오래된 실행 코드가 새 Manifest 증명을 받아 계속 동작한다. **시간 만료는 증명 재발급**, **Manifest 불일치는 현재 릴리스·코드를 다시 해석해 iframe 세대를 재생성하는 `RELOAD_REQUIRED` 계열**로 분리하고, 단순 proof refresh로 복구하지 않는다. 전체 프레임 재생성 실패 시 해당 요청은 차단한다.
- P0 잔여 2 — 빈 Manifest 식별자 발급 차단: `core/app_capability_token.py:95-100,173-178`은 빈 `manifest_fingerprint`·`manifest_version`도 발급한다. PDP는 빈 fingerprint를 거부하지만 빈 version 양쪽은 같다고 통과하며, 빈 fingerprint 토큰은 발급만 성공하고 첫 호출마다 실패한다. 발급 단계에서 fingerprint와 version을 모두 필수화하고, PDP도 token/current 양쪽의 fingerprint·version 공란을 fail-closed로 거부한다. 구형 릴리스는 Manifest를 보정한 뒤에만 Runtime을 연다.
- P1 잔여: `PreviewPanel.tsx:855`는 성공 시 `needsScope`만 내리고 이전 409 문구인 `bridgeNote`를 지우지 않아, 범위 선택 후 정상 재시도해도 상단의 「실데이터 연결 제한」 문구가 남을 수 있다. 성공 callback에서 stale note를 성공/준비 상태로 치환한다. 또한 `program_usable()`은 `status != disabled`라 미지의 상태 문자열을 허용하므로 `active/deprecated` 명시적 허용 목록으로 바꾸고 deprecated 경고 표시는 운영 이관 UX 항목으로 남긴다.
- 다음 행동 / 담당 / 착수 조건: Claude Code가 (1) Manifest mismatch를 proof-only 자동 재발급에서 분리해 iframe generation reload를 강제하고 종단 회귀를 추가, (2) 빈 fingerprint/version 발급·판정을 fail-closed로 고정, (3) 범위 재시도 성공 후 stale 안내 제거, (4) lifecycle 명시적 허용 목록을 적용한다. Codex는 보정 커밋의 프론트 종단 흐름만 짧게 재검토하고 통과 시 [4] 격리 카나리를 승인한다.
- 영향·주의사항: 현재 165개 집중 회귀가 초록인 사실은 위 의미적 우회를 부정하지 않는다. 기존 테스트는 서버의 Manifest 불일치 거부와 새 증명 복구를 각각 검증하지만, **오래된 iframe이 새 증명을 받아 계속 실행되는 전체 흐름**을 실패 조건으로 두지 않았다. 카나리에서 이 결함을 즉석 수정하거나 정상 만료와 Manifest 교체를 같은 표본으로 집계하지 않는다.
- 교대 체크포인트: 확인 커밋 = `e83a20d98`; 제품 코드 변경 0, 본 보드 기록만 추가. 집중 회귀 165 passed. 커밋/푸시 미실시. 재개 지점 = Manifest mismatch 종단 보정 커밋 재검토. 금지 범위 = Manifest 변경 후 proof-only 자동 회복, 빈 Manifest 식별자 토큰 발급, stale 범위 안내를 정상 연결 상태로 오인, 카나리 안에서 설계·수정 병행.

### [G1-B-APP-PROOF-REVIEW-83] Host App Proof 종단 배선 교차검토 — 구조 승인, 카나리 착수 보류
- 작성자 / 기록 시각: Codex / 2026-08-14 15:58 KST
- 왜 지금 기록하는가: Claude Code가 `e149c28b0`에서 [3.5] 앱 증명 발급·Runtime 전용 라우트·PDP·로그아웃 회수를 종단 배선하고 [4] 격리 카나리 승인을 요청했다. 카나리는 구현 장소가 아니므로 `[G1-B-I3-REVIEW-82]`의 필수 계약 8개가 실제 코드에서 완결됐는지 교차검토했다.
- 상태: **조건부 승인·[4] 착수 보류** — 전용 Runtime 경로, 증명 필수, 서버 파생 capability, 부모 메모리 보관, iframe 비노출, 사용자·세션·앱·릴리스·문맥·조직·현재 선언 capability 재검사, 로그아웃 회수는 승인한다. 다만 아래 P0 2건과 카나리 증거 신뢰를 해치는 P1 3건을 보정하기 전에는 [4]를 시작하지 않는다.
- 판정 근거: (P0-1) `core/app_proof.py:104-128`과 `api/routes/app_data_runtime.py:132-147`은 프로그램 사용 상태를 읽지 않아 `program_lifecycle=disabled`인 릴리스도 `ResourceScope.status=active`로 판정되어 Runtime 데이터 접근이 계속된다. (P0-2) `manifest_fingerprint`·`manifest_version`은 `AppResourceFacts`와 발급 응답에만 있고 `core/app_capability_token.py:161-178`의 증명 레코드 및 `core/app_policy.py:274-322`의 전수 대조에는 없어, 같은 동작 capability를 유지한 Manifest 교체는 기존 증명을 무효화하지 않는다. 이는 REVIEW-82의 “현재 Manifest 전수 대조” 미충족이다.
- 추가 보정: (P1-1) `app_capability_tokens.resolve()`가 PDP 판정 전에 `APP_TOKEN_USED/outcome=allowed`를 기록해 실제 거부 요청도 성공 사용처럼 감사에 남는다. 성공 사용 감사는 PDP 허용 뒤로 옮기고, 선행 기록이 필요하면 `PRESENTED/attempted`로 분리한다. (P1-2) 브리지의 전역 `reissued`는 한 Preview 수명 전체에서 단 한 번만 재발급하므로 두 번째 정상 만료부터 영구 실패한다. **요청 1건당 최대 1회**로 제한해야 한다. (P1-3) 범위 미선택 409의 안내는 `onActivity`에만 전달되지만 `PreviewPanel.tsx:842-850`이 콜백을 연결하지 않아 화면에는 고정 `UNAVAILABLE`만 보인다. 부모 화면에 조직 범위 선택 안내와 재시도 동작을 연결한다.
- 검증: 관련 집중 회귀 `tests/test_app_data_runtime.py`, `tests/test_host_runtime_bridge_contract.py`, `tests/test_app_capability_token.py`, `tests/test_app_policy.py` **152 passed**. 기존 회귀가 초록이라는 사실과 위 누락은 양립한다. 현재 시험에는 비활성 릴리스, Manifest 지문/판 변경, 두 번의 연속 만료, 409 부모 안내, 거부 요청의 감사 outcome 검사가 없다.
- 다음 행동 / 담당 / 착수 조건: Claude Code가 별도 보정 커밋으로 (1) disabled 릴리스 발급·사용 모두 차단하고 deprecated는 정책대로 경고/허용, (2) 증명에 Manifest fingerprint+version을 결속해 요청마다 현재값과 상수시간 의미 비교, 빈 fingerprint/version 발급 차단, (3) 감사 시점을 PDP 뒤로 이동, (4) 재발급 예산을 요청 단위로 변경, (5) 409 안내를 부모 UI에 연결하고 각 회귀를 추가한다. Codex가 보정 커밋을 짧게 재검토한 뒤에만 [4]를 승인한다.
- 영향·주의사항: 기존 관리/콘솔 AppData API에는 앱 증명 폴백을 추가하지 않는다. 카나리는 계속 전용 임시 DB·`AFS_TEST_SANDBOX`·별도 포트·단일 워커만 사용하며 운영 DB를 건드리지 않는다. 다중 워커 공유 증명 저장소와 조직 문맥 변경 시 자동 증명 폐기·재발급은 운영 전환 전 별도 P1로 남긴다.
- 교대 체크포인트: 확인 커밋 = `e149c28b0`; 제품 코드 변경 0, 본 보드 기록만 추가. 집중 회귀 152 passed. 커밋/푸시 미실시. 재개 지점 = Claude Code 보정 커밋 교차검토. 금지 범위 = 결함을 카나리 안에서 즉석 수정, 비활성 프로그램의 Runtime 허용, Manifest 지문 미결속 상태의 게이트 표본 채택, 거부 요청을 성공 사용으로 집계.

### [G1-B-I3-REVIEW-82] I-3 브리지·데이터셋 귀속 라우트 교차검토 — 전송계층 승인, 앱 증명 배선 선행
- 작성자 / 기록 시각: Codex / 2026-08-14 13:51 KST
- 왜 지금 기록하는가: Claude Code가 `951c479e2`(데이터셋 귀속 레코드 라우트)와 `559135b60`(iframe Host Runtime 브리지)을 완료하고 격리 카나리 착수를 제안했으나, 보고 말미에 `TOKEN_*` 표본을 만들 앱 증명 발급·PDP 연결이 아직 없다고 명시했다. 카나리에 미구현 통제를 섞으면 카나리가 구현 단계가 되어 판정 증거가 불명확해지므로 선행 경계를 확정한다.
- 상태: **조건부 승인** — `951c479e2`의 혼동된 대리인 차단과 404 은폐, `559135b60`의 기본 비활성·source/SID 검증·응답 투영·요청 상한·계약 대조는 승인한다. 다만 I-3는 현재 **브리지 전송계층 완료**이며 Host Runtime 종단 보안 경로 완료로 산정하지 않는다.
- 실측 근거: `frontend/src/lib/hostRuntimeBridge.ts:128-151`은 `apiFetch`로 일반 `/api/v1/appdata` 경로를 호출하며 앱 증명을 붙이지 않는다. `api/routes/app_data_control.py:139-143`은 Shadow 주체를 `via="session"`으로 고정하고 `session_id/token`을 전달하지 않는다. `core/app_capability_token.py`의 `app_capability_tokens`는 제품 API 호출자가 0건이고 시험에서만 `issue()`된다. 또한 현재 Shadow의 `AppResourceFacts`는 `release_id/app_class`만 채워 `app_id`, Manifest fingerprint/version, declared capabilities가 비어 있으므로 단순히 `via="app_token"`만 바꾸면 정상 요청도 `TOKEN_APP_MISMATCH` 또는 `MANIFEST_CAPABILITY`로 거부된다.
- 결정: 승인 순서에 `[3.5] Host App Proof 배선`을 삽입한다. **[3.5]가 완료·커밋되기 전 [4] 격리 카나리를 시작하지 않는다.** 카나리는 구현 장소가 아니라 완성된 경로의 종단 검증 장소다.
- [3.5] 필수 계약: (1) 세션 인증된 부모만 호출하는 앱 증명 발급 API, (2) 서버가 release·app·Manifest·현재 tenant/entity/scope·사용자·세션 해시를 직접 해석하고 capability를 서버에서 산출, (3) 전문은 부모 메모리에만 보관하고 iframe/localStorage/로그에 미전달, (4) Host Runtime 전용 API는 증명 누락 시 session 경로로 폴백하지 않는 Fail-closed, (5) 요청마다 현재 사용자 권한과 토큰·앱·릴리스·문맥·조직·Manifest를 전수 대조, (6) 로그아웃 시 해당 세션의 앱 증명 회수, (7) 만료 시 부모가 한 번만 재발급·재시도하고 반복 루프 금지, (8) 정확한 거부 사유는 서버 감사에만 남기고 iframe에는 SDK 고정 오류로 접기.
- 라우트 경계 권고: 일반 관리/콘솔 AppData API에 `proof 없음 → session 판정` 폴백을 추가하지 않는다. 브리지는 `/api/v1/appdata/runtime/*` 같은 전용 경로만 사용하고 이 경로는 앱 증명을 필수로 한다. 기존 관리 API는 사람의 관리 표면으로 유지한다. 같은 라우트에서 증명 누락을 session으로 내리면 Host Runtime capability를 헤더 하나 생략해 우회할 수 있다.
- 카나리 판정: [3.5] 이후 전용 임시 AppData DB·테스트 Release/Ownership/Manifest·`AFS_TEST_SANDBOX`·별도 포트·`AFS_SHADOW_RUN`으로 6개 작업의 허용/거부와 6개 부정 시나리오를 실제 Preview에서 수행한다. 운영 DB 쓰기 금지. 카나리 결과는 HTTP 결과와 `policy_shadow.switch_gate(run_id)`를 함께 대조하고, 표본 없음은 통과로 세지 않는다.
- 영향·주의사항: 프로세스 메모리 토큰 저장소는 단일 워커 카나리에는 사용할 수 있으나 다중 워커에서는 안전한 방향으로 간헐 거부된다. 운영 전환 시 단일 워커 제약을 명시하거나 세션/토큰 공유 저장소를 준비해야 한다. G2 구현 착수 조건은 B01~B05 계약의 종단 배선과 커밋이며, 브리지 UI만으로 충족되지 않는다.
- 다음 행동 / 담당 / 착수 조건: Claude Code가 [3.5] 설계·구현·회귀를 별도 커밋으로 완료하고, 발급 API 입력/서버 파생값/전용 Runtime route/로그아웃 회수/오류 접힘 표를 보고한다. Codex는 해당 커밋에서 iframe 자격증명 비노출·optional downgrade 부재·현재 Manifest 재검증을 교차검토한다. 그 승인 뒤 Claude Code가 [4] 격리 카나리를 실행한다.
- 교대 체크포인트: 확인 커밋 = `951c479e2`, `559135b60`. 변경 범위 = 본 보드 검토 기록만, 제품 코드 0. 테스트 재실행 = 하지 않음(Claude 보고 3,144 passed·1 skipped와 코드 정적 대조). 커밋/푸시 = 본 기록 미실시. 재개 지점 = [3.5] 커밋 교차검토. 금지 범위 = 카나리에서 설계·구현 동시 수행, 앱 증명 전문 iframe 전달, capability를 클라이언트 입력으로 발급, proof 누락 시 session 폴백, 운영 AppData DB 탐침.

### [PRODUCT-FEATURE-GUIDE-V21-81] 경영진 평가 반영 · 증거/상태/90일 파일럿/대안 역할 보강
- 작성자 / 기록 시각: Codex / 2026-08-14 10:42 KST
- 왜 지금 기록하는가: 두 차례 외부 평가에서 기능 서사는 강하지만 예산·도입 판단에 필요한 구현 상태 구분, 검증 증거, ROI 측정 계약, 기존 시스템과의 역할 경계가 부족하고 7면 대비·3면 줄바꿈 결함이 있다는 지적이 확인됐다.
- 상태: **v2.1 26면 개정 완료 · HTML/PDF 전면 재검증 완료 · 커밋/푸시 미실시**.
- 결정 및 근거: `docs/product-feature-guide-v2/index.html`과 `styles.css`를 개정해 `CURRENT PRODUCT`·`APPROVED TARGET`·`DESIGNED / IMPLEMENTATION PENDING` 상태를 분리했다. G2 온톨로지는 구현 대기, 첫 수직 폐루프는 검증 시나리오임을 명시했다. 실제 화면 14개에는 `검증용 예시 데이터`, 통제실 승인 화면에는 `승인 시안 · 시나리오 샘플` 표기를 추가했다. 7면 조직 계층 본문 대비와 3면 온톨로지 핵심 문구 줄바꿈을 수정했다.
- 가치·도입 보강: 22면을 성과 주장 대신 `환율 +10%·도입 +15일·전기료 +12%` 복합 충격의 검증 계약으로 바꾸고, 산출·결정·책임·기한·Shadow Pilot 비교 조건을 명시했다. 23면에는 90일 파일럿 단계, 기준선 KPI, 비용 항목, 확산/중단 Gate를 신설했다. 24면에는 ERP·MES·WMS·EPM·BI·Low-code·Data Platform·전문 Twin·RPA/범용 Agent와의 유지/보완/선택적 대체/비대상 경계를 추가했다.
- 사실성 원칙: 실측되지 않은 절감액·ROI·고객 효과를 만들지 않았다. 기존 시스템은 일괄 교체나 무조건 무개발 연계를 주장하지 않고 `Snapshot/읽기 전용 계약 → 원천별 API·MCP·어댑터 단계 확대`로 표현했다. 외부 파트너 포털은 특정 고객 시스템명을 제품 기본 계약으로 고정하지 않았다.
- 검증: 브라우저 실측 = 26면, 이미지 16개 정상, 가로 오버플로 0, 검증/승인 표기 14개, 상태 배지 13개. PDF `output/pdf/AI_Factory_Studio_Product_Feature_Guide_v2_2026-08-14.pdf`는 Chromium 접근성 태그·개요 옵션으로 생성했고 Poppler 실측 = 26면, 1080×675.12pt, `Tagged: yes`. 26면 전면 연락시트와 3·7·22·23·24면 원본 렌더를 육안 검수해 잘림·빈 면·이미지 누락 0건을 확인했다.
- 영향·주의사항: 본 자료는 상세 제품·기능소개서이며 구매 의사결정용 최종 제안서가 아니다. 실제 파일럿 기준선·실측 ROI·총소유비용은 데이터 오너와 적용 범위가 확정된 뒤 채워야 한다. G2 온톨로지, 완전한 결정 폐루프, Shadow Pilot을 현재 운영 완료로 재표현하지 않는다.
- 다음 행동 / 담당 / 착수 조건: Supervisor가 개정본 메시지와 90일 파일럿 Gate를 검토한다. 승인 후 Codex가 같은 증거 체계를 사용한 경영진용 8~10면 압축판과 Executive FAQ를 별도 제작한다. 실측 파일럿 결과가 확보되면 22~23면의 측정 계약을 실제 Before/After 증거로 교체한다.
- 교대 체크포인트: 마지막 확인 = HTML 26면·이미지·상태 배지·대비 검증, 태그 포함 PDF 생성, 26면 연락시트 및 핵심 5면 원본 검수. 변경 범위 = `docs/product-feature-guide-v2/`, 2026-08-14 v2 PDF, 본 보드 항목. 미변경 = 기존 v1/v2 2026-08-13 PDF·제품 코드·G1/G2 코드/설계 파일. 커밋/푸시 = 미실시. 재개 지점 = 사용자 콘텐츠 검토 또는 경영진 압축판 파생. 금지 범위 = 측정 전 ROI 주장, 목표 상태를 현재 기능으로 오표기, 특정 고객 레거시명을 범용 제품 계약으로 고정, 다른 세션 작업 파일 스테이징.

### [PRODUCT-FEATURE-GUIDE-V2-80] 온톨로지 중심 제품 서사·행위/결과 중심 화면으로 기능소개서 전면 개정
- 작성자 / 기록 시각: Codex / 2026-08-13 23:16 KST
- 왜 지금 기록하는가: 사용자가 1차 기능소개서에서 메타 설명이 전면에 나오고, 프로젝트 통제실은 구형 UI이며, 에이전트 연결·시뮬레이션 결과·회의 요청·보고서 본문·브리핑 내용이 충분히 보이지 않고, 제품의 핵심인 `현업 생성 → 중앙 데이터 환류 → 온톨로지 → 경영 질문 답변` 서사가 빠졌다고 판정했다.
- 상태: **v2 24면 개정 완료 · 기존 버전 보존 · HTML/PDF 시각 검증 완료 · 커밋/푸시 미실시**.
- 결정 및 근거: `docs/product-feature-guide-v2/`를 별도 생성해 기존 `docs/product-feature-guide/`를 보존했다. 2면은 현업 생성부터 경영 답변까지의 제품 성장 흐름, 3면은 Legacy MCP·MDM/카탈로그·외부지표·지식 허브·생성 앱 데이터를 연결하는 기업 경영 온톨로지, 4면은 경영자 질문을 근거·관계·계산·결정·실행으로 전환하는 과정을 설명한다. 온톨로지는 관계·영향 경로, G4 계산 그래프는 수치, LLM은 해석·설명이라는 역할 경계를 명시했다.
- 화면 조치: 프로젝트 통제실은 사용자 승인 시안 `uiux-prototypes/sw-factory-concepts/transparent-orchestration/`을 `APPROVED PRODUCT UI`로 표시했다. 에이전트 통제소는 실제 에이전트 구성과 React Flow 연결 그래프 2화면, 경영계획은 공격 시나리오의 영업이익·순이익·기준선 대비·계획/실적 차이, 의사결정 센터는 회의 제목·일정·채널 입력 상태, 발간은 실제 보고서 본문, 전사 브리핑은 데이터 계약 위반 등 우선 조치 내용을 수록했다. 기존 지식 허브 캡처가 빈 검은 화면인 결함을 PDF 전수 렌더링에서 발견해 실제 4팩·27문서 화면으로 교체했다.
- 근거: `docs/product-feature-guide-v2/index.html`, `styles.css`, `README.md`, `assets/screens/*.png`; PDF `output/pdf/AI_Factory_Studio_Product_Feature_Guide_v2_2026-08-13.pdf`.
- 검증: 브라우저 DOM에서 24면의 한글·이미지·기능 계약을 확인했다. Poppler 실측 = 24면, 1080×675.12pt, 약 3.11MB. 전면을 PNG 연락시트로 확인하고 핵심 14·17·18·19·20·21면을 원본 크기로 재검수했다. 잘림·면 분할·누락 0건이며, 지식 허브 빈 화면 1건을 검출·수정·재렌더링했다.
- 영향·주의사항: 통제실 1면만 승인된 목표 UI이며 현재 제품 코드의 구형 통제실과 혼동하지 않도록 별도 라벨을 사용했다. G2 온톨로지 런타임은 상세 설계 완료·구현 대기 상태이므로 개념 페이지는 제품 운영 원리와 목표 계약을 설명하며 현재 운영 완료를 주장하지 않는다. 화면의 조직 미지정·tenant_default·카나리 안건·수치는 개발 검증 데이터다.
- 다음 행동 / 담당 / 착수 조건: Supervisor가 v2 메시지·화면 선택을 검토한다. 승인 후 Codex가 필요 시 경영진용 8~10면 압축판과 외부 제안용 산업별 변형을 파생한다. Claude Code는 승인 통제실 UI를 실제 제품에 이관할 때 본 문서의 `APPROVED PRODUCT UI` 화면과 기존 UI 설계서를 기준으로 삼는다.
- 교대 체크포인트: 마지막 확인 = HTML 브라우저 검증, PDF 24면 생성, 전면 연락시트 및 핵심 6면 원본 검수 완료. 변경 범위 = `docs/product-feature-guide-v2/`, v2 PDF, 본 보드 기록. 미변경 = 기존 기능소개서·기존 브로셔·제품 코드·G1/G2 작업 파일. 커밋/푸시 = 미실시. 재개 지점 = 사용자 문구·화면 리뷰 반영. 금지 범위 = 기존 버전 덮어쓰기, 승인 시안을 현재 구현 화면으로 오표기, G2 온톨로지를 운영 완료로 표현, 다른 세션 보안·온톨로지 코드 수정.

### [PRODUCT-FEATURE-GUIDE-79] 기존 브로셔 보존 · 제품 특성/실제 화면 분리 기능소개서 신설
- 작성자 / 기록 시각: Codex / 2026-08-13 22:18 KST
- 왜 지금 기록하는가: 사용자가 승인한 12면 제품 브로셔는 그대로 보존하면서, 제품 특성 설명과 실제 프로그램 화면을 분리하고 화면부를 단순 증거가 아닌 기능소개서 수준으로 확장한 별도 버전을 요청했다.
- 상태: **23면 신규 버전 제작 완료 · HTML/PDF 시각 검증 완료 · 커밋/푸시 미실시**.
- 결정 및 근거: `docs/product-feature-guide/`를 별도 정본 후보로 신설했다. 1~7면은 제품 정의·7대 기능·폐루프·회사구조/신뢰·기능 지도에 한정하고, 9~21면은 현재 구현 서버에서 직접 캡처한 13개 기능을 `화면 목적·핵심 기능·출력·통제·업무 흐름` 계약으로 설명한다. 경영 홈, 업무·데이터 설계 상담, Software Factory, 프로젝트 통제실, 지식 허브, 마스터 데이터, 데이터 거버넌스, 조직·권한, 에이전트 통제소, 경영계획·실적, 의사결정 센터, 대내외 발간, 전사 브리핑을 포함했다. 제품 방향과 현재 구현을 상태 표기로 분리하고 검증용 수치를 실제 성과로 주장하지 않았다.
- 근거: `docs/product-feature-guide/index.html`, `styles.css`, `guide.js`, `README.md`, `assets/screens/*.png`; PDF `output/pdf/AI_Factory_Studio_Product_Feature_Guide_2026-08-13.pdf`.
- 검증: 브라우저 화면 너비 자동 맞춤과 대표 기능면 육안 검수. PDF 실측 23면·1080×675.12pt·텍스트 9,544자·파일 약 3.17MB. 23면 전부 96dpi PNG로 재렌더링하고 연락시트 및 9·20·22면 원본 크기를 육안 확인해 누락·페이지 분할·잘림 0건을 확인했다. 참조 이미지 17개 경로 존재·비어 있지 않음을 전수 확인했다.
- 영향·주의사항: 기존 `docs/product-brochure/`와 기존 12면 PDF는 수정·삭제하지 않았다. 화면의 `tenant_default`, 예시 안건·비용·사용자·데이터 수치는 검증 데이터다. `Jarvis`는 현재 화면 표시명을 그대로 보여주며 최종 명칭 확정 시 제품 UI와 문서를 함께 수정해야 한다. 본 문서는 제품 완료 선언이 아니며 22면에 Host Runtime·CERTIFIED 기준선·온톨로지·G4 계산 그래프·Shadow Pilot 후속 증명을 명시했다.
- 다음 행동 / 담당 / 착수 조건: Supervisor가 HTML의 메시지 밀도와 기능 페이지 설명 수준을 검토한다. 승인 후 Codex가 필요 시 고객 제안형 산업 변형판과 경영진용 압축판을 파생한다. 실제 파일럿 지표가 확보되면 검증 데이터 대신 효과·증거 페이지를 갱신한다.
- 교대 체크포인트: 마지막 확인 = 새 HTML·CSS·JS·README·화면 14종과 PDF 생성 완료. 변경 범위 = `docs/product-feature-guide/`, 새 PDF, 본 보드 기록. 미변경 = 기존 브로셔와 제품 코드. 커밋/푸시 = 미실시. 재개 지점 = 사용자 시각·메시지 검토 반영. 금지 범위 = 기존 브로셔 덮어쓰기, 검증 수치를 고객 성과로 표현, 목표 기능을 현재 운영 완료로 표시, 다른 세션의 G1/G2 파일 수정.

### [PRODUCT-BROCHURE-76] 대내외 설명용 제품 브로셔 1차 완성
- 작성자 / 기록 시각: Codex / 2026-08-13 11:32 KST
- 왜 지금 기록하는가: 제품 기능 구현과 병행해 경영진·현업·투자자에게 AI Factory Studio의 제품 범주, 폐루프 가치, 첫 증명 시나리오와 도입 방식을 짧고 설득력 있게 설명할 대외 산출물이 필요해 8면 브로셔를 제작했다.
- 상태: **1차 완성 · 사용자 검토 대기** — 화면·인쇄 겸용 HTML이며 제품 비전 브로셔로 표시했다.
- 결정 및 근거: `docs/AI_FACTORY_STUDIO_PRODUCT_BIBLE.md`, 독자 제품 전략, 승인된 Living Enterprise Canvas, LS CI 적용 가이드를 대조했다. 메시지는 `실제 데이터 → 현업 앱 → 제조 경영 의미지도 → 결정론적 Twin → 의사결정·실행 → 효과 측정` 폐루프로 고정하고, 원료 도입 지연→재고→생산→매출→현금흐름의 첫 수직 시나리오를 상호작용으로 구현했다. 특정 고객 효과 수치나 구현 완료율은 임의 주장하지 않았다.
- 영향·주의사항: 브로셔 속 회사와 수치는 가상 샘플이며 LS 색상은 기업 테마 예시다. 정식 대외 배포 전 회사 CI 사용 승인, 법무·보안·제품 상태 문구 검토, 실제 파일럿 성과 수치 대체가 필요하다. 전역 AI 동료의 최종 이름이 확정되지 않아 고유명사를 사용하지 않았다.
- 다음 행동 / 담당 / 착수 조건: Supervisor가 메시지·대상 독자·배포 목적을 검토한 뒤, Codex가 경영진용 4면 압축판과 고객 제안용 산업별 변형을 후속 제작한다. 실제 파일럿 지표가 확보되면 효과·고객 증거 페이지를 추가한다.
- 교대 체크포인트: 마지막 확인 = 1440×900 주요 8면 시각 확인, 390×844 모바일 가로 본문 넘침 0, 고아 줄바꿈 0, 시나리오 20일/+15% 입력 시 현금 영향 갱신, 콘솔 오류 0. 변경 범위 = `docs/product-brochure/` 4파일과 본 보드 기록. 커밋/푸시 = 미실시. 재개 지점 = 사용자 브로셔 리뷰 반영. 금지 범위 = 합성 수치를 실제 성과로 표현, 완성되지 않은 기능을 현재 운영 완료로 표시, 고객사 CI 무승인 외부 배포.

### [PRODUCT-BROCHURE-77] 사용자 1차 승인 반영 · 실제 제품 화면 4면 추가
- 작성자 / 기록 시각: Codex / 2026-08-13 14:15 KST
- 왜 지금 기록하는가: 사용자가 브로셔 전체 구성은 승인했으나 개념도 중심이라 실제 프로그램의 설득력이 부족하다고 판단해, 현재 구현 서버의 실제 데이터 화면을 제품 경험 증거로 추가하도록 요청했다.
- 상태: **12면 확장 완료 · 시각 재검수 완료 · 커밋/푸시 미실시**.
- 결정 및 근거: 현재 구현 상태와 목표 상태를 섞지 않도록 신규 5~8면 모두 `현재 구현 화면`과 `LIVE PRODUCT`로 표시했다. 실제 로그인 후 캡처한 경영 홈, Software Factory 새 업무 만들기, 지식 허브, 에이전트 통제소를 사용했다. 화면마다 기능 나열 대신 사용자의 행동, 플랫폼 통제, 경영 가치를 함께 설명한다.
- 영향·주의사항: 캡처에는 개발 환경의 `tenant_default`, 예시 프로젝트, 지식팩 4개·문서 27건, 에이전트 15개·사용자 확인 지점 6개가 보인다. 이는 고객 운영 실적이 아니라 현재 데모 데이터다. AI 비서 명칭은 실제 화면에 남아 있는 `Jarvis`를 그대로 캡처했으며 최종 명칭 확정 후 화면과 브로셔를 함께 갱신해야 한다.
- 검증: 12개 면 번호 일치, 실제 화면 이미지 4개 정상 로드, 전체 12면의 `scrollHeight=page height`, 가로 넘침 0. 1265/1280×720 실화면을 16:9로 유지했다. 네 면을 브라우저로 육안 재검수했고 에이전트 면 제목의 한 글자 고아 줄바꿈을 발견해 해당 면 제목 크기를 보정했다.
- 다음 행동 / 담당 / 착수 조건: 사용자 최종 시각 확인 후 Codex가 필요 시 경영진용 4~6면 압축판을 별도로 만든다. 현재 12면 원본은 제품 비전+실제 화면 통합 브로셔 정본으로 유지한다. 변경 범위 = `docs/product-brochure/index.html`, `styles.css`, `README.md`, `assets/actual/*.png`, 본 보드 기록.

### [PRODUCT-BROCHURE-78] PDF 인쇄 레이아웃 붕괴 근본 수정
- 작성자 / 기록 시각: Codex / 2026-08-13 20:02 KST
- 왜 지금 기록하는가: 사용자가 브라우저 화면은 정상이나 Edge의 PDF 저장 결과에서 디자인이 깨진다고 실제 PDF를 제공했다. 원본 PDF 12면을 PNG로 전수 렌더링해 재현했다.
- 원인: 화면 기준 캔버스는 1440×900인데 인쇄 CSS가 A4 297×210mm 안에서 고정 좌표 도식과 카드 그리드를 다시 배치했다. 그 결과 1면 제품 화면, 3면 폐루프, 4면 제품 3영역, 5~8면 실제 화면 설명, 9~11면 결과·원칙·도입 단계가 잘렸다.
- 조치: 인쇄 시 개별 요소를 A4에 재배치하지 않고 1440×900 캔버스를 15×9.375in(16:10) PDF 한 면으로 그대로 사용하도록 `@page`와 `.page` 계약을 교체했다. 화면 CSS와 PDF CSS가 같은 배치 모델을 사용하고, 인쇄에서는 내비게이션·그림자만 제거한다.
- 검증: 수정 PDF `output/pdf/AI_Factory_Studio_Product_Brochure_2026-08-13.pdf` 생성. Poppler 실측 = 12면, 페이지 1080×675.12pt, 파일 1,599,072 bytes. 12면 전부 PNG 재렌더링 후 연락처 시트와 대표 면을 육안 확인했으며 잘림·면 분할·누락 0건.
- 다음 행동: 브라우저에서 다시 저장할 때 인쇄 대화상자의 페이지 크기를 CSS 제안 사용자 정의 크기로 유지하고 배경 그래픽을 켠다. A4 강제본이 별도로 필요하면 현재 16:10 원본을 페이지 전체 이미지로 레터박스 처리하는 별도 산출물을 만든다.

### [G1-B-P0-REVIEW-75] Host Runtime B03·B04 구현 교차검토
- 작성자 / 기록 시각: Codex / 2026-08-13 11:04 KST
- 왜 지금 기록하는가: Claude Code가 커밋 `2e8a8f099`에서 B04 정책 결정점과 B03 단기 접근 증명을 구현하고 App Data Plane 라우트 배선을 예고했으므로, 배선 전에 권한 교집합과 G2 `ResourceScope` 재사용 계약이 실제 코드에서 성립하는지 교차검토한다.
- 상태: **조건부 승인 · 라우트 배선 전 보정 필수** — `sandbox_token`과 별도 저장소를 둔 판단, 판정/강제 분리, 짧은 만료, 개인 앱 소유자 보호, 변이 검사는 승인한다. 그러나 현재 B03·B04는 골격 구현이며 G1-B P0 완료로 산정하지 않는다.
- 결정 및 근거:
  - `core/app_policy.py:129-144`는 tenant와 entity mode만 비교하고 `Subject.ctx.scope_node_id`, `Resource.scope_node_id`, 토큰의 tenant/entity/scope를 판정하지 않는다. 필드가 존재하지만 권한에 쓰이지 않는다.
  - `core/app_policy.py:198-212`는 토큰의 `release_id`와 capability만 검사한다. 토큰 `actor`와 현재 `Subject.user_id`, 토큰 `app_id`와 자원 앱, 토큰 문맥과 현재/자원 문맥을 대조하지 않아 다른 로그인 세션·앱·회사 문맥으로 재사용될 수 있다.
  - `core/app_policy.py:165-169`는 `owner_dept_id`가 비면 식별 사용자에게 허용한다. 신규 Host Runtime 자원에서는 D-014와 충돌하므로 미바인딩은 Fail-closed여야 하며, 레거시 호환은 명시적 migration/legacy 상태로만 분리한다.
  - `core/app_policy.py:214-224`는 앱 토큰 경로의 READ를 Manifest 미선언이어도 허용한다. 신규 Host Runtime 호출은 읽기 포함 모든 capability를 선언해야 하며, 기존 앱 호환은 `manifest_version/legacy_mode`로 명시해야 한다.
  - `core/app_capability_token.py:82-157`은 `scope_node_id` 공란을 허용하고 사용자 세션 식별자를 저장하지 않는다. `resolve()`는 성공 사용 감사를 남기지 않으며 전문을 해시가 아닌 메모리 key/record로 보관한다. B03 완료 정의의 user/scope binding·재사용 통제·사용 추적을 충족하지 못한다.
  - 현 `Resource`는 `app_class/release_id/declared_capabilities`가 섞인 앱 전용 구조라 G2의 범용 `ResourceScope`가 아니다. 공통 범위 계약과 앱별 정책 사실을 분리해야 G2가 같은 PDP를 재사용할 수 있다.
- 영향·주의사항: 현재 `api/routes/app_data_control.py`는 아직 기존 `assert_release_*`만 사용하므로 즉시 노출된 신규 우회는 없다. 이 상태에서 새 PDP로 교체하면 기존보다 권한이 넓어질 수 있으므로 기존 검사를 제거하지 말고, 새 PDP의 동등성·강화가 종단 부정 테스트로 입증된 뒤 원자적으로 전환한다. 다중 워커에서 메모리 토큰이 무작위 거부되는 한계는 단일 워커 P0에만 허용하고 운영 전 공유 세션 저장소로 이관한다.
- 다음 행동 / 담당 / 착수 조건: Claude Code는 (1) 범용 `ResourceScope`와 앱 전용 `AppResourceFacts` 분리, (2) token user/session/app/release/tenant/entity/scope/capability 전수 교차검증, (3) 미바인딩·미선언 READ Fail-closed, (4) 토큰 해시 저장·성공 사용 감사·회수/만료/세션 종료 테스트, (5) 기존 `assert_release_*`와 결과 동등성 표를 먼저 완료한다. 그 뒤 App Data Plane 한 경로에 이중 판정 shadow mode를 연결해 불일치 0을 확인하고 전체 라우트를 전환한다. Codex는 이 계약으로 G2 상세 설계의 placeholder를 치환한다.
- 교대 체크포인트: 마지막 확인 = 커밋 `2e8a8f099`의 5개 파일과 현 App Data Plane 라우트 대조. 변경 범위 = 본 보드 기록만, Claude Code 제품 코드 0. 검증 = 정적 코드 대조 및 로드맵 G1-B01~B05·D-014 계약 대조, 테스트 재실행은 하지 않음. 커밋/푸시 = 미실시. 재개 지점 = 위 5개 보정 후 shadow 배선 검토. 금지 범위 = 미보정 PDP로 기존 `assert_release_*` 즉시 교체, 빈 scope/owner를 공용으로 간주, 토큰 전문 로그·iframe 전달, G2에서 앱 전용 Resource를 범용 계약으로 직접 재사용.

### [G2-ONTOLOGY-DESIGN-74] 첫 수직 폐루프 상세 설계 완료
- 작성자 / 기록 시각: Codex / 2026-08-13 10:54 KST
- 왜 지금 기록하는가: G1-B 구현과 병행해 G2의 독립 설계 범위를 완성하고, G1-B 완료 직후 Claude Code가 추측 없이 구현에 착수할 수 있는 객체·관계·질의·화면·마이그레이션·G4 인계 기준선을 고정하기 위해 기록한다.
- 상태: **상세 설계 완료 · 구현 대기** — 코드·DB·API 변경 0건, G1-B P0 착수 조건 유지.
- 결정 및 근거:
  - 스타터 키트 35개 계약을 실측해 원료 구매→선적·통관·운송→재고→생산→판매→재무의 첫 수직 폐루프를 객체 28종·관계 유형 25종·허용 조합 25개로 정의했다.
  - 객체는 원천 SSOT를 복제하지 않고 `namespace/type/id` 구조로 참조한다. 트랜잭션·이벤트는 승인 Snapshot/Query Contract의 키로만 접근한다.
  - 자연어 질문 9종을 `객체 해석→모호성 선택→결정론적 Query Plan→승인/as-of/범위 필터→근거 경로→선택적 G4 인계`로 고정했다. LLM은 SQL·권한·승인·수치 계산을 수행하지 않는다.
  - 기업 경영 의미지도는 승인 제품 셸을 재사용하고 전체 그래프가 아니라 질문에 답하는 1~3개 핵심 경로를 기본 표시한다. 근거·버전·기준시점·데이터 준비·계산 가능 여부와 다음 행동을 함께 보여 준다.
  - G4 실측에서 `driver` 가정이 `pct`만 지원하고 미승인 `driver_impacts`도 경고 후 계산되는 결함을 확인했다. `DRV-DELAY 14 DAY`, `DRV-CAPEX set`을 실행하려면 `pct|delta|set`별 승인 계산기와 미승인 계수 Fail-closed가 필요하다.
- 근거: `docs/architecture/G2_MANUFACTURING_MANAGEMENT_ONTOLOGY_DETAILED_DESIGN_2026-08-13.md`, `docs/architecture/g2_first_vertical_ontology_contract_v1.json`. JSON 파싱 검증 결과 객체 28·관계 25·제약 25·질의 intent 9.
- 영향·주의사항: 현재 `connectors/query_contracts/external_observations` 운영 행은 0이므로 DEMO Snapshot을 운영 데이터처럼 사용하지 않는다. SYNTHETIC/DEMO_ONLY 경로는 실제 경영 의사결정 기준선으로 승격 금지. G1-B `ResourceScope` 실제 계약이 확정되기 전에는 G2 저장소·API를 만들지 않는다.
- 다음 행동 / 담당 / 착수 조건: Claude Code는 G1-B P0를 완료한 뒤 본 상세 설계의 `ResourceScope` placeholder를 실제 PDP 계약으로 치환하고 G2-D1 유형 정규화부터 구현한다. Codex는 구현 전 의미지도 화면 프로토타입과 질문·근거 UX를 시각 검증한다. Antigravity는 관계 근거 등급과 외부 원천 신뢰 기준을 교차검토한다.
- 교대 체크포인트: 마지막 확인 = 상세 설계·기계 계약 작성 및 JSON 파싱 성공. 변경 범위 = 문서 3종과 본 보드 기록, 제품 코드 0. 검증 = 스타터 키트 manifest/contract/sample, calc bridge/planning driver 코드 대조. 커밋/푸시 = 미실시. 재개 지점 = G1-B 완료 후 G2-D0 ResourceScope 결합. 금지 범위 = G1-B 전 구현, 운영 데이터로 DEMO 사용, 미승인 계산 실행, 전체 그래프 기본 노출, 권한 밖 건수 노출.

### [G2-ONTOLOGY-REVIEW-73] 착수 설계 교차검토 및 정본 결정
- 작성자 / 기록 시각: Codex / 2026-08-13 10:30 KST
- 왜 지금 기록하는가: Claude Code가 `[G2-ONTOLOGY-72]`를 접수하고 읽기 전용 설계 보고서를 작성했으며, 보고서가 요청한 중복 유형 정본·관계 정의 범위와 권한·스키마 보정사항을 팀 공통 결정으로 남기기 위해 기록한다.
- 상태: **조건부 승인 · 설계 개정 대기 · 구현 금지 유지**
- 결정 및 근거:
  - 중복 유형 4쌍은 운영 바인딩·현행 시드·테스트가 사용하는 하이픈형(`quality-spec`, `finance-param`, `emission-factor`, `sensor-spec`)을 정본으로 한다. 밑줄형은 명시적 `entity_type_aliases` 또는 동등 매핑으로 보존하며 `master_records.supersedes`나 레코드용 `aliases`를 유형 별칭으로 오용하지 않는다.
  - `entity_types.relations` 21개를 전수 수작업으로 채우지 않는다. 첫 수직 폐루프에 필요한 객체·관계만 먼저 정의하고 나머지는 `UNMODELED`로 구분한다.
  - `entity_types.relations`는 M1의 허용 관계 후보/호환 projection이며 최종 SSOT가 아니다. 관계 유형·허용 subject/object 조합·관계 인스턴스·이력을 분리한 정본 계약을 설계한다.
  - 권한 밖 또는 미승인 관계의 개수·사유를 사용자 응답의 `blocked`로 반환하지 않는다. 권한은 있으나 현재 선택 문맥에서 제외된 수만 `context_omitted`로 표현하고, 진짜 권한 거부는 존재 은폐한다.
  - 프로젝트 전용 `project_visibility`를 온톨로지 노드·간선에 직접 적용하지 않는다. G1 정책 원칙과 scope primitive는 재사용하되 G1-B 정책 결정점의 범용 `ResourceScope` 어댑터를 사용한다.
- 근거: `docs/design_manufacturing_management_ontology_2026-08-13.md` §12 Codex 교차검토. 코드 실측상 현행 시드·테스트는 하이픈형을 사용하고 `org_seed.py`·과거 로더에는 밑줄형 참조가 남아 있어 단순 레코드 이관으로 끝나지 않는다. `docs/design_master_data_m1.md`는 `entity_types.relations`를 허용 관계 후보로 정의한다.
- 영향·주의사항: 본 결정은 설계 문서만 보정한다. 코드·DB·API 마이그레이션, 유형 데이터 변경, G1 작업 파일 수정은 금지한다. 사용자 응답에서 권한 밖 개수를 보여 주는 것은 Data Stealth 원칙 위반이다.
- 다음 행동 / 담당 / 착수 조건: Claude Code는 보고서 §12를 반영해 설계만 개정하고, G1-B P0 완료·커밋 후 `유형 통합 영향/롤백 계획`, `첫 수직 폐루프 객체·관계·허용 조합`, `ResourceScope 호출 흐름`을 구현 전 재보고한다. Codex는 기업 경영 의미지도 질문·경로·근거 UX를 후속 설계한다. Antigravity는 관계 근거 등급과 그래프 DB 재판정 기준을 검토한다.
- 교대 체크포인트: 마지막 확인 = Claude Code 설계 보고 수신·코드 0건 확인·Codex 교차검토 반영. 변경 범위 = 본 보드 항목과 설계 문서 §12. 검증 = 코드 검색·M1 정의·현행 시드/테스트 참조 대조. 커밋/푸시 = 미실시. 재개 지점 = Claude Code 설계 개정 후 G1-B P0. 금지 범위 = G1-B 전 G2 구현, 권한 밖 건수 노출, 관계 JSON 21개 임의 채움, 레코드 별칭을 유형 별칭으로 오용.

### [G2-ONTOLOGY-72] 제조 경영 온톨로지 런타임 정식 인계
- 작성자 / 기록 시각: Codex / 2026-08-13 00:32 KST
- 왜 지금 기록하는가: 제품 문서에는 온톨로지의 정의와 G2 구현 범위가 반영됐지만, Claude Code의 명시적 작업 큐와 완료 기준으로는 전달되지 않은 상태였다. 문서를 우연히 발견하는 것과 정식 인계를 구분하고, 현재 진행 중인 G1 보안 작업 완료 후 바로 착수할 후속 패키지를 고정하기 위해 기록한다.
- 상태: **정식 인계 완료 · 착수 대기** — 현재 진행 중인 G1-C/G1-B 보안 기준선 완료 및 커밋 이후 Claude Code가 착수한다. 이 기록 자체는 구현 완료를 뜻하지 않는다.
- 순서 보정(Claude Code 접수 후 / Codex / 2026-08-13): `G2-C 착수 보고서`에 해당하는 읽기 전용 영향도·재사용·마이그레이션 설계는 G1-C3 완료 후 바로 작성해도 된다. 그러나 **G2 코드·DB·API 마이그레이션 착수는 G1-B P0(B01~B05: SDK 계약, 상위 토큰 금지, 단기 capability token, 정책 결정점, 감사·텔레메트리)가 완료·커밋되기 전에는 금지**한다. 현재 `PROGRESS.md`의 생성 앱 데이터 평면은 2.5/7이며 I-3 iframe 브리지와 I-4 생성기 연동이 미완이므로, G1-C3 완료만으로 G2 구현 착수 조건을 충족한 것으로 보지 않는다.
- 결정 및 근거:
  - 기술명은 `제조 경영 온톨로지(Manufacturing Management Ontology)`, 제품 설명은 `경영 의미 모델`, 사용자 화면명은 `기업 경영 의미지도`로 통일한다.
  - 기존 ECM·MDM·데이터 카탈로그·용어사전·Crosswalk·계보를 폐기하거나 중복 구현하지 않고, 이들을 승인된 관계와 근거 경로로 결합하는 **통합 의미 런타임**을 만든다.
  - 1차 저장은 현재 운영 기반과 맞는 관계 테이블/서비스 계층으로 구현한다. 별도 그래프 DB 도입은 실제 질의량·깊이·성능 병목을 측정한 뒤 결정한다.
  - 공식 관계는 LLM이 자동 확정하지 않는다. LLM/Graph RAG는 후보 관계와 탐색 경로를 제안할 수 있으나 승인·버전·유효기간·조직 범위·근거가 없는 관계는 운영 답변과 시뮬레이션 입력에서 제외한다.
  - 온톨로지는 `무엇이 무엇과 왜 연결되는가`와 근거 경로를 반환하고, 영향 경로의 수치 계산은 G4 결정론적 계산 그래프가 담당한다. 온톨로지나 LLM이 경영 수치를 임의 생성하지 않는다.
- 구현 범위 / Claude Code:
  1. `G2-C01` 통합 서비스 계약: ECM·MDM·카탈로그·용어·Crosswalk·계보를 하나의 의미 질의 계층으로 연결.
  2. `G2-C02` 관계 저장 계약: `subject/object/relation_type/version/effective_from/effective_to/tenant_id/scope_node_id/entity_mode/approval_status/evidence_refs/source_lineage`를 최소 필드로 관리.
  3. `G2-C03` 승인·변경·폐기·버전 이력과 as-of 조회. 승인 전·만료·폐기 관계는 기본 질의에서 Fail-closed.
  4. `G2-C04` 조직·테넌트·REAL/VIRTUAL/SANDBOX 범위 격리 및 존재 은폐(권한 밖 자원은 404).
  5. `G2-C05` 자연어 질문을 승인된 개체·관계·정책·계산 참조의 질의 계획으로 변환하고, 답변에 객체·관계 경로·원천·근거·기준시점·권한 판정을 함께 반환.
  6. `G2-C06` 동의어·유사어·Crosswalk 후보와 Graph RAG 탐색 후보를 제안하되 사용자 승인 전에는 공식 의미망에 반영하지 않음.
  7. `G2-C07` 데이터 품질 상태가 답변·시뮬레이션 사용 가능 여부에 미치는 영향을 명시하고 G4 계산 그래프에는 승인된 `calculation_ref`만 전달.
- 완료 기준:
  - 동일 사용자·동일 기준시점·동일 버전 질문은 동일한 승인 관계 경로를 재현한다.
  - 미승인·만료·범위 밖 관계가 검색·답변·시뮬레이션 입력에 노출되지 않는다.
  - 모든 반환 경로에 `evidence_refs`, `source_lineage`, `as_of`, 버전, 범위 판정이 있다.
  - 대표 수직 질문 `원료 도입 지연이 어느 공장의 어떤 생산계획·매출·현금흐름·손익에 영향을 주는가?`에 대해 의미 경로를 반환하고, 수치 산출은 G4로 위임한다.
  - 권한·기간·승인·버전·샌드박스 격리 및 변이 테스트가 존재하며 전체 회귀 테스트가 통과한다.
- 영향·주의사항: 현재 다른 세션이 수정 중인 인증·SSE·`core/project_visibility.py`를 건드리지 않는다. G1 보안 기준선이 확정되기 전에 범위 판정기를 중복 구현하지 않는다. `entity_types` 등 기존 M1/M2 자산을 새 테이블로 무조건 복제하지 말고 SSOT와 참조 관계부터 판정한다.
- 다음 행동 / 담당 / 착수 조건: Claude Code는 현 G1 작업을 원자적으로 완료·커밋한 뒤 로드맵 `§G2-C`와 본 항목을 읽고 구현 전 영향 파일·재사용 대상·마이그레이션 계획을 먼저 보고한다. Codex는 API/화면 용어와 `기업 경영 의미지도` UX를 교차검토하고, Antigravity는 외부 표준·관계 근거 품질을 교차검토한다.
- 교대 체크포인트: 마지막 확인 상태 = 문서 명세와 팀 보드 인계만 완료, 코드 구현 0%. 변경 범위 = `.agents/TEAM_BOARD.md` 인계 항목. 검증 증거 = `docs/roadmap/AI_FACTORY_STUDIO_FINAL_COMPLETION_EXECUTION_PLAN_2026-08-03.md`의 G2-C01~C07. 커밋/푸시 상태 = 미실시. 재개 지점 = G1 완료 후 G2 영향도·재사용 설계 보고. 금지 범위 = 현재 G1 작업 파일 동시 수정, 무승인 관계 확정, 온톨로지에서 수치 계산, 별도 그래프 DB 선도입.

### [BASELINE-CONSOLIDATE-71] 후속 보안 작업 전 작업트리 기준선 확정
- 작성자 / 기록 시각: Codex / 2026-08-12 23:22 KST
- 왜 지금 기록하는가: G1-C3 통합 가시성 판정과 G1-B Host Runtime 착수 전에 여러 세션의 완결 산출물과 미완 초안을 분리해 깨끗한 기준선을 만들기 위해 기록한다.
- 상태: **검증·선별 커밋 완료** — App Data Plane 잔여 계약, UI 수용점검 보정, 기술용어 사전·용어집, 샘플 회사 스타터 키트만 포함했다.
- 결정 및 근거: 스타터 키트 405/405 PASS(quick 13,585행·full 155,508행), Excel 35종·수식 175셀 PASS, App Data Plane 39건 PASS, 프론트 `npm.cmd run build` PASS. `core/project_visibility.py`의 G1-C3 함수 초안은 목록·상세·쓰기·SSE에 아직 연결되지 않아 기준선 커밋에서 제외한다.
- 영향·주의사항: `starter_kits/` 약 45MB는 모두 SYNTHETIC 샘플이며 최대 단일 파일 약 13.4MB다. 운영 Actual/CERTIFIED로 승격하지 않는다. 다른 세션이 작성 중인 통합 가시성 초안을 덮어쓰거나 `git add -A`로 섞지 않는다.
- 다음 행동 / 담당 / 착수 조건: Codex가 아래 4개 커밋과 본 기록을 `origin/dev`에 푸시한다. Claude Code는 푸시된 기준선에서 G1-C3를 완성하고 목록·상세·쓰기·SSE 동일 판정 테스트를 추가한다.
- 교대 체크포인트: 마지막 확인 = 위 4개 검증 전부 성공. 변경/미변경 = 완결 산출물만 확정, `core/project_visibility.py` 초안과 `scripts/seed_comprehensive_test_data.py`는 미확정으로 작업트리에 유지. 커밋 = `18c1d8363`(App Data) · `b2690b6e9`(UI) · `974d582b6`(용어) · `5939597ca`(스타터 키트). 푸시 상태 = 본 기록 시점 미실행. 재개 지점 = 푸시 후 G1-C3. 금지 범위 = 미완 초안 포함 · 운영 데이터 승격 · 원격 설정 변경.

### [TERM-GLOSSARY-70] 기술·제품 용어 사전과 사용자 용어집 화면
- 작성자 / 기록 시각: Codex / 2026-08-12 23:22 KST
- 왜 지금 기록하는가: 사용자 용어와 기술 식별자를 무리하게 일괄 치환하지 않으면서 WBS·RFP·PRD 같은 표준 약어의 유지 원칙을 제품에 고정하기 위해 기록한다.
- 상태: **완료** — 정책 v0.2.1, JSON 정본, Excel 사전, 프론트 용어집, UI 전환 백로그를 생성했다.
- 결정 및 근거: WBS·RFP·PRD는 `표준 약어 유지`, 최초 노출에서 뜻 병기, API·DB·코드 식별자는 별도 승인 없이 변경 금지. 용어 감사 재실행 결과 53개 용어·1,767개 위치를 기록했고 프론트 빌드가 통과했다.
- 영향·주의사항: 감사 백로그는 자동 치환 명령이 아니다. 사용자 노출 문자열과 기술 식별자를 화면군별로 구분해 이관한다.
- 다음 행동 / 담당 / 착수 조건: Codex가 향후 UI 화면군별 용어 전환을 시각 검증과 함께 수행한다. Claude Code는 API·DB 식별자를 현행 유지한다.
- 교대 체크포인트: 정본 = `data/terminology/technology_terminology_glossary.json`, 사용자 열람 = `TerminologyGlossaryPanel`, 문서 = `docs/architecture/AI_FACTORY_STUDIO_TECHNOLOGY_TERMINOLOGY_DICTIONARY_2026-08-12.xlsx`. 검증 = 감사 스크립트·프론트 빌드 PASS. 재개 지점 = 백로그 우선순위별 화면 이관.

### [SAMPLE-KIT-69] 제조 경영 샘플 회사 스타터 키트 기준선
- 작성자 / 기록 시각: Codex / 2026-08-12 23:22 KST
- 왜 지금 기록하는가: 신규 사용자가 맨땅에서 시작하지 않고 실제와 유사한 합성 데이터·등록 양식·시나리오를 그대로 따라 할 수 있는 제품 기준선을 남기기 위해 기록한다.
- 상태: **완료·검증 통과** — 비철금속 원료구매 폐루프 키트, 배터리소재 확장 키트, 가상회사 6종, 앱 청사진·보고서·시나리오·Excel 입력 양식을 생성했다.
- 결정 및 근거: 계약·참조·대사 405/405 PASS, quick 13,585행·full 155,508행, Excel 35종 필수 6시트·수식 175셀 PASS. 모든 업무 표본은 `data_class=SYNTHETIC`, `data_origin=SYNTHETIC`으로 고정했다.
- 영향·주의사항: 샘플 수치는 제품 기능 검증용이며 실제 회사의 경영 수치가 아니다. 실제 사용 시 Source Inventory→프로파일링→대사→데이터 오너 승인 뒤에만 CERTIFIED 기준선을 발급한다.
- 다음 행동 / 담당 / 착수 조건: 기능 구현 완료 후 등록 마법사와 데이터 준비 상담사가 이 키트의 manifest·계약·Excel을 읽도록 연결한다. 운영 승격 전 시험 데이터 차단 게이트를 먼저 완성한다.
- 교대 체크포인트: 정본 = `starter_kits/KIT-MFG-NONFERROUS-PROCUREMENT/1.0.0`, 보조 키트 = `KIT-MFG-BATTERY-CHEMICAL-PROCUREMENT/1.0.0`, 실행 문서 = `docs/data-kits/`. 검증 보고서 2종 PASS. 재개 지점 = 등록 API·화면 연결. 금지 범위 = SYNTHETIC을 Actual/CERTIFIED로 표현.
...
- 근거: 커밋 `91f328381` · `tests/test_publication_visibility.py`(16건) · 진척 정본 `PROGRESS.md`(122/145) · 격리 저장소 실측 2회(봉합 전/후)
- 상태: **발간 계열 봉합 완료** · 전체 pytest **2,522 passed · 1 skipped** · 잔여 21건
- 결정 및 근거:
  - ★★★ **`Publication.get(id, user_id)` 이 `user_id` 를 받고도 쓰지 않았다.** 판정이 아예 없었다. 아무 상관 없는 식별 사용자가 남의 대외 발간물을 조회하고 두 필수 검토를 혼자 채우고 **외부로 내보낼 수 있었다.** 이 저장소가 `project.release` 에 쓴 말 그대로다 — 「남에게 나간다. 회수해도 이미 본 사람이 있다」.
  - ⚠️ **기존 테스트 33건이 못 잡았다.** 전부 절차 게이트(렌더→승인→발간, 대외 이중 검토)만 봤고 **교차 사용자 접근은 0건**이었다. 절차가 옳아도 «누가» 가 비어 있으면 통제가 아니다.
  - **`get()` 하나를 판정 지점으로 삼았다** — render·approve·publish·correct·withdraw 가 전부 여기를 지난다. 라우트마다 적으면 한 곳을 빠뜨리고 그 경로만 조용히 열린다. 거부는 **404 은폐**(403 은 존재를 알린다).
  - ★★ **«이중 승인의 사람 분리» 는 사용자 결정으로 하지 않았다.** 실측: 활성 12개 부서에 `legal` 이 **없고** 1인 부서가 넷이다. 강제하면 대외 발간이 `hq` 6명·실질 관리자 3명에게 몰리고 휴가·부재 대행이 막힌다 — 막힌 흐름은 «승인 계정 돌려쓰기» 로 우회되며 **기록만 더 나빠진다.** 위험했던 것은 「한 사람이 두 번 눌렀다」가 아니라 「관계없는 사람이 건드렸다」였다.
  - ⚠️ **숫자가 틀렸던 이유**: 종전 H 는 권한 배정표가 다루는 **7개 라우터만** 세고 「남은 4건」이라 적었다. 전체 라우터로 훑으니 28건. 분모 41 → 65, 합계 121 → 145(95% → 84%). **떨어진 것이 아니라 세기 시작한 것이다.**
  - ★ 그리고 내가 처음 적은 `44/71` 도 **유도 없이 적은 숫자**였다. 검산에서 잡아 `44/65` 로 고쳤다 — 진척은 세는 것이지 어림하는 것이 아니다.
- 영향·주의사항:
  - `viewer_scopes=None` 은 «범위를 넘기지 않았다»(레거시)로 **판정이 꺼진다.** 새 호출부는 반드시 범위를 넘길 것 — 라우트 전수 검사를 테스트에 넣어 뒀다.
  - `UNRESTRICTED` 센티넬과 `None` 을 **뭉개지 말 것.** 뭉개면 범위를 깜빡한 호출이 관리자 권한으로 통과한다.
  - 범위가 빈 발간물은 «전사» 가 아니라 «미지정» 이다 → 작성자만 본다. 미지정을 전사 공개로 읽으면 **분류하지 않은 문서가 전부 열린다.**
  - ⚠️ **`app_delivery` 6건은 봉합 대상이 아니다** — 저장소 층에 당사자 판정이 있고 `test_collaboration_scope_security` 13건이 지킨다. 정리한다면 `EXEMPT` 에 이유를 적는 일이지 봉합이 아니다.
- 다음 행동 / 담당 / 착수 조건: **Claude Code** — 잔여 21건 중 **`decision_control` 8건**부터. 의사결정 패키지는 발간의 **원천**이므로(발간물이 `source_type=decision` 으로 가리킨다) 같은 유형의 구멍일 가능성이 높다. 그다음 `admin_control` 3 · `sandbox_control` 2.
- 교대 체크포인트: 마지막 확인 상태 = 전체 pytest **2,522 passed · 1 skipped** · 봉합 후 재실측(남남 조회·승인·발간·목록 전부 404 은폐 / 관계자·작성자·관리자 통과) · 변이 검사(가시성 끄기 → 9건 실패). 변경 = `core/publication.py` · `api/routes/publication_control.py` · `tests/test_publication_visibility.py`(신설) · `PROGRESS.md`. 미변경 = 이중 승인 규칙 · `app_delivery`·`mcp` 판정 · 기존 발간 테스트 33건. 커밋/푸시 = `91f328381` + 문서. 재개 지점 = `decision_control` 8건. 금지 범위 = `viewer_scopes` 없이 발간 서비스 호출 · `None` 과 `UNRESTRICTED` 뭉개기 · 범위 빈 발간물을 전사 공개로 읽기 · 이중 승인 사람 분리 강제(조직에 법무 담당이 생기기 전까지).

### [STUDIO-GATE-65] 트랙 E 7단계 — 게이트가 5건을 잡았고, **승격은 하지 않는다(카나리 유지)**
- 작성자 / 기록 시각: Claude Code / 2026-08-08 KST
- 왜 지금 기록하는가: §8 전환 게이트를 실제로 돌렸다. **전제 5건은 이미 채워져 있었는데(`878904ed1`) 게이트를 돌리자 4건이 더 나왔고**, 이어서 오프라인 검증에서 1건이 더 나왔다. 「전제 충족 = 승격 가능」이 아니라는 것을 남기지 않으면 다음 사람이 전제만 보고 승격한다. 그리고 **사용자 결정으로 승격을 미뤘다**는 사실과 그 조건을 남긴다.
- 근거: 커밋 `b557180b5`(게이트 4건) · `ef541c1bd`(진척) · `a78c3f782`(오프라인 1건) · 진척 정본 `PROGRESS.md`(115/121) · 명세 §8 `docs/uiux/SW_FACTORY_ADAPTIVE_PRODUCTION_STUDIO_IMPLEMENTATION_SPEC_2026-08-04.md`
- 상태: **전환 게이트 0/0/0 통과** · **기본 route 승격 보류(사용자 결정 — 카나리 유지)** · 백엔드 pytest 2,470 passed·1 skipped
- 결정 및 근거:
  - ★★★ **«전제 충족» 과 «게이트 통과» 는 다른 것이다.** 두 화면에 같은 버튼이 다 있어도 통과가 아니다 — 게이트는 **같은 프로젝트 상태를 두 화면 + 서버 원본 3자로** 대조하는 것이다. 기능 목록만 봤다면 아래 ③은 절대 안 잡혔다.
  - ① **기능 누락 2**: Export(`StageArtifactCanvas` 가 스스로 「종전 통제실에서 하십시오」라고 적어 둠) · 피드백 백로그 발행(`POST /sprint/revision`).
  - ② **상태 불일치 1**: HOTL 이 멈춰 세운 작업이 신규 WBS 척추에서 「진행」. 서버가 `IN_PROGRESS` 라 그대로 쓴 것인데 사용자는 «기계가 돌고 있다» 로 읽는다. 기존은 그 줄에 「⚠️ HOTL」을 붙이고 있었다 → `FactoryWbsKind` 에 `awaiting` 추가(판정은 ViewModel 한 곳).
  - ③ ★★ **작업 보호 실패 1 — 회귀였다.** Dock 입력이 화면 전환으로 사라졌는데 **기존 3패널은 보존한다**(언마운트되지 않는다). 신규로 옮기는 순간 «하던 일을 잃는 경로» 가 새로 생긴 것이다 → `factory/decisionDraft.ts`(모듈 스코프, `jarvisSession` 과 같은 방식).
  - ④ **오프라인 1**: 백엔드가 죽었는데 「산출물이 아직 준비되지 않았거나 **권한이 없습니다**」라고 말했다. `store.saveRelease` 가 네트워크 오류·403·409·500 을 `null` 하나로 뭉갠 탓이다 → `{ok, releaseId, message}`. ⚠️ 호출부는 `if (rid)` 였는데 **객체는 항상 truthy** 라 `tsc` 가 통과시킨다 — 반환 타입만 바꾸고 호출부를 뒀다면 오프라인에서 «성공» 이라 말하는 더 나쁜 상태가 됐다.
- 영향·주의사항:
  - **복사하지 않았다.** Export·수정 요구 둘 다 `factory/sprintActions.ts` 에 올려 **종전 통제실과 같은 엔드포인트**를 쓴다. 다른 주소를 만들면 두 화면이 다른 것을 내려받고, 그때 어느 쪽이 «진짜 산출물» 인지 알 수 없다.
  - 수정 요구는 Dock 승인과 **다른 칸**이다 — 하나는 멈춰 선 결정에 답하는 것(`/hotl/resume`), 다른 하나는 새 작업을 만드는 것(`/sprint/revision`). 묶으면 「승인했는데 왜 새 작업이 생겼나」가 된다.
  - ⚠️ **아직 확인하지 못한 화면 상태**: 요구 확인 «대기»(질문 카드가 실제로 뜨는 경로) · 복구 실패 화면. 그 상태의 프로젝트가 없고 만들려면 **스프린트를 돌려야 한다(LLM 비용)**. 승격 전에 이것을 닫아야 한다.
- 다음 행동 / 담당 / 착수 조건: **승격 조건 3가지** — ⓐ 요구 확인 «대기»·복구 실패 화면을 실제 프로젝트로 확인(스프린트 1회 필요 = 사용자 승인) ⓑ 카나리 실사용 피드백 ⓒ 승격 시 「종전 통제실로」 되돌림 경로 유지(이미 있음). **Claude Code** 는 그동안 트랙 B P2(2건)·H(4건)를 진행한다.
- 교대 체크포인트: 마지막 확인 상태 = 전환 게이트 0/0/0 · 1280×720·1440×900 가로 넘침 0·12px 미만 0·Canvas↔Dock 겹침 0 · Export `application/zip` 21,947 bytes · 오프라인 문구 시정 확인 · 백엔드 pytest 2,470 passed. 변경 = `factory/` 8파일(`decisionDraft.ts` 신설) · `store/useFactoryStore.ts` · `components/ControlPanel.tsx` · `PROGRESS.md`. 미변경 = **기본 route(카나리 유지)** · 기존 3패널 · 백엔드 전체. 커밋/푸시 = `b557180b5`·`ef541c1bd`·`a78c3f782`. 재개 지점 = 트랙 B P2. 금지 범위 = 전제만 보고 승격 · 게이트 없이 3패널 제거 · 실패 이유를 지어내기 · 반환 타입 바꾸고 호출부 방치(`if (obj)` 는 항상 참이다).

### [SCREEN-MIGRATE-64] 트랙 F 8/8 완료 — 이관은 겉모습 교체가 아니었다
- 작성자 / 기록 시각: Claude Code / 2026-08-07 22:xx KST
- 왜 지금 기록하는가: 트랙 F(추가 8화면)를 끝냈다. 그런데 **8개 중 7개에서 «조용한 거짓말» 이 나왔다** — 같은 결함이 화면마다 다른 옷을 입고 있었다. 그 목록을 남기지 않으면 남은 화면·새 화면에서 같은 것이 다시 만들어진다.
- 근거: 커밋 8건(`4f086b127` `05b8f8dbd` `117ac6300` `52dff84ee` `1156430a7` `42ffab9e2` `7b41e3cff` `2ef4ed813`) · 진척 정본 `PROGRESS.md`(114.5/121) · 살아 있는 화면 검증(프론트 5175 + 백엔드 8081)
- 상태: **완료** · `tsc -b` 0 · `npm run build` 성공 · 백엔드 pytest 2,470 passed·1 skipped(회귀 없음)
- 결정 및 근거:
  - ★★★ **결함은 하나였다: 조회 실패를 «0건»·«없음» 으로 보여주는 것.** 백엔드는 이번 세션 내내 `Metric`(값 + «읽었는가»)으로 이것을 막아 왔는데, **화면이 그 구분을 지우면 백엔드가 지킨 것이 소용없어진다.** 8개 전부 `Loaded<T>` 로 바꿨다.
    · 운영 계기판: `.catch(() => setData(null))` → 403·장애·500 이 전부 「호출 0건」
    · 크로스워크: `r.ok ? r.json() : { data: [] }` → 승인 대기 **관문**이 비어 보인다
    · 워크스페이스: 「공유 없음」·「복제 없음」 → 여러 부서가 쓰고 있어도 안 보이고, 그 상태로 롤백하면 끊기는 쪽이 원인을 모른다
    · 상담사: 결정 이력 `.catch(() => [])` → **구획이 통째로 사라져** 근거가 원래 없는 줄 안다
  - ⚠️ **가장 아팠던 것(경영계획)**: 그 파일 주석이 스스로 「화면은 "등록된 시나리오가 없습니다" 라고 말하는데 실제로는 3건이 있었다 — 조용한 거짓말이다」라고 적어 두고, **바로 아래 줄에서** `s.status === 'fulfilled' ? s.value : []` 를 하고 있었다. **한 입구를 막고 다른 입구를 열어 둔 것이다** — 주석이 있다는 것과 고쳐졌다는 것은 다르다.
  - **되돌리기 어려운 행동 8곳에 확인이 없었다.** 사용 중단·전사 승격·롤백·공유 회수·매핑 확정·청사진 승인·반려·LLM 보강. 특히 상담사는 「미확보 필수 데이터 N건을 안고 승인했습니다」를 **승인 후에** 알려줬다 — 그때는 이미 결정 이력에 남았고 그 원장은 수정할 수 없다.
  - **미리보기만 `HubDialog` 를 쓰지 않았다.** `createPortal` 이 iframe 을 재마운트해 실행 중인 생성 앱 상태가 죽는다. 그 자리에서 모달 의미론만 갖췄고, 전환 후 iframe 이 **같은 DOM 노드**로 남는 것을 실측했다(`sameNode=true`).
- 영향·주의사항:
  - 데이터 계층 4곳(`shadowApi`·`workspaceApi`·`planningApi`·신규 `crosswalkApi`)이 상태 코드를 실어 던지도록 통일했다 — 그러지 않으면 화면이 «권한 없음» 과 «서버 장애» 를 구분할 수 없다. 새 API 클라이언트도 이 규약을 따를 것.
  - 디자인 시스템에 `.afs-table`·`.afs-table-wrap`·`.panel-body` 를 추가했다. 표·패널 여백을 **화면마다 새로 만들지 말 것**(종전에 `style={{padding:14}}` 가 스무 번 반복되고 있었다).
  - ⚠️ 크로스워크의 **자체 `API_BASE_URL` 선언을 제거**했다. 예전에 같은 방식으로 지식 허브의 모든 호출이 조용히 익명으로 나갔다. 화면에서 base URL 을 다시 선언하지 말 것.
  - ⚠️ **누르지 않은 것**: 승인·승격·롤백·회수·매핑 확정·LLM 보강. 상담사의 승인·반려는 저장소 청사진이 전부 승인 상태여서 **버튼 자체가 뜨지 않았다** — draft 를 만들려면 새 상담(=DB 쓰기)이 필요하다. 다음 사람이 draft 상태에서 한 번 확인할 것.
- 다음 행동 / 담당 / 착수 조건: **Claude Code** — 남은 트랙은 B P2(2건) · E 승격 판정(1건) · H(4건). **Codex** — 거버넌스 콘솔에 조직 운영 현황(ORG-ATTRIB-61)과 자산 위생(ASSET-USAGE-63)을 붙인다. 이관된 8화면의 시각 승인도 함께.
- 교대 체크포인트: 마지막 확인 상태 = `tsc -b` 0 · `npm run build` 성공 · 백엔드 pytest 2,470 passed·1 skipped · 화면 8개 살아 있는 서버 검증(12px 미만 0 · 가로 넘침 0 · 실데이터 렌더 · 장애 경로 «0건이 아닙니다» 확인). 변경 = 화면 8개 · API 클라이언트 4개 · `afs.css`(클래스 3종 추가) · `PROGRESS.md`. 미변경 = 백엔드 전체 · iframe 런타임 · `ManualRenderer`. 커밋/푸시 = 커밋 8건 + 문서. 재개 지점 = 트랙 B P2 또는 E 승격 판정. 금지 범위 = 조회 실패를 «0건»으로 표시 · 화면에서 base URL 재선언 · `alert()`/`prompt()` · 되돌리기 어려운 행동에 확인 없이 실행 · 미리보기 iframe 을 포털로 옮기기.

### [ASSET-USAGE-63] P4-4 완주 — 「미사용」을 만들지 않고 **관측을 만들었다**
- 작성자 / 기록 시각: Claude Code / 2026-08-07 17:5x KST
- 왜 지금 기록하는가: 트랙 B P4 가 4/4 로 닫혔다. 그리고 이번에도 **단위 테스트가 초록인데 서버에서 틀린** 사례가 나왔다 — 그 발견 경로를 남기지 않으면 다음 사람이 같은 방식으로 검사를 짠다.
- 근거: `core/asset_usage.py`(신설) · `core/asset_dedup.py` `find_unused` · `tests/test_asset_usage.py`(19건) · 진척 정본 `PROGRESS.md`(106.5/121) · 설계 `docs/design_p4_recommendations_2026-08-07.md` §4.1
- 상태: **완료** · 전체 pytest **2,470 passed · 1 skipped** · 살아 있는 서버 실측(익명 403 / 관리자 200)
- 결정 및 근거:
  - ★ **설계가 든 선행 2가지 중 ②(에이전트 귀속률 8%)는 이 목적에 필요 없었다.** ②는 LLM 호출 로그를 통한 **간접 추정**이고, 새로 만든 ①은 자산이 실제 해석되는 순간의 **직접 관측**이다. 간접이 좋아지기를 기다리느라 직접을 미룰 이유가 없다.
  - 기록 지점은 둘 — `agent_registry.agent_skill()`(파일 스킬이 **선택되는** 유일한 곳) · `agent_asset_adapter.resolve_workflow()`. ⚠️ **승인 가드에서 막힌 자산은 세지 않는다**(실행되지 않았다). 거기서 세면 「승인 안 된 자산이 잘 쓰이고 있다」는 모순된 화면이 나온다.
  - ⚠️⚠️ **관측은 켠 시점부터 쌓인다 — P4-2 의 귀속률 0% 와 같은 함정이다.** 다음 날이면 자산 전부가 「사용 기록 없음」이고, 그 화면을 본 사람은 전부 지운다. 그래서 관측 기간이 14일 미만이면 **목록을 만들지 않고**(빈 목록도 아니다) `available=False` 와 «약 N일 뒤 제공» 을 낸다.
  - **이름을 「미사용」으로 쓰지 않는다** — 「관측 이후 사용 기록 없음」이다. 「본 적 없음」과 「오래 전에 봤음」도 구분한다(후자만 근거가 있다).
  - ★★★ **단위 테스트가 초록인데 관측이 통째로 헛돌고 있었다.** `agent_skill()` 은 `rfp_skill` 을, 목록은 `rfp_skill.md` 를 쓴다 — 교집합 **0건**. 매 실행마다 쓰이는 스킬 31개가 전부 정리 대상으로 보고될 참이었다. **그 검사는 통과하고 있었다** — `agent_meta` 를 `"writer.md"` 로 대역해 둬서 테스트가 자기 자신과 합의했다. 잡은 것은 서버 실측이다(같은 함정 6번째). → 변환을 `asset_usage.skill_key()` 한 곳으로 모으고, 검사를 **대역 없이 실제 레지스트리·실제 `skills/`** 로 다시 썼다(변이 검사로 실제 잡는 것 확인 — 규약을 되돌리면 3건 실패).
- 영향·주의사항:
  - `conftest` 에 **관측 격리를 추가했다.** 없으면 `agent_skill()` 을 부르는 스위트 전반이 **운영 관측 기록을 만든다** — 그러면 「이 자산이 쓰이는가」의 답이 테스트 때문에 바뀐다. 검증: 전체 스위트 후 `data/asset_usage.db` 미생성 확인.
  - `data/asset_usage.db` 는 런타임 데이터이며 `.gitignore` 의 `*.db` 가 잡는다(저장소에 올라가지 않는다).
  - ⚠️ **화면은 `unused.available` 을 반드시 봐야 한다.** `False` 일 때 빈 목록을 그리면 그것은 「정리할 것이 없다」는 **거짓**이다. Codex 이관 대상.
  - ⚠️ 관측 시작 시각은 한 번 쓰면 갱신하지 않는다 — 갱신하면 창이 계속 짧아져 목록이 **영영** 안 나온다.
- 다음 행동 / 담당 / 착수 조건: **Claude Code** — 트랙 B 는 P2 2/4 만 남았다(21.5/23). 그 다음은 트랙 F(0/8) 또는 E 승격 판정. **Codex** — 거버넌스 콘솔에 조직 운영 현황(ORG-ATTRIB-61)과 자산 위생을 붙인다. 착수 조건 없음.
- 교대 체크포인트: 마지막 확인 상태 = 전체 pytest **2,470 passed · 1 skipped**(122s) · 살아 있는 서버 실측 완료. 변경 = `core/asset_usage.py`(신설) · `core/asset_dedup.py` · `core/agent_registry.py` · `core/agent_asset_adapter.py` · `api/routes/telemetry_control.py`(문서화) · `tests/conftest.py`(격리) · `tests/test_asset_usage.py`(신설 19건) · `PROGRESS.md`. 미변경 = 중복 판정 로직 · 권한 판정 · 자동 삭제 경로(만들지 않았다). 검증 증거 = 전체 스위트 · 변이 검사 · 서버 실측 · 스위트 후 운영 관측 DB 미생성 · `projects/` 56개 동일. 커밋/푸시 = 아래 커밋. 재개 지점 = 트랙 B P2 잔여 2건. 금지 범위 = `unused.available=False` 를 빈 목록으로 그리기 · 「미사용」이라는 이름 쓰기 · 자동 삭제 버튼 · 관측 시작 시각 갱신 · 대역으로 키 규약 검사하기.
...
- 근거: 상세 인계 `docs/chronicle/handoffs/handoff_2026-08-06_factory_studio_3to6.md` · 진척 정본 `PROGRESS.md`(43/80) · 커밋 5건(`f4867c5a2` ~ `054181b29`, 푸시 완료) · 명세 `docs/uiux/SW_FACTORY_ADAPTIVE_PRODUCTION_STUDIO_IMPLEMENTATION_SPEC_2026-08-04.md`
- 상태: **트랙 E 6/7 · 7단계 전제 미충족(기능 누락 5건)** · 검증 통과(`tsc -b` 0 · 두 해상도 넘침 0 · 12px 미만 0 · 콘솔 오류 0 · 6단계 12/12)
- 결정 및 근거:
  - **기존 자산을 옮겨 적지 않고 재사용했다(3곳).** ① `PreviewPanel` 의 iframe 런타임(샌드박스·가상 모듈 레지스트리·postMessage 프로토콜·팝업 싱글톤·출처 검증) — 옮기면 회귀가 날 곳이 여덟 군데다. ② 답변 직렬화 — **백엔드 `RFP_Analyst` 가 파싱하는 계약**이라 `factory/clarifyAnswers.ts` 로 추출해 `HOTLInput` 과 공유한다. ③ 마크다운 렌더러 `ManualRenderer` — export 를 붙여 재사용.
  - **없는 것을 만들지 않았다(7건).** 준비도 %·전환 조건·RFP 대비 Diff·누적 비용·빌드 번호·8칸 단계 지도 등. 특히 비용은 **₩0 으로 쓰면 「공짜로 돌고 있다」로 읽히므로** 「— 누적 비용 미측정」으로 뒀다. 각각 화면에 «무엇이 없는지» 를 적는다.
  - **「아직 없다」와 「서버가 아예 주지 않는다」를 구분한다.** 전자는 기다리면 생기고 후자는 안 생긴다 — 사용자가 해야 할 일이 다르다. 6단계에서 `UI_DESIGN`·`VISION_QA` 가 「올 예정입니다」로 **거짓말**하던 것을 이 구분으로 고쳤다.
  - ★★★ **WBS 진행 상태가 틀리게 표시되고 있었다.** 화면은 「진행 0 · 차단 3」, 실제는 「진행 1 · 차단 1」. `WbsSpine` 은 `'COMPLETED' || 'DONE'` 을 완료로 봤고 `factoryViewModel` 은 `!== 'COMPLETED'` 만 비교했는데 **실제 데이터가 `'DONE'`** 이었다. 완료 판정을 `isTaskDone()` 하나로 모으고 종류(`kind`)를 ViewModel 에서만 정한다. ⚠️ **3단계 스크린샷에 이미 있었는데 놓쳤다** — 두 표시가 모순인데 각각은 그럴듯했고, Inspector 가 같은 데이터를 다른 각도로 보여 주자 비로소 나란히 놓였다.
- 영향·주의사항:
  - **기존 3패널·기존 15개 화면은 건드리지 않았다.** 신규는 `.afs-studio` 스코프로 격리되고 「🏗 새 작업공간」 버튼 하나로만 들어간다. `HOTLInput`·`PreviewPanel` 변경은 **export 추가와 공용 함수 호출뿐**이며 동작 불변이다.
  - ⚠️ **`clarifyAnswers.ts` 의 문자열 형식을 바꾸면 백엔드 파서도 함께 봐야 한다.** 화면만 고치면 답변이 **오류 없이 「선택 없음」으로 읽힌다** — 가장 찾기 어려운 종류다.
  - ⚠️ **7단계 전제 미충족 5건**: Sprint 시작 · 재개(`resume-quota`) · 복구(`triggerSelfHealing`) · WBS 재분할(`wbs/replan`) · Release 저장·Export. **지금 3패널을 지우면 이것들이 사라진다.**
  - ⚠️ **Sprint 시작을 옮길 때**: `ControlPanel:234` 가 `project_state_payload`(스키마 버전·모드·`master_data`)를 조립한다. **복사하면 두 화면이 다른 payload 를 보내고** 「두 화면이 다른 프로젝트를 만든다」가 된다 — `clarifyAnswers.ts` 처럼 공용 모듈로 먼저 추출할 것.
  - 미확인 5건(모두 「그 상태의 프로젝트가 없다」): 요구 확인 **대기** 상태 · Decision Dock **표시** 상태 · 제출 실제 동작 · Jarvis 실제 응답 · 「선택≠현재」 강한 검증. 실행하려면 파이프라인·LLM 을 돌려야 하므로 **사용자 승인 사안**이다.
- 다음 행동 / 담당 / 착수 조건: **Claude Code** 가 7단계 전제 5건을 채운다(권고 순서: 재개·복구·재분할 → Release 저장 → Sprint 시작은 payload 공용화 선행). 그 다음 7단계(기본 route 승격, §8 전환 게이트는 「승격 전 3패널을 즉시 되돌릴 수 있게 유지」도 요구). 이어서 **트랙 G**(무방비 53 라우트 / 16 파일). **Codex** 는 stash 의 두 셸 체계 통합을 판정한다. **사용자 결정 4건**은 상세 인계 §8.
- 교대 체크포인트: 변경 = 신규 프론트 파일 11개(`frontend/src/factory/`) · `PreviewPanel`·`HOTLInput` 에 export/공용함수 배선 · `design/terms.ts` 에 `STAGE_STATUS_KO` 추가 · `PROGRESS.md`(41→43/80, 3~6단계 실측표·7단계 전제표) · `AI_HANDOFF.md` · 상세 인계 1건. 미변경 = 백엔드 전체·기존 3패널·기존 15화면·제품 데이터. 검증 = `tsc -b` 0 · 헤드리스 2해상도 넘침 0·12px 미만 0·콘솔 오류 0 · 6단계 12/12 · Inspector 폭 1202→1202px·스크롤 120→120px. 커밋/푸시 = **수행**(`054181b29`). 재개 첫 행동 = 7단계 전제 5건 중 ⓐ 착수 선언. 금지 범위 = 기존 3패널 선삭제, 단계 목록 하드코딩, 없는 지표 채우기, payload 복사, Mock 데이터로 완료 처리.

### [MERGE-VERIFY-59] 화면 이관 `dev` 병합 · 트랙 E 2/7 — 이번 세션 결함은 전부 «검증하려다» 나왔다
- 작성자 / 기록 시각: Claude Code / 2026-08-05 23:5x KST (세션 종료)
- 왜 지금 기록하는가: 사용자 지시(«별도 브랜치에서 병합 후 검증», «충돌은 더 엄격한 쪽 + 이유 기록»)에 따라 화면 이관 브랜치를 `dev` 에 병합하고 검증을 마쳤다. 그 과정에서 **파이썬 테스트 2,089건이 잡지 못한 결함 4종**이 나왔고, 다음 사람이 같은 함정을 다시 밟지 않도록 발견 경로까지 남긴다.
- 근거: 상세 인계 `docs/chronicle/handoffs/handoff_2026-08-05_merge_verification_and_factory_studio.md` · 진척 정본 `PROGRESS.md`(39/80) · 커밋 8건(`32386e233` ~ `3196c1c66`, 병합 커밋 `361a78762`)
- 상태: **병합 완료 · 트랙 C 10/10 · 트랙 E 2/7 · 검증 통과**(pytest 2,089 passed·실패 0 / `tsc -b` 오류 0 / `npm run build` 성공)
- 결정 및 근거:
  - **「충돌 마커 0건」은 안전이 아니다.** 충돌 4건은 해결했으나 위험했던 것은 자동 병합 쪽이었다. `api/deps.py` 에 같은 이름이 **두 벌** 남아 뒤의 정의가 앞을 덮어 `plan` 규칙만 사라졌고, `factory_control.py` 충돌을 dev 로 해결해 **브랜치 통제 9개**(`POST /{id}/hotl/resume` 포함)를 잃었다. 그래서 병합 검증은 ①중복 정의 ②통제 손실 ③정의 소실 ④실제 테스트 **네 가지를 따로** 본다. 익명 프로젝트 목록·생성 구멍은 **오직 ④가** 잡았다.
  - **의존성이 있다는 것과 판정이 있다는 것은 다르다.** `GET /projects` 는 `current_principal` 을 받으므로 자동 점검이 «통제됨» 으로 셌지만 익명에게 목록을 줬다. 정규식으로 판정 유무를 판별하려는 시도는 실패했다(상한 21개 검산 → 대부분 오탐).
  - **빈 파라미터가 가장 넓은 조회다.** `GET /planning/facts` 가 익명에게 경영계획 금액 20건을 줬다(dev 원본도 동일 — 병합 무관). `org_id`·`scope_node_id` 를 비우면 범위 판정이 «전사» 로 통과시키고 행 필터도 걸리지 않는다. 30개 라우트 중 17개가 `_assert_identified` 를 부르지 않았고 대부분은 **우연히** 막혀 있었다. 17개 전부 봉합 + 라우터를 읽어 목록 누락을 잡는 테스트 추가.
  - **프론트 프로덕션 빌드가 깨져 있었다.** `useConfirm` 이 `open` 을 안 돌려주는데 8곳이 그것을 읽어 **확인창이 한 번도 열리지 않았다** — 에이전트 구성 초기화·부서 폐지·부서 시드·스킬 승인·업무표준 폐지 버튼이 전부 죽어 있었다. `vite dev` 는 타입을 보지 않아 화면은 정상으로 보였다.
  - **프로젝트 작업공간이 상대경로였다**(37곳). D-018 ①이 DB 만 고정했다. 서버를 루트 밖에서 띄우면 다른 디렉터리를 읽고 `makedirs` 로 **빈 작업공간을 새로 만든다**. `reconcile_ownership` 기본값도 그래서, 미러가 0건이 되면 부서 필터가 «소유권 미기록» 으로 전부 통과시킨다 — **경로 하나가 권한 필터를 연다.**
- 영향·주의사항:
  - **Codex 미커밋 3파일을 `git stash@{0}` 에 보관했다**(`.agents/DECISIONS.md` +24 · `.agents/TEAM_BOARD.md` +52 · `frontend/src/App.tsx` +12/-10). 병합 대상과 겹쳐 그대로 두면 통합이 불가능했고, 사용자가 «지금 통합»을 선택했다. 파일 원본과 diff 를 별도 백업했다. ⚠️ **`git stash pop` 은 단순 복원이 아니다** — Codex 의 `App.tsx` 변경은 `.afs-product-shell` 로 기존 화면까지 라이트 전환하는 것이고, 병합된 쪽은 `.afs-scope` 안에서만 적용하는 격리 방식이다. **두 셸 체계가 반대 방향이라 통합 판정이 필요하다.**
  - ⚠️ `frontend/src/index.css` 가 `design/product-shell.css` 를 `@import` 하는데 그 파일은 **git 미추적**이다. 지금 clone 하면 프론트 빌드가 깨진다 — Codex 가 커밋하면 해소된다.
  - **테스트 격리 지점이 바뀌었다.** `monkeypatch.chdir(tmp_path)` 만으로는 더 이상 `projects/` 가 격리되지 않는다. `monkeypatch.setattr(core.paths, "PROJECTS_DIR", str(tmp_path / "projects"))` 를 함께 써야 한다(7곳 적용). 이 한 줄이 없으면 **테스트가 제품 `projects/` 에 프로젝트를 만든다** — 수정 전 실측 9개.
  - 트랙 E 는 **병행 카나리**다. 기존 3패널을 지우지 않았다(§7-7). 신규 화면에 Sprint 시작·정지·HOTL 승인이 없으므로 지우면 그 기능이 사라진다.
- 다음 행동 / 담당 / 착수 조건: **Claude Code** 가 트랙 E 3단계(Adaptive Canvas — 요구 확인·구현 실행 미리보기 두 극단 먼저)를 잇고, 그다음 트랙 G 첫 작업(익명에게 자료가 나가는 26경로를 «스키마·자기상태»와 «업무 자료»로 분류)을 한다. **Codex** 는 stash 의 두 셸 체계 통합을 판정하고 `product-shell.css` 를 커밋한다. **사용자 결정 4건**은 상세 인계 §8 에 있다 — 특히 ①에이전트 제어판 저장을 지금 아무도 할 수 없다(`SYSTEM_DEFAULT_EDIT` 보유자 0명·플랫폼 관리자 0명).
- 교대 체크포인트: 변경 = 병합 커밋 1 + 교정·봉합 커밋 7 · `PROGRESS.md`(38→39/80, 프론트엔드 검증 기준선 신설) · `AI_HANDOFF.md` · 상세 인계 문서 1건 · 신규 프론트 파일 5개(`frontend/src/factory/`) · 테스트 격리 7곳. 미변경 = 기존 3패널·Codex 미커밋 작업·제품 `projects/` 의 `__audit_probe__`. 검증 = pytest 2,089·실패 0 / `tsc -b` 0 / 헤드리스 Chrome 2해상도 넘침 0·12px 미만 0·콘솔 오류 0. 커밋/푸시 = **수행**. 재개 첫 행동 = 트랙 E 3단계 착수 선언. 금지 범위 = 기존 3패널 선삭제, 단계 목록 하드코딩, Mock 데이터로 완료 처리, 제품 DB·`projects/` 대상 쓰기 탐침.
...
### [SCOPE-DONE-58] D-018 완주(10/10) + 전체 범위 점검 — 세지 않던 일 15항목을 찾았다
- 작성자 / 기록 시각: Claude Code / 2026-08-05 KST
- 왜 지금 기록하는가: 사용자가 «부분 진척도를 전체인 것처럼 보여주니까 헷갈린다» 고 지적했고, 「진행하던 것 마무리 → 전체 점검 → SW 생성기 UI 를 전체 범위에 포함」을 지시했다. 셋을 다 했다.
- 상태: **D-018 10/10 완료 · 진척 정본 갱신(분모 49 → 64)**
- ★★★ **① D-018 ⑩ 완료 — 그리고 ⑦이 실제 결함을 막아 줬다.** `seed_example_organization(force=True)` 가 `node_id` 를 주지 않아 **새 id 를 만들려 했고**, ⑦(코드 유일성)이 `EcmError` 로 막았다. **막힌 것이 다행이다** — 종전에는 같은 코드의 노드가 하나 더 생기고 그 뒤 코드 해석이 «모호» 로 거부됐을 것이다. `node_id`·`entity_id` 를 보존하도록 고쳤다(실측 9/9 보존 · 엔티티 증가 0 · 엣지 동일 · 내용 갱신은 유지).
  ⚠️ **전체 테스트는 그때 통과했다** — `force=True` 경로를 검증하는 테스트가 없었기 때문이다. 그래서 `tests/test_scope_seed_preserves_ids.py`(7건)를 만들었다. 회귀를 잡은 것이 아니라 **회귀를 잡을 수 있게** 만든 것이다. 노드를 지우는 코드가 생기면 실패하는 소스 검사도 넣었다(ECM 노드는 `status` 로만 폐지된다 — 행을 지우면 그 id 를 가리키는 소유·권한 기록이 «없는 조직» 을 가리키고 fail-closed 는 «아무에게도 보이지 않음» 이다).
- ★★★ **② 전체 점검 — «세지 않던 일» 15항목을 찾아 분모를 49 → 64 로 늘렸다.** 진척률이 57% → **45%** 로 내려갔는데, 떨어진 것이 아니라 **세기 시작한 것**이다.
  · **트랙 E 신규(7항목)** — SW Factory Studio 재구축. `[UX-SW-FACTORY-ADAPTIVE-37]` 이 **담당 = Claude Code · «선택사항 아님»** 으로 지시했는데 **어느 트랙에도 없었다.** Concept E 채택으로 «이관» 이 아니라 **재구축**이 됐으므로(기존 3패널 → 병행 카나리 → 승격) 성격이 달라 C 에서 분리했다. 근거: `docs/uiux/SW_FACTORY_ADAPTIVE_PRODUCTION_STUDIO_IMPLEMENTATION_SPEC_2026-08-04.md` §7 구현 순서 7단계 + §8 완료 게이트 15개.
  · **트랙 F 신규(8항목)** — 화면 이관 추가 8화면. 사용자가 «전부 이관» 을 승인했으나 10단계 트랙에 자리가 없었다.
  · **Wave 정의와 실제 순서의 불일치**를 기록했다(`TEAM_BOARD:503`). 계획은 `Wave 1 데이터기반 → 2 안전연계 → 3 AI통제 → **4 핵심생성(SW Factory)** → 5 경영·Twin → 6 보조` 였고 **Wave 4 = 트랙 E** 다.
- ★★ **③ 내 보고 방식을 정본에 못 박았다.** 머리말에 «부분 트랙의 진척을 전체처럼 보고하지 말 것» 을 넣었다 — «D-018 10/10» 은 **64항목 중 10항목**이고, 그것을 맨 위 큰 막대로 놓으면 «거의 끝났다» 로 읽힌다. 실제로 그렇게 보고해 지적받았다.
- 현재 정본 숫자: A 6/6 · B 11/23 · C 2/10(dev, 다른 브랜치 10/10) · D 10/10 · E 0/7 · F 0/8 = **29/64 (45%)** · 병합 대기분 포함 **37/64 (58%)**.
- ⚠️ **여전히 세지 않는 것**(분모 밖): **`Principal` 없는 라우트 105개**(전체 381개 중 실측) — 일부는 의도적 공개일 수 있으나 **분류하지 않았다.** 분류 전에는 분모에 넣을 수 없고, 이 영역에서 권한 구멍을 여섯 번 연속 찾았으므로 규모가 작지 않다. 그 밖에 테스트 격리 없는 저장소 7개 · 가오픈 준비 · 결정 대기 3건.
- 검증: 신규 7건. `pytest tests/` **2,037 passed · 1 skipped · 실패 0 · exit 0**(기준선 2,030).
- 다음 행동 / 담당 / 착수 조건: **Claude Code** — ⓐ 무방비 라우트 105개 **분류**(분모를 확정해야 진척 숫자가 정직해진다) 또는 ⓑ 트랙 E 착수. ⚠️ E 의 착수 조건은 «데이터·권한 이관의 활성 파일 작업 종료 + Factory 변경 범위를 보드에 선언» 이다. ⚠️ **C 병합이 E·F 의 선행**이다 — 공통 셸이 다른 브랜치에 있다. **Supervisor** — 결정 대기 3건 그대로.
- 교대 체크포인트: 변경 = `core/enterprise_context/seed.py`(id 보존) · `tests/test_scope_seed_preserves_ids.py`(신규 7건) · `PROGRESS.md`(트랙 E·F 추가, 분모 64, 보고 규칙). 미변경 = 운영 데이터, 화면, 라우트. 검증 = 2,037 passed. **재개 지점 = 라우트 105개 분류 또는 트랙 E.** 금지 범위 = 시드가 `node_id` 를 재생성하도록 되돌리는 것, ECM 노드를 행 삭제로 폐지하는 것, 부분 트랙 진척을 전체로 보고하는 것, 분류하지 않은 항목을 분모에 넣는 것(추정이 된다), 트랙 E 를 «화면 이관» 으로 되돌려 C 에 합치는 것.

### [SCOPE-ALIAS-57] D-018 후속 7 — 코드 유일성 제약 + 변경 이력·별칭 보존
- 작성자 / 기록 시각: Claude Code / 2026-08-05 KST
- 왜 지금 기록하는가: `[SEC-SCOPE-51]` 후속 ⑦(코드 별칭·변경이력 + tenant·mode 유일성)을 구현했다. 종전에는 코드 중복이 **조회 시점에만** fail-closed 로 막혔고 **저장 시점 제약이 없었다** — 즉 같은 코드를 두 노드에 넣는 것 자체는 가능했다.
- 상태: **완료(후속 7) · 후속 8(저장 계층 fail-closed 완료)·10 남음**
- 무엇을 만들었는가:
  · 신규 표 `organization_node_code_aliases`(`node_id`·`code`·`tenant_id`·`replaced_by`·`reason`·`recorded_at`, `UNIQUE(node_id, code)`) + 인덱스 2개(`idx_node_code`·`idx_alias_code`). 멱등 DDL 이므로 기존 DB 에 자동 추가된다(실측: 운영 DB 에 생성 확인, 별칭 0건 = 개편이 없었으므로 정상).
  · `upsert_node` 가 **저장 시점에 유일성을 검증**하고, 코드가 바뀌면 **옛 코드를 별칭으로 남긴다.**
  · `find_nodes_by_code_alias` · `code_aliases_of` 신규. `resolve_scope_ref` 가 별칭 경로를 타고 `kind="ecm_code_alias"` + `requested_code` 를 준다.
- ★★ **DB `UNIQUE` 로 걸 수 없어 애플리케이션에서 검증했다.** 유일성 범위가 `(tenant_id, entity_mode, code)` 인데 `entity_mode` 는 **다른 표**(`enterprise_entities`)에 있다. 그래서 `upsert_node` 가 같은 문맥의 다른 노드를 찾아 거부한다 — 오류 메시지에 **충돌한 노드의 id·이름**을 담았다(그러지 않으면 어느 쪽을 고쳐야 하는지 모른다).
  ⚠️ 실측으로 기존 데이터가 제약을 이미 만족함을 확인했다(`(tenant, mode, code)` 중복 0건) — 그래서 회귀가 없었다. 만족하지 않았다면 시드·복제가 전부 막혔을 것이다.
- ★★★ **왜 옛 코드를 버리지 않는가.** `code` 는 조직 개편으로 바뀐다(`LS_MNM` → `LS_METALS`). 정본 `node_id` 는 그대로이므로 **내부 관계는 안전하지만**, 옛 코드로 저장된 외부 연계·문서·사람의 기억은 그 순간 끊긴다. ⚠️ **끊긴 참조는 조용하다** — 조회가 «없음» 을 돌려주고, 그것은 «권한이 없다» 와 구분되지 않는다. 그래서 옛 코드를 별칭으로 남겨 계속 해석되게 한다. 별칭으로 찾아도 **정본으로 정규화**되므로 판정은 언제나 `node_id` 를 탄다.
- ★★★ **현재 코드가 이긴다 — 순서가 통제다.** 별칭은 현재 코드로 **못 찾았을 때만** 본다. 반대로 하면 «옛 코드를 물려받은 새 조직» 대신 **옛 조직이 해석**되고, 그것은 권한을 과거로 되돌리는 일이다. 조직 개편에서 실제로 일어나는 형태이므로 `test_current_code_wins_over_an_alias` 로 잠갔다. 코드를 되돌리는 경우(`V1 → V2 → V1`, 개편 철회)도 함께 확인했다 — 그때 `V1` 은 현재 코드이므로 현재 경로가 이긴다.
- ★ 별칭이 여러 노드에 걸리면(코드 재사용) 현재 코드와 **같은 규칙**으로 거부한다(`code_alias_ambiguous`). 이력이 **쌓인다** — 두 번 바뀌면 중간 코드로 온 참조도 해석돼야 하므로 마지막 것만 남기지 않는다.
- 검증: 신규 `tests/test_scope_code_alias.py` **13건**. `pytest tests/` **2,030 passed · 1 skipped · 실패 0 · exit 0**(기준선 2,017). 유일성은 **네 축**으로 확인했다 — 같은 문맥 거부 / 다른 `entity_mode` 허용(가상 복제가 접두사 없이도 가능해야 한다) / 다른 tenant 허용 / **자기 자신은 충돌 아님**(그러지 않으면 이름만 바꾸는 갱신도 막힌다).
- 영향·주의사항: `upsert_node` 가 **거부할 수 있게 됐다** — 조직 편집 화면·시드 스크립트가 같은 코드를 두 번 넣으면 `EcmError` 가 난다. 지금 데이터는 위반이 없으므로 즉시 영향은 없다. ⚠️ 빈 코드는 유일성 대상이 아니다(여러 노드가 코드 없이 존재할 수 있다 — 빈 값은 «코드가 없다» 는 사실이지 중복이 아니다).
- 다음 행동 / 담당 / 착수 조건: **Claude Code** — 후속 ⑩(**백업·복원·시드에서 기존 `node_id` 보존 · 재생성 금지**)이 D-018 의 마지막 조각이다. `core/org_seed.py`·`clone_service`·백업 경로가 노드를 **다시 만들지 않는지** 확인해야 한다 — 재생성하면 백필한 정본이 전부 끊긴다(이번 세션에 격리 ECM 에서 그 형태를 실제로 겪었다: 새 id 를 만들자 `readable_scope_nodes` 와 매칭되지 않았다). **Supervisor** — 결정 요청 그대로(상위 조직 경영계획 상속).
- 교대 체크포인트: 변경 = `core/enterprise_context/repository.py`(별칭 표·유일성 검증·조회 3함수) · `resolver.py`(별칭 경로·`kind` 2종 추가) · `tests/test_scope_code_alias.py`(신규 13건). 미변경 = 운영 데이터(별칭 0건), 화면, 라우트. 검증 = 2,030 passed. **재개 지점 = D-018 후속 ⑩.** 금지 범위 = 별칭을 현재 코드보다 먼저 보는 것(권한이 과거로 되돌아간다), 별칭 이력에서 중간 코드를 지우는 것, 모호한 별칭에서 하나를 고르는 것, 빈 코드를 유일성 대상으로 넣는 것, 유일성 검증을 DB `UNIQUE` 로 바꾸려 하는 것(`entity_mode` 가 다른 표에 있어 불가능하다).
...
### [UIUX-MIG-27] 공통 셸 결함 6건 수정 + 이관 3/10 업무표준 (검토 대기)
- 상태: 검토 대기
- 책임 수행자 / 교차 검토자: Claude Code(완료) / **사용자(재승인 기준)·Codex(제품 영향)**
- 목표·완료 기준: 사용자 지적 6건을 **공통 셸에서** 수정한 뒤 이관 3/10(업무표준) 착수.
  3/10 을 먼저 붙이면 같은 결함을 한 화면 더 복제하게 되므로 순서를 고정했다.
- 변경 이유(왜 이 순서였나):
  ① 레일 아이콘이 한글 첫 글자 자동 생성이라 「받은 앱」과 「대내외 발간」이 **둘 다 «발»** 이었다.
  ② 720px 에서 마지막 레일 항목이 잘리고 **활성 항목이 스크롤 밖으로** 밀렸다.
  ③ 한글이 글자 단위로 끊겨 "확정값입니/다" 고아 줄바꿈이 카드마다 있었다.
  ④ Jarvis 로그가 250px 비어 «비서가 아무것도 하지 않는 자리»로 보였다.
  ⑤ 92vh + 16px 여백 때문에 뒤의 다크 상단 바가 12px 비쳤다.
  ⑥ "유형 14개와 레코드 0건" 배너가 전체 기준으로 읽혀 «숨겨진 자료 없음»으로 오해됐다.
- 결정/가정(계약 변경):
  · **정확한 숨김 건수는 DA·관리자에게만** 준다(`api.deps.hidden_envelope`). 가려졌다는 사실
    (`hidden_present`)은 누구에게나. 404 로 존재를 숨기면서 "옆 조직에 47건" 을 말하면 통제가
    앞뒤로 어긋난다. 프론트에서 가리지 않는다 — 응답에 숫자가 있으면 다른 클라이언트에 새어 나간다.
    `test_dept_scope_binding` 의 종전 기대(누구나 건수를 본다)를 새 계약으로 바꿨다.
  · `metricText` 기본 문구는 유지하고 수치가 아닌 칸만 자기 언어를 넘긴다(`MetricNotes`).
    «미측정과 0 은 다르다»는 판정은 지표 화면들의 단일 지점이라 바꾸면 함께 틀어진다.
  · 업무표준 `retired` 는 «구판»과 «폐지»를 겸한다(저장소 계약). 백엔드 값을 바꾸지 않고
    «더 높은 버전이 존재하는가»로 화면에서 구분한다.
- ★★ 이관 3/10 중 발견한 통제 구멍: **`standard_control.py` 의 모든 라우트에 권한 검사가
  없었다.** 익명이 `POST /standards` 와 `POST /standards/seed?force=true` 로 **에이전트의
  통과·반려 기준을 바꿀 수 있었다.** 지식팩 삭제 구멍보다 파급이 넓다(이후 모든 산출물의 판정이
  바뀐다). UI 이관보다 먼저 막았고 `WORK_STANDARD_CHANGED` 감사 이벤트를 추가했다.
- 검증 근거:
  · pytest **1783 passed / 실패 0**(신규 6건: 숨김 건수 계약 2 + 업무표준 권한 4) · `tsc` 통과
  · 실서버 8080 실측 — 업무표준: 익명 목록 0건+이유·상세/이력 403·쓰기 403 / 일반 8건 조회·쓰기
    403 / 관리자 8건 조회·쓰기 가능
  · 화면 **30개 전 항목 통과**(`scripts/capture_screens.py`, 1280×720·1440×900 × 15화면,
    관리자·일반·익명). 자동 검사 항목: 아이콘 중복·활성 항목 가시성·잘린 항목·고아 줄바꿈(글자
    단위)·Jarvis 빈 공간·Jarvis 근거 잘림·모달 바깥 여백·«0건» 표현·내부 슬러그 노출·12px 미만
  · **자동 통과 후 실제 캡처 육안 검토** — 그 과정에서 자동 검사가 못 잡은 결함 6건을 추가로
    찾아 고쳤다(익명에게 Jarvis 가 «할 수 있는 일» 을 말함, 내부 슬러그 2곳, 흰 배경에 흰 글씨로
    사라진 «(Esc)», 단계 수가 빨간 대기 배지로 보임, 정상 조회에 «가져오지 못했습니다» 동시 표시,
    익명에게 «조회만 가능합니다» 라는 거짓 안내)
- 다음 행동: 이관 4/10. 이후 이관은 아래를 **처음부터** 완료 조건으로 적용한다 —
  ① 사용자 용어만 노출(`design/terms.ts`, 사전에 없는 값은 콘솔 오류로 드러난다)
  ② 권한별로 실제 가능한 행동만 Jarvis 가 안내(`lib/actingScope.ts`)
  ③ 조회 실패·자료 없음·미지정을 각각 구분 ④ 1280×720·1440×900 시각 회귀
  ⑤ 자동 측정 통과 후 실제 캡처 육안 검토 ⑥ 이 게시판에 주체·이유·근거 기록
- 커밋: `f224ae685`(셸 6건) · `0512bd522`(문구) · `5e8e58583`(이관 3/10)

### [UIUX-MIG-28] 이관 4/10 조직·권한 — 익명이 전 직원 명부를 보고 있었다 (검토 대기)
- 상태: 검토 대기
- 책임 수행자 / 교차 검토자: Claude Code(완료) / **사용자(재승인 기준)·Codex(제품 영향)**
- 변경 이유(무엇이 잘못돼 있었나):
  ★★★ **읽기 라우트에 자격 검사가 하나도 없었다.** 익명 요청 하나로 전 직원 21명의
  **이름·이메일·소속 부서·관리자 여부**가 그대로 나왔다. 목록이 새는 것이 아니라 개인정보가
  새는 것이고, 그 조합은 조직 규모·인사 구조·권한 보유자를 한 번에 알려준다.
  (쓰기 경로는 `assert_can_edit_org` 로 이미 막혀 있었다 — 읽기만 비어 있었다.)
- 통제 결정:
  · 부서 트리·부서 목록은 **자격 있는 사용자에게 전체**를 준다. 조직도는 사내 공개 정보이고,
    범위로 자르면 상위가 사라져 트리 자체가 해석되지 않는다. 가릴 것은 **사람**이지 구조가 아니다.
  · 사용자 명부는 **읽을 수 있는 부서 + 자기 자신**으로 필터한다. 부서 미배정 계정은 보이지
    않는다 — 미배정을 «전사 공개»로 읽으면 이행 기간 계정이 전원에게 노출된다.
  · 정확한 숨김 인원수는 **조직 편집 권한자에게만**. 전 직원 수는 그 자체로 조직 규모다.
    `hidden_envelope(exact_for=...)` 로 «자료 종류마다 자격이 다르다»를 단일 지점에 넣었다 —
    DA 에게 전 직원 명부 규모가 새거나, 조직 관리자가 고칠 대상 건수를 못 보는 일이 없게.
  · 상세는 403 이다(404 아님). 부서·사람의 존재는 비밀이 아니고, 404 로 두면 «없는 부서»와
    «권한 없음»이 섞여 관리자가 원인을 못 찾는다.
- ★★ 통제가 부순 것을 함께 복구했다(중요):
  명부에 자격 검사를 넣자 **로그인 경로가 막혔다.** `UserSwitcher` 가 `users.length === 0` 으로
  스스로 사라져, 익명 사용자는 «우측 상단에서 사용자를 지정하십시오» 를 읽지만 지정할 수단이
  없었다 — 아무도 진입할 수 없는 상태. 계정을 **직접 입력**해 진입하는 길을 만들었다(형태가
  실제 로그인과 같고 명부를 노출하지 않는다). 목록은 «권한이 있을 때 편해지는 것»이다.
  또한 **401·403 을 서버 이상으로 세지 않게** 했다(`backendHealth`). 통제를 켜자 Jarvis 머리가
  «일시 중단»으로 바뀌고 "요청이 실패하고 있습니다" 가 떴다 — 사실이 아니고, 그렇게 두면
  진짜 장애가 왔을 때 그 표시를 아무도 믿지 않는다. 호출부 15곳에 상태 코드를 전달했다.
- 화면(완료 조건 ①~⑥ 적용): `prompt()` 6곳·`confirm()` 2곳·자체 `API_BASE_URL`(열 번째)·
  자체 모달·`asUser` 중복 입력창 제거. 특히 **조직 범위를 `prompt()` 로 받고 있던 것**을
  선택 목록 + 화면 안 입력으로 바꿨다 — 오타 하나로 부서 전체가 자료를 못 보는 값이었다.
  역할 편집은 사용자마다 12개 셀렉트를 그리던 것을 «한 사람 골라 편집»으로 바꿨다.
- 남은 사실(내가 정하지 않았다): 부서는 `LS_MNM` 계열 **scope code** 를 쓰고 ECM 트리는
  `node_*` 해시를 쓴다. 두 체계가 공존한다. 어느 쪽이 정본인지는 이 이관의 범위가 아니므로
  실제로 쓰이는 값을 제안 목록으로 주었다. **정본 결정이 필요하다.**
- 검증 근거:
  · pytest **1789 passed / 실패 0**(신규 6건: 명부 자격 3 + 범위 필터 1 + 권한자 전량 1 +
    자료 종류별 건수 자격 1) · `tsc` 통과
  · 실서버 8080 실측 — 익명 users/tree/departments **403** / 일반 사용자 21명 중 **2명**
    (`hidden_present=true`, 건수 없음) · 타 부서 상세 403 · 자기 상세 200 / 관리자 21명 전량
  · 화면 **42개 전 항목 통과**(21화면 × 1280×720·1440×900), 자동 통과 후 육안 검토
  · 육안으로 더 찾은 결함 4건: 목록 아바타가 빈 원(제목을 **NBSP** 로 들여썼다 — 눈에도
    안 보이고 `trim()` 으로도 안 잡히는 문자였다) / 부서 탭에 **동작하지 않는 검색창** /
    배너에 마크다운 `**` 가 그대로 노출 / 익명에게 Jarvis 가 «일시 중단» 이라고 거짓 보고
- 다음 행동: 사용자 지정 순서대로 **5/10 거버넌스 → 6/10 에이전트 통제소 → 7/10 프로젝트 관리**,
  모듈 구조가 안정된 뒤 **런처**. 이관마다 «읽기 라우트에 자격 검사가 있는가»를 먼저 확인한다 —
  3/10 과 4/10 에서 연속으로 같은 구멍이 나왔다.

### [UIUX-MIG-29] 이관 5/10 거버넌스 — 익명이 «취약점 지도»를 보고 있었다 (검토 대기)
- 상태: 검토 대기 · **이관 6/10 은 미착수**(사용량 한도로 대기)
- 책임 수행자 / 교차 검토자: Claude Code(완료) / **사용자(재승인 기준)·Codex(제품 영향)**
- 변경 이유:
  ★★★ 거버넌스 읽기 라우트가 익명에게 열려 있었다(3/10·4/10 과 **세 번째 연속** 같은 유형).
  실측으로 익명이 받아 본 것 — 위반 계약 1건과 사유(«생산자 자산이 폐기됐다»), 외부지표 6개의
  필요 등급·**격차 영향**·다음 행동(«물량 계획의 외부 근거가 없어 낙관 편향을 검증할 수단이
  없습니다»). 자료 본문이 아니라 **집계**라서 가볍게 보이지만, 거버넌스 콘솔은 정의상
  «무엇이 안 되어 있는가»를 모으는 화면이므로 집계 자체가 취약점 목록이다.
- 통제 결정: `api.deps.governance_block_reason` / `assert_governance_readable` 단일 지점.
  자격은 **데이터 관리자·조직 관리자·경영진**. 일반 사용자에게 막아도 업무가 멈추지 않는다
  («내 업무»가 아니라 «전사 정비 상태»다). 강제가 꺼져 있으면 아무것도 막지 않는다.
  ⚠️ 자산·계약·연계 시스템 **목록**은 이미 등급 기반 가림(§6-2 사용자 결정)이 있으므로
  403 으로 덮지 않았다 — 문서화된 결정을 조용히 지우지 않는다. 익명 차단만 더했다.
- 화면: 다섯 섹션을 한 페이지에 쌓던 구조를 셸 레일 다섯 항목으로. 자체 전체화면 → `HubDialog`.
  ★ 이 화면의 핵심은 **«0건»과 «못 봤다»의 구분**이다. 종전에는 403 에도 «후보 없음»·«활성 계약
  없음»을 표시해 «정비 완료»로 읽혔다. 이제 정비 완료 / 권한 없음 / 조회 실패를 각각 말하고,
  탭을 옮겨 다녀도 잊지 않도록 **확인하지 못한 항목을 화면 아래에 모아** 다시 밝힌다.
  `governanceApi.get()` 이 상태 코드를 버리던 것도 고쳤다(403 과 장애를 구분할 수 없었다).
- ★ 용어 사전 장치가 실제로 작동했다: 캡처 게이트가 사전에 없는 결손 종류 3개
  (`cost_exceeds_price` · `cpk_unreachable` · `uom_price_scale_suspect`)를 콘솔 오류로 잡아
  화면 통과를 막았다. 뜻을 생성 지점(`core/master_data_seed.py`)에서 확인해 사전에 옮겼다.
- 셸 결함 2건 동시 수정(다음 이관들이 물려받을 자리): `FoundationToolbar` 의 검색을 **선택적**
  으로(검색 없는 화면이 «동작하지 않는 빈 검색창»을 두던 것 — 4/10·5/10 에서 반복됐다) ·
  `.afs-banner b` → `> b`(본문 강조가 블록이 되어 줄이 끊겼다. `.afs-dialog-bar b` 와 같은 실수).
- 검증 근거: pytest **1797 passed / 실패 0**(신규 8건) · `tsc` 통과 ·
  실서버 8080 — 익명·일반 거버넌스 7경로 **403**, 관리자 200 / 등급 가림 목록 3경로는 익명만 차단 ·
  화면 **54개 전 항목 통과**(27화면 × 1280×720·1440×900) · 자동 통과 후 육안 검토
- 다음 행동(재개 시): **6/10 에이전트 통제소 → 7/10 프로젝트 관리 → 런처**(사용자 지정 순서).
  이관마다 «읽기 라우트에 자격 검사가 있는가»를 **먼저** 확인한다 — 세 번 연속 나왔다.
  미결: 부서 `LS_MNM` 계열 scope code 와 ECM `node_*` 해시의 **정본 결정**([UIUX-MIG-28] 참조).
- 커밋: `3ff5c1eed`

### [UIUX-MIG-30] 이관 6/10 에이전트 통제소 — 익명이 HOTL 중단점을 지울 수 있었다 (검토 대기)
- 상태: 검토 대기
- 책임 수행자 / 교차 검토자: Claude Code(완료) / **사용자(재승인 기준·사고 보고)·Codex(제품 영향)**
- 변경 이유:
  ★★★ **네 번째 연속** 같은 유형. 이번엔 읽기뿐 아니라 **쓰기까지** 열려 있었다.
  익명 요청으로 `PUT /factory/agents` 와 `POST /factory/agents/reset` 이 통했고,
  `POST /skills/proposals/{id}/approve`(에이전트 스킬 문서를 실제로 고침)도 통했다.
  이 구성은 «누가 무엇을 어떤 순서로 하는가»와 **HOTL 중단점**(전문가 개입 지점)을 정한다.
  중단점을 지우면 사람 확인 없이 파이프라인이 끝까지 흐른다 — 자료를 훔치지 않고 통제를
  무력화하는 경로다. LLM 을 호출하는 추천 라우트도 익명에게 열려 있어 예산 남용 경로였다.
- ★★★ **내가 낸 사고(반드시 검토해 주십시오)**: 권한을 확인하려고 `POST /agents/reset` 을
  그대로 호출했고, 그 엔드포인트는 `agents_registry.json` 을 **삭제**한다. 파일이 사라졌다.
  · 피해 확인: reset **직전** GET 이 돌려준 15개 = 기본값 15개(목록·순서 동일). 이 파일은
    git 에 한 번도 커밋된 적 없는 런타임 산출물이라 형상관리로 복원되지 않는다.
    삭제 전 내용이 기본값과 같았을 가능성이 높지만 **확정할 수 없다.** 편집해 두신 구성
    (모델·순서·HOTL)이 있었다면 잃었다 — 기억나는 변경이 있으면 알려 주십시오.
  · 원인: 파괴적 쓰기를 «차단 확인» 목적으로 부르면 **차단되지 않을 때 실제로 파괴된다.**
    앞선 검증 스크립트에 그 주의를 적어 뒀는데 이번에 지키지 않았다.
  · 재발 방지: ① `reset_registry()` 가 직전 상태를 `agents_registry.prev.json` 으로 남기고,
    **백업 실패 시 삭제하지 않는다**(복구 수단 없는 삭제는 하지 않는다). ② `POST /agents/restore`
    로 한 번은 되돌릴 수 있다. ③ 테스트가 이 계약을 지킨다(`tests/test_agent_config_gate.py`).
    ④ 앞으로 파괴적 엔드포인트는 **차단 확인만** 하고, 권한자 경로는 대역으로 고정한다.
- 통제 결정: 읽기(구성·템플릿·스킬 제안)는 등록 사용자, 쓰기(저장·초기화·복원·템플릿 생성·삭제·
  스킬 승인/반려)는 데이터 표준 관리자·관리자. LLM 추천은 익명만 차단(등록 사용자는 허용 —
  설계 보조 도구다). 구성 변경·스킬 승인은 `WORK_STANDARD_CHANGED` 로 감사에 남는다.
- 화면: 런처를 덮던 자체 전체화면 → `HubDialog` + 3탭(에이전트 / 실행 흐름 / 워크플로우 템플릿).
  `alert()` 4곳·`confirm()` 4곳·`prompt()` 2곳 제거(특히 **새 템플릿 식별자를 `prompt()` 로**
  받고 있었다 — 형식 오류를 잡아 줄 자리가 없었다). store 가 403 을 삼켜 «템플릿 없음»으로
  보이던 것도 고쳤다. 「사람 확인 지점」은 0 을 숨기지 않는다 — 0곳은 «통제 없음»이라는 사실이다.
  ★ 범위: 상세 편집 패널(`AgentFlow/AgentDetailSidebar`, 12개 필드·231줄)은 **그대로 재사용**
  했다. 다시 만들면 필드 누락 위험이 크다. 아직 다크 스타일이며 별도 단계로 옮긴다.
  (재승인 기준을 지키려고 그 컴포넌트의 12px 미만 글자만 올렸다 — 기능은 건드리지 않았다.)
- 육안 검토에서 더 찾은 결함 3건: **권한 잔상** — 익명으로 바꿨는데 «403 으로 못 읽었다» 배너와
  **이전 사용자의 에이전트 목록**이 같은 화면에 함께 떴다(store 가 오류만 담고 데이터를 비우지
  않았다) / 내부 단계 코드 `CLARIFICATION`·`UI_DESIGN`·모델 등급 `pro` 노출 → 사전 확장
  (업무표준은 8단계, 레지스트리는 15단계였다 — 사전을 합쳤다) / 레일 항목이 3줄로 늘어짐.
- 검증 근거: pytest **1808 passed / 실패 0**(신규 11건) · `tsc` 통과 ·
  실서버 8080 — 익명 읽기·쓰기 전부 **403** / 일반 사용자 읽기 200·쓰기 403 / 관리자 편집 가능 ·
  화면 **66개 전 항목 통과**(33화면 × 1280×720·1440×900) · 자동 통과 후 육안 검토
- 보고할 기존 결함(내가 만든 것 아님): `App.tsx` 에 모달 렌더 목록이 **두 벌** 있다(프로젝트
  없음 / 있음 화면). 한쪽에만 추가하면 특정 상태에서만 안 열린다 — 정리 대상.
- 다음 행동: **7/10 프로젝트 관리 → 런처**(사용자 지정 순서). 이관마다 «읽기·쓰기 라우트에
  자격 검사가 있는가»를 먼저 확인한다 — 3·4·5·6 에서 네 번 연속 나왔다.
- 커밋: `c592b5ef8`

### [UIUX-MIG-31] 이관 7/10 프로젝트 관리 + 런처 — HOTL 관문 우회 경로 차단 (검토 대기)
- 상태: 검토 대기
- 책임 수행자 / 교차 검토자: Claude Code(완료) / **사용자(재승인 기준)·Codex(제품 영향)**
- ★★★ 통제(커밋 `080cd119a`): **다섯 번째 연속** 같은 유형. `Principal` 조차 없는 라우트 8개가
  전부 «실행을 바꾸는» 쓰기였고, 그중 `POST /{id}/hotl/resume` 는 **HOTL 중단점을 통과**시킨다.
  6/10 에서 «중단점을 지우려면 관리자 권한»을 막았는데 **지울 필요 없이 넘길 수 있었다** —
  통제 하나를 다른 경로가 무력화하는 전형이다.
  · 판정 함수(`assert_project_readable/writable`) **자체**에 관문 A 를 세웠다. 두 함수는
    «소유권 미기록 프로젝트는 통과»라는 하위호환을 갖고 있었고 그 관대함이 익명까지 열려 있었다.
    판정 안에 두면 이 함수를 쓰는 모든 경로(스프린트·삭제·복제·릴리스·재시뮬레이션)가 함께 보호된다.
  · `GET /projects` 익명 차단(0건+이유), `POST /projects`·`/projects/mega` 익명 차단 —
    익명이 만들면 소유 부서가 비어 프롬프트 주입 필터가 걸리지 않는다(그 함수 주석이 경고한 경로).
  · **기록해 둔 한계**: 소유권 미기록 프로젝트는 식별된 사용자에게 여전히 열려 있다(하위호환).
    → **소유권 미기록 프로젝트 수를 상시 관측해야 한다.** 0 이 되면 걷어낸다. 테스트가 이 계약을
      명시적으로 고정하므로 조용히 바뀌면 깨진다.
- ★ 범위 결정(사용자 승인): 프로젝트 관리 UI 가 런처(App.tsx 856줄) **안에** 있어 둘을 나눌 실익이
  없다 → **런처까지 함께 이관**. 그리고 미이관 10개 화면도 **전부 이관**하기로 확정했다.
- 런처 이관 방식(판단 기록):
  런처는 모달이 아니라 **페이지 자체**라 3열 셸을 씌우지 않고, 기존 마크업의 다크 Tailwind 클래스를
  **의미 토큰 유틸리티**(`afs-bg-*` · `afs-ink` · `afs-action` 등)로 옮겼다.
  ⚠️ 전역 반전(`@theme` 숫자 스케일 뒤집기)은 쓰지 않았다 — Codex 가 `51982c898` 에서 철회한 이유가
    그대로 유효하다(숫자는 명도만 말하고 의미를 말하지 않는다). 대신 **한 클래스씩 뜻을 정해** 옮겼고,
    그 매핑 표가 곧 판단 기록이다(3라운드 86종 → 6종만 남김: 작은 상태 점과 focus 링).
    `!important` 를 쓰지 않고 Tailwind 클래스를 **지우고** 넣었다 — 우선순위 싸움을 남기지 않는다.
  ⚠️ 상단 바는 **구조색(남색) 위**라서 본문 유틸리티가 그대로 들어오면 어두운 글자가 어두운 바에
    얹혀 **글자가 사라진다**(실측: 제목 «Private AI Cockpit» 이 보이지 않았다). 바 안에서는 같은
    유틸리티를 «바 위에서의 뜻»으로 다시 해석하도록 CSS 를 한 곳에 두었다.
- 검증: pytest **1816 passed / 실패 0**(신규 8건, 판정 함수를 건드렸으므로 전체 회귀 확인) ·
  `tsc` 통과 · 실서버 8080 익명 실행 경로 8개 전부 403 · 런처 캡처 육안 확인
- 커밋: `080cd119a`(통제) · `7c60dcf80`(런처·8/10·9/10)

### [UIUX-MIG-32] 이관 8·9/10 스킬 개선안·전사 브리핑 — 요약 화면이 통제를 우회했다 (검토 대기)
- 상태: 검토 대기
- 책임 수행자 / 교차 검토자: Claude Code(완료) / **사용자(재승인 기준)·Codex(제품 영향)**
- ★★★ **여섯 번째** 구멍이고 종류가 달랐다: 브리핑이 **5/10 에서 막은 자료를 다시 모아** 익명에게
  보여 줬다(«데이터 계약 breached — 생산자 자산이 폐기됐다» · «스킬 제안 검토 대기 18건»).
  ⚠️ 여러 소스를 모으는 화면은 각 소스의 통제를 **모으는 쪽에서 다시** 판정해야 한다.
  → 브리핑에 관문 A + 거버넌스 성격 섹션(`data_health`·`cost`)을 `withheld` 로 표시해 보낸다.
    **빈 배열로 조용히 비우지 않는다** — «해당 항목 없음»으로 읽히면 5/10 에서 고친 오독이
    되돌아온다. 부분 갱신 경로(`/sections/{name}`)도 같은 판정을 받는다.
- 스킬 개선안: `if (res.ok)` 만 처리하고 else 를 버려 **403 이어도 «제안이 없습니다»** 로 보였다.
  승인 대기열에서 «없다»와 «못 봤다»를 섞으면 관문이 조용히 열린 것과 같다.
  승인이 **영구 반영**임을(되돌리는 화면이 없다는 사실까지) 확인 문구에 명시했다.
- 셸 결함 1건: `.module-menu` 에 `align-content: start` — 항목이 적으면 버튼이 세로로 늘어났다.
- 검증: pytest **1821 passed / 실패 0**(신규 5건) · `tsc` 통과 · 화면 **84개 전 항목 통과** ·
  실서버 실측(익명 403 / 일반은 `my_decisions` 200·`data_health` 403+`withheld` / 관리자 전량)
- 커밋: `7c60dcf80`

### [UIUX-HANDOFF-33] 세션 교대 기록 (참고)
- 상세 인수인계: `docs/chronicle/handoffs/handoff_2026-08-04_uiux_migration_3to9.md`
- `AI_HANDOFF.md` 최상단을 갱신했다 — 이전의 «디자인 미적용 · 스크린샷 못 봄» 경고는 해소됐다.
- ★ 다음 사람이 먼저 할 일: **Planning 경영 계획 통제**(무방비 라우트 13개. 지금은 데이터가
  0건이라 실제 유출은 없지만 계획이 들어오면 새어 나간다) → 그 다음 작은 화면부터 이관.
- ★ 사용자 결정이 필요한 것 3건: 조직 범위 **정본**(scope code vs `node_*` 해시) ·
  **경영 계획 열람 자격** · **소유권 미기록 프로젝트** 관대함을 언제 걷어낼지(상시 관측 필요).
- ⚠️ 내가 낸 사고 2건을 인수인계서 §5.3 에 적었다: 파괴적 엔드포인트를 «차단 확인» 목적으로
  호출해 `agents_registry.json` 을 지웠고, LLM 프로브가 `skills/a_skill.md` 를 만들었다.
  재발 방지를 코드(백업·복원)와 규칙(존재하지 않는 id·무효 payload 로만 찔러 본다)로 남겼다.
...
| 샌드박스 자료를 자산 사용 집계에서 제외 | ✅ 61개 중 **59개 제외**(관측 2→1) |
| `emit_to` 테넌트 경계 | ✅ 다른 테넌트 창으로는 안 감(근거 없으면 대조 안 함) |
| `emit_to` 「현재도 안건 참여자인가」 | ✅ **구조적으로 충족** — 아래 근거 |
...
#### 「현재도 안건 참여자인가」에 대한 확인 (2026-08-12)

코드를 고치지 않고 **읽어서 확인**했다. 세 발신 경로가 모두 «방금 읽은 레코드»를 넘긴다.

    core/decision_case.py:713  _notify(event, case, actor)
      ← 호출부 6곳 전부 `out = self.get(decision_id, ...)` 직후(425·456·493·538·586·618행)
    core/app_delivery.py:514   delivery_recipients(delivery)
    core/publication.py:768    publication_recipients(pub)

즉 참여자 목록은 **발신 시점에 다시 계산**되고, `emit_to` 는 그 자리에서 큐에 넣는다(지연 없음).
참여자에서 빠진 사람은 다음 알림부터 목록에 없다. 따로 재검증 계층을 만들면 같은 뜻을 두 곳에서
읽게 되고, 그 둘이 갈라지는 날 «목록보다 알림이 넓은» 상태가 된다.

⚠️ 대신 시간이 지나며 달라지는 두 가지는 **보낼 때마다 다시 본다** — 계정 유효성과 테넌트.

#### 샌드박스 집계 제외의 실제 효과

`core/asset_project_usage.py` 에서 59개를 뺐지만, 그중 사용 스냅샷이 있던 것은 **1개**다
(관측 2 → 1). 지금 데이터에서 수치 변화는 작다 — 그러나 이 집계는 「이 자산을 쓰는 프로젝트가
몇 개인가」로 **자산 폐기·전사 승격을 정하는** 근거이고, 시험 산출물이 섞이면 아무도 안 쓰는
자산이 «사용 중» 으로 보여 폐기가 막힌다. 효과가 작은 지금 넣어 두는 것이 맞다.

⚠️ 경영 브리핑·승격 게이트는 프로젝트 디렉터리가 아니라 **릴리스·승격 DB 행**을 읽고, 이미
  `scope_node_id`·`tenant_id` 필터가 있다(`enterprise_briefing._visible`). 다만 **이번
  마이그레이션 이전에 만들어진 릴리스 행**이 어느 범위로 기록돼 있는지는 확인하지 않았다 —
  남은 확인 항목으로 남긴다.
