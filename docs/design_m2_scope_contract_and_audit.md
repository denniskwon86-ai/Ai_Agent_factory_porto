...
| # | 작업 | 여는 관문 | 선행 | 되돌림 | 규모 |
|---|---|---|---|---|---|
| ~~1~~ | ~~`audit.py` 신설 + 거부 지점 배선~~ **✅ 완료(2026-07-29)** | B-2 ✅ | 없음 | 쉬움 | 소 |
| ~~2~~ | ~~MCP 에 `current_principal` + 서버측 범위 교차 검증~~ **✅ 완료** — `core/scope_guard.py` | B-4 ✅ | 1 | 쉬움 | 중 |
| ~~3~~ | ~~거부 응답 409 → 404~~ **✅ 완료** — §3.3 경계표대로 "등록되지 않은 시스템"만 404, 상태 충돌은 409 유지 | B-1 ✅ | 1·2 | 쉬움 | 소 |
