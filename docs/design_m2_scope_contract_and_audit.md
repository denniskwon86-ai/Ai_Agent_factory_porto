...
| # | 작업 | 관문 | 되돌림 |
|---|---|---|---|
| 1 | `audit.py` 신설 + 거부 지점 배선 | B-2 | 쉬움(부가 기록) |
| 2 | MCP·크로스워크에 `current_principal` + 교차 검증 | B-4 | 쉬움(인자 추가) |
| 3 | 거부 응답 409 → 404 (§3.3 경계 준수) | B-1 | 쉬움 |
| 4 | `scope_type` 등 4필드 마이그레이션 + `LEGACY_UNSCOPED` 표시 | A-3·A-4 | **어려움 — 데이터 변경** |
| 5 | `is_visible` / `_in_scope` 기본값을 비노출로 전환 | A-1·A-2 | 중간(플래그로 단계 전환) |
| 6 | `bc3b8bbe9`(보류)를 새 계약으로 보정해 해제 | — | — |

> **4번 전에 반드시 백업한다.** 데이터 변경이 유일하게 되돌리기 어려운 단계다.
> 5번은 환경변수 플래그(`SCOPE_FAIL_CLOSED`)로 감싸 단계적으로 켠다 — 한 번에 전환하면
> 무엇이 안 보이게 됐는지 아무도 모른다.
