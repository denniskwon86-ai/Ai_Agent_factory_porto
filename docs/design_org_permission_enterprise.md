...
> **ECM 확장 결정(2026-07-28):** 기존 부서 트리와 권한 설계를 기업집단·법인·사업부·사업장·공장·공유서비스를 포괄하는 다중 관계 Enterprise Context Master로 확장한다. 단일 트리는 화면 기본 보기일 뿐이며, 권한은 `주체 × 조직 범위 × 도메인 × 행동 × 실제/가상/경쟁사 상태`로 판정한다. 가상 조직 복제와 경쟁사 모델의 격리 규칙은 [`design_enterprise_context_master.md`](design_enterprise_context_master.md)를 우선한다.
...
## Phase 1 — 조직·사용자·권한 (부서 = 기준정보) ✅ **완료 (2026-07-27)**

> 구현: `core/org_directory.py` · `core/org_seed.py` · `api/routes/org_control.py` ·
> `frontend/src/components/OrgChartPanel.tsx` · 테스트 31건 (전체 362건 통과)
>
> **설계 대비 변경 1건 — 부트스트랩 잠금 방지**
> 설계는 "`departments` 가 비면 unrestricted" 만 규정했다. 그런데 실호출 검증에서
> **시드로 부서가 생기는 순간 모두가 권한을 잃고, 첫 관리자를 만들 권한을 가진 사람이
> 아무도 없어 시스템이 잠기는** 것을 확인했다(실측 403).
> → `is_bootstrap()` = 부서가 없거나 **사용자가 0명**이면 무제한. 첫 사용자 등록부터 강제한다.
> 아울러 `ORG_ENFORCE`(기본 False)를 단계적 도입 안전판으로 반영했다.
>
> **실측 검증**: 부트스트랩 통과 → 첫 관리자 생성 → 순환 이동 400 거부 →
> 강제 시 `bob`=영업 읽기·쓰기 / `kim`=품질 읽기만(viewer) / `ghost`=권한 없음,
> 소유권 필터가 부서별로 분리됨.
