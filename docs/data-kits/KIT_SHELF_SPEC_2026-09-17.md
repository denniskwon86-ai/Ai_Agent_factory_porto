...
- 진행: **1~7 전부 완료** (2026-09-21). 선반 상태는 `python scripts/kit_shelf.py` 가 답한다
...
### 4.1 결과 (2026-09-21)
    ...
    AFS_KITS_DIR           없으면 <PROJECT_ROOT>/docs/data-kits
    AFS_STARTER_KITS_DIR   없으면 <PROJECT_ROOT>/starter_kits

고친 곳 셋 — `kits_dir()` · `starter_package_catalog()` · `demo_vertical_slice.kit_root()`.

| 결정 | 왜 |
|---|---|
| **상대 경로는 `PROJECT_ROOT` 기준** | cwd 기준이면 systemd 로 돌 때와 손으로 돌릴 때가 달라져 **「키트가 없다」가 조용히 나온다** |
| 빈 값(`AFS_KITS_DIR=`)은 미설정으로 | 배포 스크립트가 흔히 그렇게 넘긴다 |

시험 7 건 (`tests/test_kit_registry_paths.py`) — 기본값이 **지금과 같다**는 것,
읽는 쪽 셋이 **모두 같은 자리**를 본다는 것을 지킨다.
...
| | 무엇 | 되돌리기 |
|---|---|---|
| ✅ **1** | `BusinessDef` 에 셋 추가 + 두 사업에 값 (2026-09-21) | 커밋 되돌리기 |
| ✅ **2** | 생성기가 그 값을 쓰게 (3 장) | 〃 |
| ✅ **3** | 시험 **6 건** (6 장) — **여기서 멈췄다** | 〃 |
| ✅ **4** | 제련 키트 생성 → 검증 **420 PASS** → 봉인 (2026-09-21) | 디렉터리 삭제 |
| ✅ **5** | 전지소재 키트 생성 → 검증 **420 PASS** → 봉인 (2026-09-21) | 〃 |
| ✅ **6** | 경로 설정화 (4 장) + 시험 **7 건** (2026-09-21) | 커밋 되돌리기 |
| ✅ **7** | 선반 카탈로그 — **문서가 아니라 도구**로 (2026-09-21) | 파일 삭제 |
...
### 1~3 결과 (2026-09-21)
    ...
    ①  기본 조합    KIT-MFG-NONFERROUS-PROCUREMENT   AFS 데모소재그룹
    ②  제련만       KIT-MFG-SMELTING-NONFERROUS      AFS 메탈 주식회사
    ③  전지소재만   KIT-MFG-BATTERY-MATERIALS        AFS 첨단소재 주식회사

| | |
|---|---|
| **데이터 불변** | 기본 조합이 정본 1.2.0 과 **한 파일도 다르지 않다**(manifest 제외) |
| 하위호환 | 기본 조합은 기존 `kit_id` 를 그대로 쓴다 — 1.0.0~1.2.0 이 그 이름으로 봉인돼 있다 |
| 다른 조합 | **거부한다** — 순서만 바꿔도 `--kit-id`·`--kit-name` 을 요구한다 |
| `industry_codes` | 제거됐다. 자리에 `sector` 가 들어갔다 |
| 시험 | `test_business_defs` **27 건** 통과 |

★ **다음은 4·5(새 키트를 실제로 봉인)이고 결정이 필요하다**(9 장 ③).

### 7 번 결과 — 카탈로그는 문서가 아니라 도구다 (2026-09-21)

손으로 쓴 표는 낡는다. 이 저장소에서 여러 번 겪었다 — 「사업 하나 50 줄」이 실제로는
30 줄이었고, 존재하지 않는 `wire_cable.py` 가 예시에 남아 있었다. 그래서 **씨앗과
선반을 대조해 내는 도구**로 만들었다.
...
| 한쪽에만 있으면 | 뜻 |
|---|---|
| **씨앗만** (`business_defs/*.py`) | 뽑아서 봉인하면 줄 수 있다 — 명령 세 개 |
| **선반만** (`starter_kits/`) | 어느 사업에서 나왔는지 **모른다.** 재현할 수 없다 |

지금 상태를 그대로 비춘다.

    씨앗 3 · 그중 선반에 1          ← 사업 단독 둘이 아직 없다
    선반 키트 2 · 사업 정의가 없는 것 1  ← KIT-MFG-BATTERY-CHEMICAL-PROCUREMENT

★ `kit_name` 이 없는 판본은 **「AFS 데모소재그룹 ← 회사명」**으로 적는다. 카탈로그가
  회사명을 키트 이름으로 쓰고 있다는 사실(D4)이 눈에 보이게 하기 위해서다.

시험 13 건 (`tests/test_kit_shelf.py`) — 사업을 빠뜨리지 않는가 · **조합도 씨앗인가**
(빼면 조합 키트가 고아로 잡힌다) · 구조가 다르거나 깨진 manifest 에 죽지 않는가 ·
**A2 로 연 경로를 따르는가** · **선반의 확정 판본이 모두 온전한가**(키트가 늘어도
자동으로 함께 본다).

### 4·5 결과 — 선반이 찼다 (2026-09-21)

| 키트 | 판본 | 행 | 검증 | 봉인 |
|---|---|---|---|---|
| `KIT-MFG-SMELTING-NONFERROUS` | 1.2.0 | 123,817 | **420 PASS** | 140 파일 |
| `KIT-MFG-BATTERY-MATERIALS` | 1.2.0 | 131,740 | **420 PASS** | 140 파일 |

    씨앗 3 · 그중 선반에 3     ← 전부 찼다
    선반 키트 4 · 사업 정의가 없는 것 1  ← KIT-MFG-BATTERY-CHEMICAL-PROCUREMENT 만 남았다

| 확인 | 결과 |
|---|---|
| 남의 사업 흔적 | **0 행** (제련에 황산니켈 · 전지소재에 동정광 · 남의 공장·창고) |
| 재고 음수 | **0 행** |
| 키트 이름 | 카탈로그에 **제 이름**이 뜬다 — `← 회사명` 이 아니다 |
| 용량 | 119 MB → **189 MB** (+70 MB, 예상 100 MB 보다 적다) |

⚠️ 검증기에 **`--kit` 을 더해야 했다.** `KIT_ID` 가 상수라 새 키트를 볼 수 없었다 —
  키트가 하나뿐일 때 쓴 하드코딩이다. 경로도 `starter_packages_dir()` 를 쓰게 해
  **A2 로 연 자리를 검증기도 따르게** 했다(한 곳만 고치면 키트를 옮겼을 때 검증기만
  옛 자리를 본다).
