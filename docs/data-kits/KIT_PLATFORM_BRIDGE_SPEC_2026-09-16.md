...
#### 왜 넘어가지 않나 — 플랫폼 쪽 제약이다

`expand_driver_assumption(code, pct_change)` 가 **비율 변화만** 받는다. 「48 시간
정지」가 몇 퍼센트인지는 그 설비의 가동 시간이 있어야 나오고, 「1,800 억 투자」는
애초에 비율이 아니다.

이것은 이음매가 풀 수 있는 문제가 아니다 — **플랫폼의 시나리오 모델이 절대량을
받지 못한다.** 설계 7 장 ⑤로 넘기되, 이 문서는 **40% 라는 크기**를 기록해 둔다.
...
★ 그 기준값은 **회사 데이터**다(설계 2.0 의 오른쪽 칸). 즉 이 시나리오는 현업이
...
| | 왜 |
|---|---|
| **시나리오 10 개 중 4 개가 안 넘어간다**(2.1) | 플랫폼이 **비율 변화만** 받는다. 설비 정지·투자·납기 지연처럼 **경영 판단에 가장 가까운 것들**이 걸린다. 이음매로는 못 푼다 |
