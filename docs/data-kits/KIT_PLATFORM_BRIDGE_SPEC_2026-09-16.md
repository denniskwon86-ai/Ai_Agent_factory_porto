...
### 5.0 ✅ 1~4 구현 완료 (2026-09-21) — `scripts/kit_bridge.py`

**별도 모듈로 냈다.** 명세는 `seed_starter_data.py` 에 붙이라 했지만, 「누가
구현하나」가 아직 정해지지 않았고(6 장) 그 파일은 **다른 세션 영역**에 가깝다.
순수하게 옮기기만 하는 모듈로 두면 **연결은 언제든 한 줄**이다.

| 실제로 돌려 본 결과 (세 키트 모두 같다) | |
|---|---|
| 동인 | 심음 **11** · 뺌 **1**(`DRV-GM`) |
| 계정 | 심음 **76** · 건너뜀 **4**(전부 `BALANCE_SHEET`) · 실패 0 |
| 시나리오 | 10 개 중 **4 개가 막힌다**(`DAY`·`HOUR`·`KRW`·`PERCENT_POINT`) |

⚠️ 명세 3.1 의 「30 건 중 26 건」은 **quick 프로필** 기준이었다. full 은 80 건 중
  76 건이다 — 시험은 프로필에 기대지 않고 **`BALANCE_SHEET` 4 건이 전부 건너뛰어
  진다**로 잠갔다.

### 5.1 ★★★ 구현해 보니 드러난 것 — **옮길 산업 특성이 없다**

제련 · 전지소재 · 조합 — **셋이 완전히 같은 동인 11 개**를 준다.

    환율 · 원자재가격 · 운임 · 납기지연 · 공급 · 수율 · 설비정지
    전력단가 · 수요 · 투자 · 대금시점

제련수수료(TC/RC)도 금속 회수율도 부산물 크레딧도 **없다.** 이음매는 제대로 도는데
**지나갈 것이 범용 제조업의 것뿐**이다.

★ 그래서 도메인 검토 1 번(제련 드라이버 다섯)이 **이 작업 전체에서 가장 급하다**
  (`docs/decisions/DOMAIN_REVIEW_KIT_2026-09-21.md`). 시험
  `test_사업이_달라도_같은_동인만_나온다` 가 **깨지는 날**이 그 답이 반영된 날이다.

### 5.2 남은 순서

1. ~~`to_pct_change()` + 시험 1·2~~ ✅
2. ~~대응표를 데이터로 (`_DRIVER_MAP`) + 시험 4·6~~ ✅
3. ~~`seed_plan_drivers()` + 시험 3·5·8~~ ✅
4. ~~`seed_plan_accounts()` + 시험 7~~ ✅
5. **`seed_starter_data.py` 에 연결** — ⏸ 「누가 구현하나」(6 장)와 빈 설치본 결함
   확인이 선행이다
6. 시나리오 — 계수가 쌓인 뒤 (3.3)
