...
2-1. **별칭은 매칭 가능한 형태로 파생한다** — 문서의 정식명 `"Mixed Hydroxide Precipitate (MHP)"` 를 그대로 별칭에 넣으면 단어경계 매칭에 영원히 걸리지 않아 **별칭 히트 경로 전체가 죽는다**(2026-07-29 실측). `core/master_data_seed.derive_aliases()` 가 괄호 앞 본문·괄호 안 토큰·문서상 식별자로 분해한다. 새 문자열을 창작하지 않고 문서에 이미 적힌 부분만 쓴다.
3. **도메인 일치 레코드 전량**: 조직 범위를 통과하고 도메인이 일치하는 레코드는 **전부** 대상이다. `is_core` 는 **관문이 아니라 정렬 신호**다(종전에는 관문이어서 비핵심 레코드가 도메인이 맞아도 영원히 주입되지 않았다).
4. **전수 주입 · 표시 순서만 우선순위** *(2026-07-29 개정 — 종전 「합집합 상한 12건 / 3,000자」 폐기)*
   - **상한 없음이 기본.** 적용 가능한 것은 전부 넣는다. 근거: 12건 상한에서 배터리소재는 적용 가능 30건 중 12건만 주입되고 **잘린 18건에 표준원가 산식·MPS·라우팅·배출계수·시뮬 확률분포가 전부** 포함됐다 — 「LLM 이 산식을 지어내지 못하게 확정 주입한다」는 M1 의 존재 이유가 무력화됐다. 전량은 배터리소재 30건 ≈ 6,000자 / 전 문서 44건 ≈ 8,700자로 부담이 없다.
   - **표시 순서**: 별칭 히트 → **전사 표준·산식**(`kpi`/`finance-param`/`work-center`/`logistics-param`/`emission-factor`/`sim-param`/`sensor-spec`) → 도메인 핵심 → 나머지, 각 계층 내 `master_code ASC`. 종전 tie-break 가 코드 알파벳순뿐이어서 **무엇이 버려지는지가 중요도가 아니라 철자로 결정됐다**(`RM-`/`WIP-` 항상 탈락).
   - **상한을 걸 수 있는 경우**: 환경변수 `MASTER_INJECT_MAX_ITEMS`/`MASTER_INJECT_MAX_CHARS`(운영 비상시) 또는 호출자의 `max_items`/`max_chars` 인자(미리보기 UI). 상한에 걸리면 `break` 가 아니라 `continue` 로 처리해 **긴 레코드 하나가 뒤를 전부 잘라내지 않으며**, 잘린 건수를 주입 블록에 명시한다 — 조용히 잘리는 것이 가장 위험하다.
   - active + 현행 버전 한정.
