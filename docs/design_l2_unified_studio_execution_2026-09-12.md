...
> **실행 갱신 2026-09-13:** 위 표제의 원격 대기/미착수는 설계 종결 당시 상태다. 이후 사용자 승인으로 두 브랜치 push/원격 확인 완료, B0-A 구현·독립 코드검토 및 B0-B 원자성 부분 구현을 수행했다. 전체 B0는 미완료다. 최신 상태와 검증은 `docs/handoff/L2_STUDIO_B0_IMPLEMENTATION_2026-09-13.md` 및 PROGRESS 최상단을 따른다. 설계 본문을 구현 완료로 일괄 승격하지 않는다.

> **후속 실행 2026-09-13 01:01 KST:** 위 실행 갱신 이후 B0-B의 권한/불변 대상/멱등을 구현해 지정 회귀382 통과·독립 정적 지적 해소로 B0-A/B 출구를 완료했다. 다음 B1 착수 가능. 최신 근거는 `docs/handoff/L2_STUDIO_B0B_CERTIFICATION_2026-09-13.md`. B1~B7/전체 T3/브라우저/운영 회사 인증을 완료로 승격하지 않는다. 전체21/40=52.5% 유지.

> **후속 실행 2026-09-13 09:42 KST:** B0~B3 지정 코드 출구 완료. 최종 B0/B1/B2/B3 통합1923PASS/6Windows symlink권한skip/실패0, 격리·소스보존 및 Hilbert 독립 증거대조PASS. 최신 인계는 `docs/handoff/L2_STUDIO_B3_CONTEXT_2026-09-13.md`와 PROGRESS다. B4~B7·브라우저·현업수용은 미완료이며 전체21/40=52.5% 유지. B4는 §11 연결 보완을 포함해150~240분 예상, 아직 구현하지 않았다.
...
### 7.4 B5 실제 구현 경계 — 2026-09-13 18:57 KST

> **후속 2026-09-13 23:03 KST:** 6종 실행 UUID/영속접수/같은키GET·UNKNOWN차단, 실제worker종료, 같은PLANNING·수동정지task 비파괴재개·상태투영, HEAL 원실패/고정새task·서버한도·다른현재task HOTL우선을 구현·집중검증했다. 서버260PASS/프런트127PASS/제품·fixturebuild PASS, 독립P2두건정적종결. 아래 과거 ‘일반명령영수증·같은PLANNING 미구현’은 이 범위에서 해소됐다. 일반HOTL초안소비/구release·replan영수증없는UNKNOWN/B5잔여판정/B6/B7는남는다. 전체21/40=52.5%유지. 상세계약·한계·근거는 PROGRESS와B5인계 최상단을 따른다.

> **후속 2026-09-13 22:04 KST:** 이 절 아래 원문은 당시 경계다. Kit v2 검토 묶음은20:44, 수정 요청의저장초안/산출물/UUID결속·멱등접수·같은WBS파일원자교체·실제영수증소비는22:04에구현·집중검증했다. 산출물조회실패와독립된원키GET/다건실패순회도연결했다. 분산DB/FS원자성은아니며 일반HOTL소비/일반명령UNKNOWN/같은PLANNING비파괴재개/B6/B7는여전히잔여. 전체21/40=52.5%유지,최신판정·시험근거는PROGRESS와B5인계최상단을따른다.

공통 StudioContent/대표 행동, 요구·결정·명확화·수정 초안 저장과 대상별 입력 보호를 연결했다. 전체 완료 판정/정본은 PROGRESS와 `docs/handoff/L2_STUDIO_B5_CORE_2026-09-13.md`를 따른다. 결정 승인·선택 기록의 초안 사용 완료는 실제 사건 대조 후 명시 기록이며 일반HOTL/수정 제출의 서버 소비 결속은 아직 미완료다. 수정 입력의 제출 전 GET은 원자적 서버 CAS가 아니다. 서버 명령 영수증이 없는 UNKNOWN을 기존 캐시 task ID로 해제하지 않는다. 같은PLANNING 비파괴 재개는 현재 서버의 아카이빙 동작 때문에 안전 차단한다. Kit v2 검토 reader/화면 및 전역 단일 진입(B6), 실제 브라우저·현업 수용을 완료로 표시하지 않는다.
...
### 2026-09-13 실행 보고 규칙 보완

Supervisor의 추가 요청에 따라 이후 단계 보고에는 **현재 전체21/40 기준 진척, 완료한 배치, 다음 작업과 예상 시간 범위**를 함께 기록한다. 시간은 착수 시 코드/회귀 범위에 근거한 추정이며 변경 요인이 확인되면 갱신한다. B1 이후 B2 팩·설치/재개·명시 이관의 초기 추정은90~150분이다. 예상 시간을 완료 약속이나 검증 결과로 취급하지 않는다.

### B4 착수 전 연결 계약 보완 — 2026-09-13 09:40 KST
...
1. **서버 문맥과 행동 조회.** 명시 선택 scope에서 B1의 활성·유효기간·단일 OPERATING_PARENT
   ...
   resolved.capabilities는 역할 집합이므로, 같은 _authorize를 사용한 범위별 permitted_actions를 별도 제공한다.
   실제 쓰기는 현재 권한을 다시 검사하며 조회 장애를 허용/빈 데이터로 바꾸지 않는다.
2. **대기 건 발견과 재접속.** 설치 operation 및 변경안의 범위별 목록, 변경안 상세 조회를 제공한다.
   읽기 권한·문맥을 먼저 검사하고 페이지 크기를 제한한다. 다른 문맥의 이름·개수·digest를 노출하지 않는다.
   상세는 원문·고정 digest를 확인하고, GET으로 설치·승인·재개를 수행하지 않는다.
3. **기본 수정 명령.** 기존 ADD_NODE/RENAME/SET_USAGE/MOVE_NODE/ADD_SHORTCUT/REMOVE_SHORTCUT에
   설명(note) 및 placement 순서 변경을 보강한다. 부모 이동과 순서 변경은 구분한다.
   같은 process/placement ID·기존 앱 참조·불변 승인판을 유지하며 의미/표시 지문 회귀를 포함한다.
   데이터·앱 참조 편집을 새 인증이나 자동 실행 계약 발급으로 취급하지 않는다.
4. **설치 UI의 실제 호출 순서.** 후보 조회 → 적격자 register → plan → 원래 Plan 입력과
   plan_digest/client_request_id로 start → 최신 revision으로 resume/명시 인수 → 적격 타인 승인 →
   operation GET 및 최신 resolved를 각각 조회한다. 실제 경로는 /process-installations다.
   반환된 plan 전체를 start 입력에 펼치지 않으며, NO_DATA/DOMAIN_REVIEW_REQUIRED/setup_only를 유지한다.
5. **프런트 구현·검증.** companyApi에 별도 v2 DTO/구조화 오류 adapter, CompanySetupPanel에
   L1/L2 탐색·변경 제안/검토, EnterprisePage/KitOperationsPanel에 기존 권한을 보존한 연결을 순서대로 추가한다.
   409 입력 보존·다른 문맥의 늦은 응답 폐기·자가승인 차단·조회 실패와 빈 구성 구분을 검증한다.
   타입/빌드·단위/SSR·실제 API/브라우저·현업 수용을 각각 보고한다. 전역 App.tsx는 B6 전까지 변경하지 않는다.

권장 소유 분리: core/API 1명, 프런트 1명, 독립 부정 시험 1명, 권한/최종 UX 검토자 별도.
실제 배정은 B4 시작 때 TEAM_BOARD에서 확정한다. 서버 보강 → 지정 격리 회귀 → UI 연결 →
프런트 검증 순으로 진행하며, 구현의 독립 검토 없이 B4를 완료 처리하지 않는다.
예상 시간은 서버 보강을 포함해 **150~240분**으로 갱신한다(사전 UI 중심90~150분 대체).
표준 업데이트/복원과 현업 수용은 B7에 남으며, 이번 사전 대조는 전체21/40=52.5%에 가산하지 않는다.
...
### B2 안전 구현 보정 — 2026-09-13 02:13 KST

- 기존 DP registry/instance 테이블 ALTER와 영구 제약 추가안은 자동 안전 검토에서 차단되어 적용하지 않는다. 신규 `kit_process_artifacts`(불변원문/전용색인) 및 `kit_process_instances`(operation/id/root/원본 결속)를 동일 DP DB에 추가하는 방식으로 영향 범위를 축소한다. 두 표는 실제 업무지도 복제가 아니다.
- 기존1.0.0 Starter/운영키트 원본은 수정하지 않는다. `process_packs/.../1.1.0`에 새 독립 NO_DATA 참조 후보를 둔다. 이는 데이터/앱을 갖춘1.1.0 Starter 출시가 아니라 8L1·29L2의 검토용 단일 표준 원본이며 앱 후보는 REFERENCE_ONLY다. 레거시 registration과 동일 kit/version 충돌은 양방향 차단한다.
- DP 인스턴스는 전용 불변 artifact에서 읽고 mutable registry의 현재값을 기준선으로 사용하지 않는다. 업무 승인 직전에도 instance 상태/문맥/원본을 재검증한다. ECM 승인·head·change·outbox·설치APPLIED는 같은 ECM transaction, DP 생성과는 재개형 사가다.
- 설치 재개는 `expected_revision` 및 서버 `attempt_id` CAS. 적격 설치자의 명시 인수와 인수 이력, 이전 시도의 늦은 실패 무효화를 포함한다. 이미 적용된 작업의 이력은 후속 head 변경으로 미적용으로 되돌리지 않는다.
- 이 보정은 로컬 코드의 구현 방식이며 운영 DB 적용/현업 수용/70% 달성 증거가 아니다. B2 검증 및 B3~B7 출구를 순차 확인한다.
