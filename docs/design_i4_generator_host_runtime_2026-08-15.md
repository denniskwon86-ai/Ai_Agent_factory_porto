...
### 16-1. ★★★ 계약 지문 **하나로는 부족하다** (교차검토 93)

⚠️⚠️ **「계약서는 승인됐지만 DB 결속이 다른 상태」**를 계약 원문 지문은 잡지 못한다.
원문은 승인된 그대로인데 물질화가 어긋나 있으면, 앱은 **승인받은 계약과 다른 권한·다른
스키마로 돈다.** 그리고 원문 지문은 계속 일치하므로 아무 신호도 나지 않는다.

그래서 **두 지문을 나눠 봉인**한다:

| 지문 | 무엇의 사실인가 | 계산 |
|---|---|---|
| `runtime_contract_fingerprint` | **승인된 계약 원문** | `app_runtime_contract.semantic_fingerprint()` |
| `materialization_fingerprint` | **실제 DB 결속 상태** | `app_data_service.materialization_fingerprint(release_id)` |

물질화 지문에 들어가는 것(정렬본):

```
(runtime_name, dataset_key, schema_fingerprint, allowed_actions, contract_bound)
```

· 둘 중 **어느 쪽이 달라져도** 기존 증명은 **일반 만료가 아니라 `410`** 이다 —
  매니페스트 변경과 같은 처리(재발급 금지 · **프레임 폐기**).
  ⚠️ 만료로 다루면 브리지가 새 증명을 받아 **옛 코드를 계속 돌린다.** 그 앱은 이미
  다른 계약 위에서 도는 앱이다.
· ★ 정렬하는 이유: 행 순서에 흔들리면 **같은 상태가 다른 지문**을 갖고, 곧 거짓 경보가
  나며, 그러면 아무도 그 판정을 믿지 않는다.
· 라벨·생성시각 같은 설명값은 넣지 않는다(§의미 지문과 같은 이유).

**지문 폭** — ⚠️ 증명에 봉인되는 값은 **축약하지 않는다.** 16자(64비트)는 화면 표시에는
충분해도 권한 결속에는 좁다. `materialization_fingerprint`·`schema_fingerprint` 는
**전체 sha256(64자)** 이고, 사람에게 보일 때만 `short_fingerprint()` 로 줄인다.
★ 표시용과 결속용을 나눠 두면 기존 UI 계약을 깨지 않고도 보안 축을 넓게 유지할 수 있다.
(계약 원문의 `semantic_fingerprint` 는 계약 문서 스키마가 `^[0-9a-f]{16}$` 로 못박고
있으므로 **3단계에서 함께** 넓힌다 — 지금 바꾸면 1단계 계약 문서 형식이 함께 깨진다.)

**구현 상태(2026-08-15)**: 물질화 지문 **계산은 2.1 보정에서 완료**됐고 회귀도 있다.
**증명 봉인·`410` 강제는 3단계**다 — 그때까지는 계산만 있고 강제가 없다.
