...
core/external_intelligence/            (기존 .py 를 패키지로 — import 경로 불변)
  ...
  observation_promotion.py ★★★ [09-09] 격리 적재본 → 외생 지표 «관측값» 승격.
                           계획 동인이 읽는 자리로 — Q2 가 막혀 있던 고리다
  providers/
...
| | |
|---|---|
| ~~0~~ | ~~T3 재실행~~ — **완료**(6,445 통과 · 869초). 실패 1건은 원인·수정·재현까지 끝냄 |
| **0b** | ⚠️ 그 수정 뒤 **전체 재실행은 안 했다**(T3 하루 1회 예산). 다음 T3 때 확인 — §8 |
| **A** | `PUB-01` 편입 → 준비도 재평가. ⚠️ **실물 인증 경로가 없다** — §4-3 (거버넌스가 아니라 배선 문제) |
| **H** | ⚠️ **F-6 탄력도** — 환율 1%가 어느 계정을 몇 % 움직이나. **도메인 결정이고 지어내면 안 된다**(Bible §9.4) |
| **I** | ⚠️ **F-3 합의** — 격리 적재본을 앱에 «직접 열지 않고» 인증된 공통 자료를 계약으로 연결(Codex 권고) |
| **J** | 원천 승인 — 승인 전에는 승격이 **전부 거부**된다(정상이다) |
...
⚠️ **다음 Provider 를 붙일 때**(C 는 끝났다): `normalize()` 의 기본 구현을 만들지 말 것. 기반 클래스가 일부러 주지
...
```
전체 스위트 T3   6,445 passed · 2 skipped · 1 failed        869초 (14:29)
                 시점: 커밋 d56dfcda2 + Codex 미커밋분 (아래 ⚠️)
                 실패 1건은 «내 것»이었고 고쳤다 — 해당 파일 재실행 20건 통과
수집·준비도 회귀  753건 통과 (64초) · 네트워크 0회 · LLM 0회
tsc -b           0건
프로덕션 빌드     성공 · 패널 문구·경로 상수가 번들에 실림 확인
제품 경로 실측    없는 경로 404 · 수집 라우트 4개 401
```

### ⚠️ 이 T3 는 「커밋 `d56dfcda2` 의 것」이 아니다

작업 트리에 **Codex 의 미커밋 작업이 섞인 채로** 돌았다. 정확한 표현은
「`d56dfcda2` + Codex 미커밋분의 T3」다. 다른 사람이 같은 트리에서 동시에 일하는 동안
받은 결과에 커밋 하나의 이름을 붙이면, 나중에 그 커밋만 보고 재현하려는 사람이 속는다.

### 정산이 맞는다 — 다만 한 번 «안 맞았고», 그 이유가 시간이었다

    기준선 수집(7a782aa3e)                    6,284
    + 내 공공데이터포털                            36
    + 내 World Bank                             76
    + Codex kit_sample_audit                     25
    + Codex kit_sample_revision (T3 수집 시점)     27
    ────────────────────────────────────────────────
    = T3 수집                                 6,448  = 6,445 + 2 + 1  ✓
    + Codex 가 «T3 가 도는 중» 더한 것              18
    = 지금 수집하면                             6,466  ✓

★★★ 처음에 6,466 과 6,448 이 어긋나 **시험 18건이 사라진 줄 알았다.** 이 저장소에서
  편집이 회귀 21건을 조용히 지운 적이 있어서 그냥 넘기지 않고 파고들었다.

  ① `git diff --stat 7a782aa3e -- tests/` → 시험 파일에서 **삭제된 줄이 없다**
  ② 기준선을 별도 worktree 로 세워 수집 → **6,284** (기준선 숫자 자체는 맞다)
  ③ 양쪽 노드 목록을 `comm` 으로 대조 → 사라진 것은 **worktree 경로가 parametrize id 에
     박힌 3건뿐**이고 실제 손실이 아니었다
  ④ 그래서 손실이 «수집»이 아니라 «시각»에 있다고 보고 mtime 을 봤다:

        T3 시작 ≈ 15:01:21   (완료 15:15:50 − 869초)
        Codex 가 tests/test_kit_sample_revision.py 수정   15:07:19   ← 도는 «중»

  pytest 는 시작할 때 한 번 수집한다. 18건은 **사라진 게 아니라 수집 뒤에 더해진 것**이다.

⚠️ **교훈: 공유 트리에서 정산이 안 맞으면 「지워졌나」보다 「언제 세었나」를 먼저 의심한다.**
  단, 순서를 바꾸면 안 된다 — ①~③ 으로 「지워지지 않았음」을 **먼저 증명한 뒤에** 시각을
  봤다. 시각부터 의심하면 진짜 삭제를 시간 탓으로 덮게 된다.

### 실패 1건 — 내 것이었고, 낡은 «불변식»이었다

    tests/test_ecos_canary.py::test_catalog_ranks_both_providers_and_names_missing_credentials

    assert chosen == ()      # 「키가 없으면 아무것도 못 고른다」

이 단언은 **모든 원천이 키를 요구하던 동안만** 참이었다. 다섯째(World Bank)는 공식
파일이라 자격증명이 없다 — 키 없는 환경에서도 고를 수 있는 것이 **맞다.**
`ranked()` 를 읽어 코드가 옳고 시험이 낡았음을 확인한 뒤 시험을 고쳤다.

★ 이름 목록이 아니라 **규칙**을 단언하게 바꿨다 — 원천이 늘어도 같은 뜻으로 남는다:

    ① 키 없이 고를 수 있는 것은 「키를 요구하지 않는」 원천뿐이다
    ② 키를 요구하는 원천은 하나도 빠짐없이 「사유와 함께」 제외된다
    ③ 사유가 «어느 환경변수인지» 이름을 댄다

⚠️⚠️ 처음 쓴 규칙 ①에 구멍이 있었다. `chosen` 이 비면 `all(...)` 이 **공허하게 참**이 되어
  시험이 아무것도 지키지 않은 채 초록이 된다. `assert chosen` 을 앞에 뒀다.
★ 그리고 이 시험이 **실제로 반응하는지** 증명했다 — 다섯째를 「키가 필요한」 것으로
  바꿔치기하니 `chosen` 이 비면서 걸렸다. 「고쳤으니 됐다」로 끝내지 않았다.

⚠️ **고친 뒤 전체 스위트를 다시 돌리지 않았다.** T3 는 하루 1회 예산이고 오늘 썼다.
  고친 것은 시험 파일 하나뿐이며 그 파일(20건)과 인접 파일(총 91건)을 재실행해 통과했다.
  **「T3 전부 초록」이라고 주장하지 않는다** — 「6,445 통과 · 실패 1건은 원인·귀속·수정·
  재현까지 끝냈다」까지가 이번에 말할 수 있는 것이다.

### 「수집·준비도 회귀 753건」의 선택 집합 — 그대로 다시 돌릴 수 있게
...
⚠️ 앞선 커밋들의 「594건」·「579건」·「677건」과 이 753건은 **선택이 다르다** — 서로 빼서
증감을 계산하지 말 것. 위 목록이 지금부터의 기준이다.
⚠️ pytest 를 동시에 두 개 돌리지 말 것. 이 저장소에서 경합으로 3시간짜리 실행이 나왔고
멈춘 줄 알았다(실제로는 단독 실행 시 64초).
...
### 이 브랜치의 T3 이력
    ...
    6,445   d56dfcda2        + 공공데이터포털 36 · World Bank 76 · Codex 52
            (실패 1 — 낡은 불변식. 고쳤고 그 파일만 재실행)
    6,735   3b9ab8520 «동결»   + 내 73(추천기40·승격23·전달10) · Codex 222   ← **현재**
            (실패 3 — 전부 «환경»이다. 아래 §8-C 가 대조군으로 귀속을 증명한다)
...
`1bdada5a1`(Codex 기준선) 위에서 돌렸다.

★ **앞선 T3 들의 교훈은 그대로 둔다.**
  앞선 두 번은 T3 가 도는 동안 내가 작업을 이어가 결과가 「직전 시점의 것」이 됐다.
  이번에는 **내가** 코드를 건드리지 않았지만 **다른 에이전트가** 건드렸다 — 공유 트리에서는
  「내가 안 건드렸다」가 「트리가 안 변했다」를 뜻하지 않는다.
  ⚠️ 다음 커밋이 들어오면 이 줄을 다시 갱신할 것. 「거의 최신」인 검증 기록을 최신처럼
    두면 그 줄만 읽는 사람이 속는다.

## 8-B. 【2026-09-09】 승격 경로와 그 «보정 넷»

Q2「환율·원자재가 바뀌면 어느 계정에 영향?」이 막힌 기계적 원인은 **두 저장소가 만나지
않는 것**이었다 — 수집은 `data_acquisition_rows`, 계획은 `external_observations` 를 읽는다.
잇는 코드가 없어서 **Provider 를 다섯 개 만들어도 계획에는 한 방울도 흐르지 않았다.**
...
★ 관문을 다시 만들지 않았다 — `record_observation()` 이 이미 강제한다(vintage 필수 ·
  승인된 원천만 · 출처보다 높은 등급 금지). 재구현하면 한쪽만 고쳐지는 날이 온다.

### ⚠️⚠️ 첫 판에 결함 넷이 있었다 (Codex 가 찾았다 · 재현 후 수정)

| 결함 | 왜 시험이 못 잡았나 |
|---|---|
| 원천 ID 불일치 — 수집은 `provider_id`, 등록은 `src_…` | **내 fixture 가 진짜 `src_…` 를 주입했다** |
| **빈 `source_id` 가 승인 검사를 «건너뛴다»** | 아래 관문에 기댔다 — 거기 `if source_id:` 가 있다 |
| 5,001행 중 5,000만 세며 `accounted=True` | 저장소의 조회 한도를 몰랐다 |
| DB 장애가 「승인 대기」로 보였다 | `except Exception` 하나로 뭉쳤다 |

★★★ 첫째가 이 저장소의 반복 함정이다 — **내 말로 쓴 fixture 가 계약을 대신 정의했다.**
  실제 경로(`orchestrator.py:587`)는 `provider_id` 를 싣는데 fixture 는 등록 id 를 넣었다.
  17건이 100% 초록인데 **실제로는 한 건도 못 올라갔을 것이다.**

★ 고친 뒤: 원천 id = `provider_id` 로 못 박고(매핑표를 두지 않는다), 빈 값은 **승격이
  자기 층에서** 거부하고, 한도에 닿으면 **멈추고** 나누라고 말하고, 장애를 `REJECT_ERROR`
  로 갈랐다. 회귀 6건 추가(총 23건).

## 8-C. 【2026-09-10】 T3 — 동결 worktree 에서 돌렸다

    6,735 passed · 5 skipped · 3 failed · 978초(16:18)
    SHA 3b9ab8520 «고정» · 별도 worktree(/c/t3wt) · 데이터 20 DB · 릴리스 28건 복사

★ 동결 worktree 를 쓴 이유: 지난 T3 는 «도는 중 남이 시험을 더해» 정산이 18건 어긋났다.
  이번에는 검사 대상이 고정돼 그 일이 생길 수 없다. Codex 도 같은 방법을 요청했다(§6.3-3).

### 정산이 맞는다

    직전   6,445 + 2 + 1 = 6,448 수집   @ d56dfcda2 + Codex 미커밋분
    이번   6,735 + 5 + 3 = 6,743 수집   @ 3b9ab8520 (동결)
    증가   295

    내 것        추천기 40 · 승격 23 · 전달(실물) 10           =  73
    Codex 신규   kit_* 5파일 220 · release_* 2파일 29         = 249
    합계 322 인데 실제는 295 — **차이 27 이 설명된다:**
      `test_kit_sample_revision.py` 는 «직전 T3 에도 있었다»(당시 미커밋 27건 → 지금 46건).
      실제 증가는 19건뿐이라 46−19=27 을 과대계상했다.  322 − 27 = 295 ✓

### ⚠️ 실패 3건은 «제품 결함이 아니라 내 씨앗 누락»이다

    test_openrouter_only_mode.py (2건)    worktree 에 `.env` 가 없다 → OPENROUTER_API_KEY 부재
    test_starter_package_catalog.py (1건)  DB(체크섬)만 복사하고 «자산 파일»은 git 판 → 불일치

**대조군으로 증명했다** — 본 트리에서 같은 두 파일이 **23건 전부 통과**한다. 그리고
`3b9ab8520..HEAD` diff 에 두 시험의 대상이 없어 «커밋 차이»도 아니다.

★★★ 다음에 동결 worktree 로 T3 를 돌릴 때 **씨앗에 이것들을 포함할 것**:
  `.env` · `data/` 의 **DB 뿐 아니라 자산 파일 전부**(`company_profiles/` 등) · `library/`.
  ⚠️ DB 만 복사하면 «체크섬은 새것, 파일은 옛것»이 되어 무결성 시험이 깨진다.
    이번이 그것이었다 — 격리를 만들면서 **격리가 깨뜨리는 것**을 안 셌다.

### 그래서 말할 수 있는 것 / 없는 것

    ✅ 「6,735 통과 · 실패 3건은 환경이며 대조군으로 귀속을 증명했다」
    ✘ 「전부 초록」          — 씨앗을 고쳐 다시 돌리기 전까지는 아니다
    ✘ 「6,738 통과」         — 돌려 보지 않은 수를 적지 않는다
