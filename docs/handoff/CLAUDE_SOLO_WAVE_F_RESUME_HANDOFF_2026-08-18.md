...
```text
Wave A~E · F-0(계약 물질화) · F-1(Preview 경계) 완료.  다음은 F-2 (I-4 7 ACTIVE 승격).
전체 회귀 4,102건 통과 · 실패 0.

⚠️⚠️ F-2 를 시작하기 전에 반드시 읽을 것:
   §4.1c — **F-1 은 «경계를 세운 것» 이지 «Preview 가 도는 것» 이 아니다.**
            아직 아무도 릴리스를 `candidate` 로 만들지 않는다.
   §4.2  — 증명이 Snapshot 지문을 봉인하지 않는다(F-2 에서 함께 닫는다)
   §6.1b·§6.1c — 이 세션이 실제로 다친 자리
```
...
**현재 기준선: 4,102건 · 실패 0 · 오류 0 · 건너뜀 1.** 소요 약 9분.
...
Wave F-1 (I-4 6 Preview 경계)             ██████████  완료 — ⚠️ §4.1c 먼저 읽을 것
Wave F   (I-4 7 · 8)                     ░░░░░░░░░░  미착수 ← 여기부터
