...
⚠️ **승격이 Snapshot 지문을 남기지 않는다.** §4.2 와 한 쌍이다 — 「이 판을 올릴 때
  어느 데이터 판을 보고 있었나」에 답하려면 승격 기록에 그것이 있어야 한다.

⚠️ **화면이 없다.** 승격은 API 로만 가능하다(Wave H).

### 4.5 ⚠️ 아직 안 한 것 — 파일럿 전에 볼 것

**① 변이 검사가 밀려 있다.** Supervisor 방침(속도 우선)에 따라 Wave G·H 는 변이
  검사를 돌리지 않았다. F-0~F-2 는 돌렸다(23/23 · 22/22 · 23/24).
  ⚠️ Wave G 는 **숫자를 만드는 코드**다 — 여기서 조용한 오답이 나면 그것이 곧 회의에
    올라가는 숫자다. 파일럿 전에 한 번 묶어 돌리기를 권한다.

**② 화면을 빈 상태에서만 눌렀다.** 각 화면이 열리는 것은 캡처로 확인했지만, 실제
  데이터가 채워진 화면(준비도 보드가 READY 로 차고 · 결과표가 나오고 · 안건이
  만들어지는 것)은 보지 못했다. 백엔드 동선은
  `tests/test_pilot_walkthrough.py` 가 덮지만, **화면이 그 응답을 제대로 그리는지는
  다른 문제**다.

**③ 승격이 Snapshot 지문을 남기지 않는다**(§4.2 와 한 쌍). 「이 판을 올릴 때 어느
  데이터 판을 보고 있었나」에 아직 답할 수 없다.
