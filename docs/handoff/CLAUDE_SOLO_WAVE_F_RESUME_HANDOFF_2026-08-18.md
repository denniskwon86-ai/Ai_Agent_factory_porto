...
**⑤ 기준값 7개를 사람이 손으로 채운다.** 이 키트의 데이터로는 `production_qty` ·
  `purchase_payment` 정도만 유도할 수 있고 `ending_cash` · `operating_profit` ·
  `power_cost` 는 재무 데이터라 나오지 않는다. 「유도할 수 있는 것은 채우고 나머지는
  묻되, **둘을 구분해 보여 준다**」가 이 제품의 방식에 맞는 답이다 — 아직 안 했다.
