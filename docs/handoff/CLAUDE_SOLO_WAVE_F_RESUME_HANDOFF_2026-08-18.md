...
```text
Wave A~E · F-0(계약 물질화) · F-1(Preview 경계) · F-2(ACTIVE 승격) 완료.
다음은 F-3 (I-4 8 종단 카나리).
전체 회귀 4,152건 통과 · 실패 0.

★★★ **이제 관통한다**: 게시 → 후보 → Preview 확인 → 승격 → 운영.
    F-0·F-1 에서 두 번 반복된 「통제는 있는데 부르는 경로가 없다」가 F-2 에서 닫혔다.

⚠️ F-3 을 시작하기 전에 반드시 읽을 것:
   §4.2 — 증명이 **Snapshot 지문을 봉인하지 않는다**(아직 열려 있다)
   §4.4 — F-2 가 남긴 것
   §6 — 이 세션이 실제로 다친 자리(특히 §6.1e — 같은 실수를 두 번 했다)
```
...
**현재 기준선: 4,152건 · 실패 0 · 오류 0 · 건너뜀 1.** 소요 약 9분.
...
Wave F-3 (I-4 8 종단 카나리)              ░░░░░░░░░░  미착수 ← 여기부터
