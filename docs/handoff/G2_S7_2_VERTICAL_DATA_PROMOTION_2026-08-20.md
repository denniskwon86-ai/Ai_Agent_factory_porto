...
2. 다섯 종은 전부 **한 tenant·한 범위**(`plant-afs-smelting-01`)다. 조직 간 격리를
   시험하려면 다른 범위가 필요하다.
   ⚠️⚠️ **정본 데이터에 타 조직 행을 섞지 않는다** — 섞는 순간 §2.2 의 「tenant 혼입 0」이
   무너지고, 정본이 「격리를 시험하려고 일부러 오염시킨 자료」가 된다.
   ★ 대신 **시험용 두 번째 가상 범위를 별도 fixture** 로 만든다(3단계).
