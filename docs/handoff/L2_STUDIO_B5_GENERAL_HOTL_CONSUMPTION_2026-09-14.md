...
## 추가: 설계 문서 상태 실측 대조 · 미정 출구 판정 — 2026-09-14 KST

Claude Code / 권고10 / G3. 전체21/40=52.5% **유지**(문서 정정과 판정이며 기능 변화 없음).

### 왜 했는가

23:03 Codex 인계가 잔여 항목에 대해 「**B5 또는 후속의 어느 출구인지 설계와 대조해 명시한다**」
고 지시했는데 나는 「운영 판단이 필요해 미정」으로 넘겼다. 지시를 이행하지 않은 것이다.
아울러 설계 문서 세 곳의 「미구현」 표시가 실제와 다를 수 있어 함께 대조했다.

### 1. 낡은 「미구현」 표시 세 곳을 실측해 정정했다

| 위치 | 표시 시점 | 실측 |
|---|---|---|
| 실행 설계서 §8.3 API 변경 목록 | 09-12 | **6행 모두 구현** — 각 행에 근거 파일을 표로 적었다 |
| 실행 설계서 B4 연결 계약 보완 | 09-13 09:40 | **5항목 모두 구현**(B4 는 같은 날 17:14 마감) |
| 업무키트 L2 설계서 머리말 | 09-12 | §5 팩·§6 v2 저장·§7 설치 흐름 **구현**, §4 카탈로그 **부분** |

★ 방치하면 다음 담당이 **이미 있는 것을 다시 만든다.** 각 정정은 근거 파일·경로를 함께 적었고,
「구현됐다 ≠ 그 절의 합격 기준을 모두 만족한다」를 못박아 두었다.

### 2. 미정으로 남겼던 항목의 출구 — **B5 가 아니라 후속(운영)이다**

「서버 PROCESSING/UNKNOWN 잔류의 운영 복구」를 설계와 대조한 결과다.

- 설계 B5 출구는 「결정 종류·개별 실행/일반/쿼터 재개·저장·오류/접근성·정적/브라우저 분리」다.
  검증 항목 **U26** 이 요구하는 것은 「거절/응답 유실 시 **상태 초기화·추가 REVISION 자동 생성이
  없을 것**」이다 — 즉 **자동으로 손대지 않는 것**이 B5 의 합격 기준이다. 그 기준은 이미 지켜진다
  (다른 키 신규 실행 차단, 원키 GET 만 제공, 자동 재전송 없음).
- 반면 「사람이 확인하고 잔류 기록을 닫는 경로」는 **운영 절차와 권한 설계**가 먼저 있어야 한다.
  누가 어떤 근거로 닫는가, 그 판단을 무엇으로 증명하는가는 제품 오너 결정 영역이다.
- 설계 B7 출구가 「process upgrade/restore, 회귀·브라우저·현업 측정」이므로 **운영 복구는 B7
  계열**로 두는 것이 맞다. 다만 B7 은 B6 를 선행 조건으로 둔다.

★ 결론: **B5 출구는 이 항목 때문에 열려 있지 않다.** B5 잔여는 실제 브라우저 수용뿐이다.
운영 복구는 B7 로 넘긴다. 자동 재실행·한도 초기화·서버 내부 UNKNOWN 자동 종결은
`L2_STUDIO_B5_CORE` 기록대로 **구현하지 않은 상태를 유지**한다.

### 검증

문서 정정과 판정이므로 코드 변경 없음. 대조에 쓴 근거는 각 정정 표에 파일·경로로 적었다.
회귀는 직전 절 기준(서버 392PASS·프런트 139PASS·설치 105PASS)에서 변동 없다.

---

## 추가: 접근성 정적 보강과 **배선 누락 1건 수정** — 2026-09-14 KST

Claude Code / 권고10 / G3. 전체21/40=52.5% **유지**.

### ★★ 먼저 — 배선 누락을 스스로 만들었고 점검에서 잡았다

앞 절에서 `studioDecisionFlow.recheckSubmission`(원키 재조회)을 만들고 **화면에 붙이지
않았다.** 계약 시험은 flow 를 직접 불러 초록이었지만 **사용자는 그 기능을 쓸 수 없었다.**
응답이 유실됐을 때 접수 여부를 확인할 유일한 경로였는데도 그랬다.

⚠️ AGENTS 가 「생산자→소비자 배선 누락은 이 프로젝트에서 반복된 결함 유형」이라고 못박은
바로 그 유형이다. 같은 기능이 수정 요청 화면(`RevisionRequestEditor`)에는 버튼으로 있다.

조치:
- `StudioDecisionPanel` 의 HOTL 처리 기록에 「이 제출의 접수 상태 확인 · GET 조회만」 버튼 추가.
- ★ 버튼이 쓰는 값은 `eventId` 가 **아니라** 보존된 요청 본문의 `body.client_request_id` 다.
  `eventId` 는 접수를 확인했을 때만 채워지는데, 이 버튼이 정작 필요한 때는 **응답이 유실된
  UNKNOWN** 이다. 처음엔 `eventId` 로 썼다가 이 사실을 확인하고 바꿨다.
- 재발 방지로 STATIC 검사를 넣었다 — 화면 소스에 `flow.recheckSubmission(` 호출이 있고,
  `record.body?.client_request_id` 를 쓰며, `eventId` 로 부르지 않는지 대조한다. 실제 클릭이
  아니라 소스 대조다.

### 접근성(U16) — 정적으로 닫을 수 있는 것만

설계 §U16 은 「1280×720/1440×900 및 좁은 화면; 핵심 조작/오류/중단 접근 가능」이다.
**본체는 실제 브라우저이며 이번에 닫지 못했다.** 정적으로 확인·보강한 것만 적는다.

| 항목 | 현황 |
|---|---|
| 오류·상태 알림 | `role="alert"`·`role="status"` 로 이미 배선돼 있었다(패널 10곳) |
| 비활성 사유 안내 | 계약·능력 쪽은 있었으나 **HOTL 제출 버튼에는 없었다** → 추가 |
| 새 버튼 설명 | 원키 재조회 버튼에 `aria-describedby` 로 「재전송 아님」을 붙였다 |
| 좁은 화면·키보드 이동·포커스 순서 | **NOT_RUN.** 실제 브라우저 필요 |

HOTL 제출 버튼은 비활성 조건이 셋(잠김·질문 미확인·확인란 미선택)인데 이유가 화면 다른 곳에
흩어져 있었다. 버튼 옆에서 읽히도록 `role="status"` 안내를 두고 `aria-describedby` 로 묶었다.

### 검증

서버 **392PASS/178.30초**(`protected_assets_unchanged`·`sources_unchanged` true),
프런트 **139PASS**(직전 138 + STATIC 배선 1), tsc 0 · build PASS ·
