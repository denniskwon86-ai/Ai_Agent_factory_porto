...
> **데이터 전달 변경(2026-09-15, 사용자 명시 승인):** data_sync/session-data-20260913.zip 평문 공유본을 추가했다. 새 PC는 키 없이 [최신 ZIP 복원 절차](../../data_sync/README.md)를 따른다. 아래 §8의 키 필요 설명은 기존 암호화본에만 적용된다. 데이터 시점(09-13)·제외 범위·동일 절대경로·무덮어쓰기 조건은 그대로다. 전체164파일이며 새 운영 데이터 수집은 아니다.
...
**현재 권장: 키 없는 ZIP.** data_sync/README.md 최상단 명령으로 session-data-20260913.zip을 검증·복원한다.
다음은 호환성을 위해 남긴 **암호화본 전용 과거 안내**이며, ZIP 사용자에게 키를 요구하지 않는다.
