...
**작성 2026-08-12 · 갱신 2026-08-13 · 상태: 판정기 완성 · 배선 완료 · 실서버 실측 미완**
...
## 0. 2026-08-13 갱신 — **배선을 끝냈다.** 다음 사람은 여기부터 읽는다

아래 §3 이 「아직 안 된 것」으로 적어 둔 것 중 **3.1·3.3·3.5 는 끝났다.** 남은 것과 새로
드러난 것만 먼저 적는다.

| §3 항목 | 상태 |
|---|---|
| 3.1 라우트·SSE 배선 | ✅ 목록·상세·쓰기·SSE 전부 `project_visible` |
| 3.2 파장 측정 | ✅ 아래 「실측」 |
| 3.3 화면 오류 계약 | ✅ 목록 응답에 `viewing_context`·`context_blocked_count`·`context_needs_attention`·`context_blocked_reasons` |
| 3.4 검증 매트릭스 | ✅ `tests/test_visibility_wiring_parity.py` 14건(⬜ 6칸 중 5칸) · **화면 반영은 미착수** |
| 3.5 변이 검사 | ✅ 판정기 8/8 · 배선 5/5 |

### 아직 안 된 것 (재개 순서)

1. **⚠️⚠️ 실서버 실측을 하지 않았다.** 단위·종단 초록은 실측의 대체물이 아니다 — 이 저장소에서
   다섯 번 어긋났다. **재개 시 첫 순서.** 확인할 것: 관리자 로그인 → `GET /projects` 가
   `context_blocked_count=59`·`context_needs_attention=0` 을 싣는지, 사유별 집계가
   `MODE_MISMATCH=59` 인지.
2. **화면(프론트)이 아직 이 값을 쓰지 않는다.** 서버는 「왜 안 보이는지」를 싣기 시작했는데
   화면은 여전히 목록만 그린다. 그대로 두면 **59건이 사라진 자리가 「0건」으로 보인다** —
   이 배선이 막으려던 바로 그 상태다. `LLM_MASTER_SPEC §19`(완료 기준 4가지)상 **아직 완료가
   아니다.**
3. **`demo-todo-app` 의 범위가 코드다.** `enterprise_scope_id="hq"`(부서 코드) — 61개 중
   **유일하게** 정본 노드 id 가 아니다. D-018 상 백필 대상이며, 그대로 두면 그 프로젝트만
   `SCOPE_OUTSIDE` 로 빠진다. **런타임 정규화로 때우지 말 것**(판정이 두 곳이 된다).
4. **G1-C05**(알림 재전송·읽음·멱등성·오프라인 복구) 미착수 — P1.

### 2026-08-13 실측 (읽기만)

| 사용자 | 문맥 | 배선 전(권한만) | 배선 후 | 차단 사유 |
|---|---|---:|---:|---|
| hikwon@lsmnm.com | (전체)·REAL | 61 | **2** | MODE_MISMATCH 59 |
| hikwon_7@lsmnm.com | (전체)·REAL | 0 | 0 | UNAUTHORIZED 61 |
| hikwon_2@lsmnm.com | (전체)·REAL | 0 | 0 | UNAUTHORIZED 61 |
| hikwon_17@lsmnm.com | (전체)·REAL | 0 | 0 | UNAUTHORIZED 61 |

★ **`PROGRESS.md` 의 「61/54/54」 실측(2026-08-09)은 이미 낡았다.** G1-C1.1·C2 격리가 시험
  산출물 59건을 `private`+SANDBOX 로 옮기면서 비관리자에게는 **0건**이 됐다. 즉 지금 이
  제품은 비관리자에게 보여줄 프로젝트가 없다 — 배선 때문이 아니라 **데이터 상태**다.

### 설계 판단 3건 (뒤집으려면 근거부터 보라)

1. **고르지 않은 범위로 좁히지 않는다.** 종전 정규화기는 주 부서로 추측해 채웠고, 그것이
   관리자에게서 법인 소속 프로젝트를 지웠다(`hq`→`MNM_SHARED`, 자원은 상위 `LS_MNM`).
   부서 추측을 되살리면 「내 프로젝트가 안 보인다」가 다시 시작된다.
2. **지정 알림(`emit_to`)은 테넌트·실행 모드까지만 본다**(`notification_context_visible`).
   프로젝트 이벤트와 같은 판정기를 쓰면 결정 요청이 결정권자에게 도달하지 못한다.
3. **미바인딩 자원은 관리자에게도 실시간으로 가지 않는다.** 목록이 안 보여 주는 것을 SSE 만
   보내면 G1-C3 가 없애려던 비대칭이 그대로 남는다. 관리자는 `context_needs_attention`
   **집계**로 부채를 본다 — 이벤트로 알리면 멈춰 있는 부채는 영영 안 보인다.

### 이번에 배운 것 — 검사기를 먼저 검증한다

첫 변이 검사에서 **8개 변이가 전부 «살아남음»** 으로 나왔다. 코드가 아니라 **검사기가
고장**이었다 — `pytest.ini` 의 `addopts=-q` 와 내가 준 `-q` 가 합쳐져 `-qq` 가 되면 요약 줄이
사라지고, 그것을 파싱하던 함수가 언제나 「통과 0」을 돌려줬다. 기준선이 「통과 0」인 것을 보고
멈추지 않았다면 판정기 8곳을 전부 오판했을 것이다. **스캐너를 검증 없이 믿어 틀린 네 번째
사례다.** 종료코드를 1차 신호로 쓰도록 고쳤다.
