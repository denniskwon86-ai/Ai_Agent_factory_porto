...
## 16. Codex §15 검토 보완 — 정본 자료 기반 읽기 (2026-09-24 KST)

상세 정본은 요청서 **§16**.

| 항목 | 결과 | 증거 |
|---|---|---|
| R01.1 읽기를 키트 정본 합성 샘플(정본 필드·업무키)로 보완 | 3 passed / exit 0 | `output/usage-holds-ugm6t54x/` |
| 새 필드 단언의 검출력 | 보완 첫 실행이 `record_id` 누락으로 FAIL — 제품 규칙(예약 이름)이라 기대를 규칙에서 유도 | `output/usage-holds-l9zqpelv/` |

- 제품 변경 없음(시험 1개). §15.7 회귀 386 passed 는 제품 해시가 같아 유효.
- 새 빈틈(보고만): 인증이 키트 데이터셋 계약의 필수 필드·업무키를 보지 않는다 — `amount` 한 칸 판이 운영 조회까지 통과. 착수 여부 Codex 결정.


---

## 17. 인증 계약 관문 — 설치 고정 계약과 봉인 원문 대조 (2026-09-25 KST)

상세 정본은 요청서 **§17**.

| 항목 | 결과 | 증거 |
|---|---|---|
| 팩 1.2.0(데이터셋 계약 35개 수록), 1.1.0 번들 지문 불변 | 적재·재검증 통과, 1.1.0 지문 `670973f6…5f0d` 유지 | `tests/test_pack_dataset_contracts.py` |
| 인증 관문(누락·빈 업무키·중복 차단, 고정 계약 없으면 차단) | 관문 시험 7 + 대조기 27 통과 | `output/usage-holds-t2w47ck7/` |
| 관련 회귀 32개 파일(병렬 7묶음) | **1,146 passed · 2 skip(심볼릭 링크) · 실패 0** | 요청서 §17.5 표 |
| 변이(관문이 대조 결과 무시) | 대조 시험 3건 FAIL, 나머지 4건 통과 | 워크트리 제거함 |
...
## 20. §19 보완 — 승인 뒤 활성화 · 판본별 릴리스 · 목록 활성 판본 · 회귀 정산 (2026-09-25 KST, Claude)

상세 정본은 요청서 **§20**.

| 항목 | 결과 | 증거 |
|---|---|---|
| 승인 뒤 활성화(반려 → 1.1.0 유지, 승인 직후 활성화 장애 → 운영 차단 → 같은 승인 재요청 복구) | 통과 | `tests/test_kit_upgrade_recertification_e2e.py` (G7 `output/usage-holds-d9om94ug/`) |
| 판본별 릴리스(새 ID 게시·운영 조회, 옛 게시물 파일·cohort 불변·운영 거절) | 통과 | 같은 파일 |
| 목록 활성 판본(`version`·`pending_upgrade`, 실제 목록 API) | 통과 | 같은 파일 |
| 관련 회귀 111개 파일(9묶음) | 관련 회귀 111개 파일 3,750건 **새 실패 0**(기존 실패·오류는 대조와 nodeid 일치) | 요청서 §20.7 |
| §18.4 수치 정산 | P3·P6 을 testcase 기준으로 다시 적음, 실패 nodeid 대조 일치 | `docs/handoff/evidence/W03_R01_S18_REGRESSION_ACCOUNTING_2026-09-25.json` |

- 남음(결정): 옛 판본 릴리스 자동 퇴역, 활성화 복구 주체, Native 작성 데이터 이관(SIM-02 와 함께), 화면 연결(Codex), SIM-02.
- 진척 1895/5300=35.8% 유지(Codex 원장), 점수 주장 없음. 실제 브라우저 NOT_RUN.


---

## 21. §20 결정 반영 — 목록·이력·활성화 대기 화면 연결과 브라우저 확인 (2026-09-26 KST, Claude)

상세 정본은 요청서 **§21**.

| 항목 | 결과 | 증거 |
|---|---|---|
| 화면 연결(적용본 판본·대기, 이전 판본 이력, 활성화 대기·승인자 재시도) | 연결 | `KitOperationsPanel`·`KitAppPanel`·`ProcessInstallationPanel` |
| 설치 흐름 검사(메모리 대역·SSR) | 108/108 | `output/process-installation-check-3c108bb9-…/report.json` |
| 실제 브라우저 · 합성 API fixture | 클릭 흐름·역할별 버튼·세 상태 표시 확인 | fixture 2종, 헤드리스 Chrome 캡처 |
| 실제 서버 전체 경로 브라우저 | NOT_RUN(격리 파종 + 사람 로그인 필요) | — |
| 백엔드 집중 | E2E 4 · 설치 묶음 394(2 skip) · B1/B4 117 passed | `output/usage-holds-9n75xnw6/`·`9k0uhrtj/`·`w1g4r83s/` |

- 발견: 계약 검토 401/403/404 → 앱 목록 무한 재조회(기존 결함, 별도 작업).
- 진척 1895/5300=35.8% 유지, 점수 주장 없음.
