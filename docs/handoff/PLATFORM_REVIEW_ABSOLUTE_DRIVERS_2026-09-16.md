...
## 4.5 ★ 이음매를 구현해 **실측으로 재확인했습니다** (2026-09-21)

설계 단계의 계산이 아니라 **실제로 돌린 결과**입니다.

    scripts/kit_bridge.py    to_pct_change(value, unit)

키트 시나리오 10 개를 넣어 보니 **정확히 4 개가 거부**됩니다.

| 단위 | 건 | 어떻게 되나 |
|---|---|---|
| `%` | 6 | ✅ ×100 해서 넘어갑니다 |
| `DAY` · `HOUR` · `KRW` | 3 | ✗ `UnsupportedScenarioUnit` — **조용히 버리지 않고 멈춥니다** |
| `PERCENT_POINT` | 1 | △ `NeedsBaseline` — 기준값(현재 수율)이 있어야 옮깁니다 |

★ **옮길 수 없는 것을 옮긴 척하지 않습니다.** 그러면 그 시나리오는 「실행됐는데 아무
  일도 안 일어난」 것이 되고, 그 사실을 아무도 모릅니다.

⚠️ `%p` 는 특히 조심했습니다. 수율 92%→90% 는 −2%p 이고 **−2.17%** 인데, `%` 로
  잘못 보면 −2.0 이 되어 **그럴듯하게 틀립니다.** 차이가 작아 눈으로 못 잡습니다 —
  시험으로 잠갔습니다(`tests/test_kit_bridge.py`).

**이 문서의 관찰은 그대로입니다.** 막히는 곳은 여전히 **동인 → 가정 전개** 하나이고,
해결 방향은 플랫폼 쪽이 정합니다.
