...
## 0-B. ★★ 추가 조사 결과 (2026-07-27 01:30) — 진짜 근본 원인은 "모델"이었다

> `docs/handoff_2026-07-27_a1_pause_review.md`(외부 세션, v4 중단 보고)를 받아 재조사한 결과다.
> 그 문서의 F-1~F-3 은 모두 타당하지만 **순서가 틀렸다.** 아래가 선행되어야 한다.

### 실측 — 코드를 쓰고 있던 모델

`data/llm_call_log.jsonl` 집계:

| 프로젝트 | 총 LLM 호출 | 사용 모델 |
|---|---|---|
| `test_a1_v3` | 100건 | **100건 전부** `meta-llama/llama-3.3-70b-instruct` |
| `test_a1_v4` | 35건 | **35건 전부** 동일 모델 |

- **Gemini 가 코드 생성에 쓰인 횟수: 0회.**
- `attempts` 체인 길이가 31건에서 **1** — 폴백을 타고 내려간 게 아니라 **처음부터 llama 만 시도**했다.
- 무료 티어(Gemini·xAI·Groq·Cerebras)가 소진되어 `MODEL_COOLDOWN_SEC=1800` 쿨다운에 갇혔고,
  `_compose_chain` 의 '전 모델 쿨다운 → 유료 백스톱 재프로브' 분기로 체인이 **유료 1개로 붕괴**한 채 계속 돌았다.
  (쿨다운이 만료돼 체인 7~8개로 복귀한 호출이 4건 있었으나, 무료가 전부 실패해 다시 붕괴했다 —
   즉 **로직 결함이 아니라 무료 쿼터가 실제로 죽어 있는 환경**이다.)

### 원인 지목

[config.py:62](../config.py#L62)

```python
"openrouter": { "pro": "meta-llama/llama-3.3-70b-instruct", ... }
```

**Pro 티어의 유료 백스톱이 llama-3.3-70b 다.** 이 모델이 모든 코드를 쓴다.
이 모델의 출력 상한은 정확히 **8,192**([config.py:151](../config.py#L151)) — 결함 #15 가
"정확히 8,192 에서 파싱 실패"하는 이유가 **상한 부족이 아니라 이 모델의 천장**이라는 뜻이다.

### 이 하나가 v4 의 세 증상을 전부 설명한다

| 증상 | 설명 |
|---|---|
| `def get_history):`, `def load_data):` 구문 오류 | 약한 모델의 코드 품질. 우리 후처리는 무관함을 확인했다(§아래) |
| 구조화 응답이 정확히 8,192 소진(#15) | llama 의 출력 천장 + 거대한 `CodeOutput` 스키마 |
| 228초 / 420초 / 77초 지연 | 70B 모델이 8k 토큰을 뽑는 시간 |

**Gemini 2.5 Pro 는 `def get_history):` 를 쓰지 않는다.**
지금까지 파이프라인을 의심해 왔지만, 실제로는 **가장 어려운 작업(코드 생성)을 체인에서 가장 약한 모델에게 맡기고 있었다.**

### 우리 후처리는 무죄임을 확인함

손상이 우리 코드 탓인지 검증했다.
- `_extract_files_from_json`([nodes/execution.py:38](../nodes/execution.py#L38)) — 정상 JSON 파싱만 한다.
- `_repair_and_parse_json`([core/llm_gateway.py:719](../core/llm_gateway.py#L719)) — 보존적이다. 괄호 균형 수술을 하지 않으며, 원본 → 트레일링콤마 제거 → 제어문자 escape 순으로 시도할 뿐이다. 또한 이 함수는 `output_mode="json"` 전용이고, **code 모드는 LangChain 의 구조화 파싱을 거친다.**
- 프로젝트 자체 git(`projects/test_a1_v4/.git`)에 손상본이 커밋되지 않았다 = **빌드 게이트는 정상 작동**해 손상 코드를 차단했다.

→ **결론: 코드 손상은 LLM 출력 그대로다. 우리 파이프라인이 만든 게 아니다.**

### F-1~F-3 에 대한 평가

| 항목 | 평가 |
|---|---|
| **F-1** 출력 계약 축소 | **옳다.** 단 llama 의 8k 천장을 완화하는 조치이지 근본 원인 제거가 아니다. 강한 모델로 바꾸면 필요성이 크게 줄지만, 해두면 어떤 모델에서도 폭주 확률이 낮아진다 |
| **F-2** 종료 계약 완성 | **옳고 필요하다. 그러나 완주를 막는 요인이 아니다.** 실패를 더 잘 보고하게 만들 뿐 성공하게 만들지 않는다 |
| **F-3** 공급자 실패 승격 | 같은 성격. 420초 대기가 안 보이는 것은 UX 결함이지 완주 차단 요인이 아니다 |

⚠️ **F-2·F-3 을 먼저 하면 "실패를 더 예쁘게 보고하는 시스템"이 된다.** 완주가 목적이면 순서를 바꿔야 한다.

### 권고 순서 (F-0 신설)

1. **F-0 (신설·최우선)** — OpenRouter **Pro 백스톱을 강한 코딩 모델로 교체**([config.py:62](../config.py#L62)).
   한 줄 변경이고 효과가 가장 크다. **비용이 드는 결정이므로 사용자 승인 필요** — 2026-07-27 시점 미승인.
2. **F-1** 출력 계약 축소
3. 새 프로젝트 `test_a1_v5` 로 재실행 (v3·v4 는 오염되어 재개 금지)
4. **F-2·F-3** — 완주 확보 후

### 부수 발견 (미등재 결함 후보)

`백엔드 스모크 실패: OperationalError: unable to open database file` —
생성된 백엔드가 존재하지 않는 디렉터리에 SQLite 를 만들려 했다.
[nodes/utils/backend_smoke.py](../nodes/utils/backend_smoke.py) 는 `tempfile.mkdtemp` 에 파일만 풀고 **상위 디렉터리를 만들어주지 않는다.**
#20(렌더 하네스)과 같은 성격이나 이번엔 LLM 코드 쪽 책임도 절반이다. 스모크 하네스가 부모 디렉터리를 생성하도록 보완할 가치가 있다.
