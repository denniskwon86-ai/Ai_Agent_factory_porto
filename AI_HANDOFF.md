...
- **최종 갱신**: 2026-08-04 (UI/UX 이관 3~9/10 · **읽기·쓰기 권한 구멍 6건 수정** — 아래 첫 섹션)
...
> 팀 보드: `.agents/TEAM_BOARD.md` `[UIUX-MIG-27]` ~ `[UIUX-MIG-31]` · 기준 HEAD `7c60dcf80`
>
> ### 이어받는 사람이 가장 먼저 알아야 할 세 가지
>
> **① 이관은 «화면을 라이트로 바꾸는 일»이 아니다.** 화면 7개를 옮기는 동안 **읽기·쓰기 라우트에
> 자격 검사가 없는 구멍이 여섯 번 연속** 나왔다. 익명이 에이전트 판정 기준을 개정하고, 전 직원
> 명부를 보고, **HOTL 중단점을 통과**시킬 수 있었다. 매번 화면 작업보다 통제 복구에 시간이 더 들었다.
> → **화면을 열기 전에 그 화면이 쓰는 라우트부터 훑으십시오.** 남은 «Principal 없는 라우트»가
> 약 60개다(planning 13 · crosswalk 9 · benchmark 6 …).
>
> **② 통제는 한 경로만 막아서는 안 된다.** 6/10 에서 «중단점을 지우려면 관리자 권한»을 막았는데
> 7/10 에서 보니 **지울 필요 없이 통과**시킬 수 있었다. 9/10 브리핑은 5/10 에서 막은 자료를
> **다시 모아** 보여 줬다. 판정은 라우트가 아니라 **판정 함수 안**에 둔다.
>
> **③ 자동 게이트 통과는 «검증 완료»가 아니다.** `scripts/capture_screens.py` 가 재승인 기준 10여
> 항목을 기계로 검사하는데, **그것이 전부 통과한 뒤에도 화면마다 2~4건이 육안으로만 잡혔다**
> (빈 원 아바타·동작하지 않는 검색창·사라진 제목·권한 잔상). 반드시 PNG 를 열어 보십시오.
>
> ### 디자인 적용 상태 (이전 경고 해제)
>
> 이전 인계서의 «디자인 미적용 · 스크린샷 못 봄» 경고는 **해소됐다.** 스크린샷 경로를 확보했고
> (`scripts/capture_screens.py` — Browser 패널 없이 헤드리스 Chrome), 이관 완료 화면 10개는
> 실제 이미지로 라이트 테마를 확인했다. **미이관 8개**는 여전히 다크다.

### 화면 이관 현황 (2026-08-04 기준)

| 이관 완료 (셸 위·라이트) | 미이관 (다크·동작은 정상) |
|---|---|
| 지식 허브 · 기준정보 마스터 · 협업 | AdvisorPanel (694줄) |
| **의사결정 센터**(CL-2 화면 — 아래 표의 «화면 없다»는 이제 낡았다) | PreviewPanel (658) |
| 대내외 발간 · 업무표준 · 조직·권한 | PlanningPanel (374) ← **무방비 라우트 13개** |
| 거버넌스 · 에이전트 통제소 | WorkspacePanel (373) |
| 스킬 개선안 · 전사 브리핑 | CrosswalkPanel (313) ← 대화상자 10곳 |
| **런처(App.tsx)** | ShadowModePanel (305) · ProgramAdminPanel (239) · TelemetryPanel (227) |

- 사용자 결정: **미이관 8개도 전부 이관**한다.
- 다음 권고 순서: **Planning 통제 먼저** → 작은 화면부터(Telemetry → ProgramAdmin → Shadow →
  Crosswalk → Workspace → Planning → Preview → Advisor)
- 검증 수치: pytest **1821 passed / 실패 0** · `tsc` 통과 · 화면 캡처 **84개 전 항목 통과**
