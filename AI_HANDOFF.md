...
- **최종 갱신**: 2026-08-06 (**SW Factory Studio 6/7** — 요구 확인·구현·산출물 Canvas ·
  Decision/Jarvis Dock · 근거·상태 Inspector · Project Header. 전체 진척 **43/80**)

---

## 🤝 2026-08-06 — **SW Factory Studio 3~6단계 + Project Header**

> 📄 **상세 인계**: [`docs/chronicle/handoffs/handoff_2026-08-06_factory_studio_3to6.md`](docs/chronicle/handoffs/handoff_2026-08-06_factory_studio_3to6.md)
> — 재사용 판단 3건 · 만들지 않은 것 7건 · 잡은 결함 4건 · **7단계 전제 표**가 그 문서에 있습니다.

### ★ 한 문장 — **같은 데이터를 다른 각도로 보여 주는 화면을 만들 때마다 기존 화면의 거짓이 드러났다**

| 만든 것 | 그때 드러난 것 |
|---|---|
| 3단계 Canvas | `small` 태그가 **9.6px**(§6 위반) · 단계 라벨이 에이전트 **한 명** 이름 |
| 5단계 Inspector | **WBS 진행이 틀렸다** — 화면 「진행 0 · 차단 3」, 실제 「진행 1 · 차단 1」 |
| 6단계 산출물 Canvas | `UI_DESIGN`·`VISION_QA` 가 「올 예정입니다」로 **거짓말**하고 있었다 |

넷 다 **판정이 두 곳에 있었기 때문**입니다(이 저장소에서 여덟 번째). 각각은 그럴듯했고, 둘을
나란히 놓아야 모순이 보였습니다.

### 만들지 않은 것 — 없는 것을 채우면 그것이 거짓이 된다

준비도 %(서버가 계산 안 함) · 전환 **조건**(서버 로직) · RFP 대비 Diff · 누적 LLM 비용
(**₩0 으로 쓰면 「공짜로 돌고 있다」로 읽힌다** → 「— 누적 비용 미측정」) · 빌드 번호(프로토타입
예시 데이터) · 8칸 고정 단계 지도(실제로는 프로젝트마다 14·16칸).
각각 화면에 **무엇이 없는지** 적었습니다.

### ⚠️ 7단계(기존 3패널 제거)는 아직 할 수 없다

§8 기능 게이트 11항목 중 **6 충족 · 5 미충족**(Sprint 시작 · 재개 · 복구 · WBS 재분할 ·
Release 저장·Export). 지금 지우면 그 5개가 사라집니다 — 명세가 금지한 「선삭제」입니다.
**Sprint 시작을 옮길 때는** `project_state_payload` 조립을 먼저 공용 모듈로 추출해야 합니다 —
복사하면 두 화면이 다른 payload 를 보내고, 그것은 「두 화면이 다른 프로젝트를 만든다」가 됩니다.

---

## 🤝 2026-08-05 — **병합 검증과 SW Factory Studio 착수**

> 📄 **상세 인계**: [`docs/chronicle/handoffs/handoff_2026-08-05_merge_verification_and_factory_studio.md`](docs/chronicle/handoffs/handoff_2026-08-05_merge_verification_and_factory_studio.md)
> — 다음 할 일·함정·결정 대기 4건이 그 문서에 정리돼 있습니다. **먼저 그것을 읽으십시오.**

### ★ 한 문장 요약 — **이번 세션에 찾은 결함은 전부 «검증하려다» 나왔다**

병합을 검증하려다 회귀 5건, 화면을 브라우저로 보려다 경영계획 유출, `tsc -b` 를 돌리려다
프론트 빌드 파손, 트랙 E 를 검증하려다 경로 결함. **검증 수단이 없던 영역은 예외 없이 깨져
있었고**, 파이썬 테스트 2,089건은 그중 하나도 잡지 못했습니다.
→ 다음 사람의 첫 일은 **아직 검증 경로가 없는 영역을 찾는 것**입니다(상세 문서 §7).

> 사용자 지시로 «별도 브랜치에서 병합 후 검증», «충돌은 더 엄격한 쪽 + 이유 기록» 방식을
> 따랐습니다. 병합 작업은 `.claude/worktrees/merge-check` (브랜치 `merge/screens-into-dev`)
> 에서 했습니다 — **Codex 가 `.agents/TEAM_BOARD.md`·`frontend/src/App.tsx` 에 미커밋
> 변경을 갖고 있어** `dev` 워킹트리를 건드리지 않아야 했습니다.

### ★★★ 이 병합에서 배운 것 하나 — 「충돌 마커 0건」은 안전을 뜻하지 않는다

충돌 4건은 눈에 보였고 해결했습니다. **정작 위험했던 것은 충돌이 나지 않은 쪽**입니다.

| # | 무엇이 | 어떻게 드러났나 |
|---|---|---|
| 1 | `api/deps.py` 에 `_EXACT_COUNT_RULES`·`hidden_envelope` 가 **두 벌** 남았다. 양쪽이 각자 추가했고 위치가 달라 git 이 충돌로 보지 않았다. 뒤의 정의가 앞을 덮어 **`plan` 규칙만 사라졌다** | 문법·임포트 정상. 테스트 7건이 깨져서야 |
| 2 | `factory_control.py` 충돌을 dev 쪽으로 해결해 **브랜치 통제 9개를 잃었다.** 그중 `POST /{project_id}/hotl/resume` — 사람 확인(HOTL) 중단점 재개 | 테스트 6건 |
| 3 | 같은 파일에서 브랜치에만 있던 `_assert_agent_config_readable` 이 함께 사라져 `POST /ai-recommend/pipeline`(LLM 호출 = 예산 소모)이 무방비로 돌아갔다 | 테스트 1건 |
| 4 | `GET /projects` 가 **익명에게 프로젝트 목록을 줬다.** 소유권 필터는 있었지만 미태깅 프로젝트가 하위호환으로 통과한다 | 테스트 1건 |
| 5 | `POST /projects`·`POST /projects/mega` 가 익명 생성을 허용했다 — 소유권이 비면 프롬프트 주입 필터가 **아예 걸리지 않는다**(fail-open) | 테스트 1건 |

⚠️ **4번을 자동 점검이 놓쳤습니다.** 라우트에 `current_principal` 의존성이 **있으므로**
「통제됨」으로 셌습니다. **의존성이 있다는 것과 판정이 있다는 것은 다릅니다** — 주체를 받아
놓고 아무것도 판정하지 않는 라우트가 가장 안전해 보입니다.

### ★★★ [트랙 E 2단계 중 발견] 프로젝트 작업공간 경로가 여전히 상대경로였다

D-018 ①에서 **DB 경로를** `core/paths.py` 로 고정했습니다. 그런데 **생성 프로젝트 작업공간
(`projects/`)은 남아 있었습니다** — `os.path.join("./projects", pid)` 형태로 **37곳**.

실측으로 드러난 방식: 워크트리에서 백엔드를 띄우고 `GET /api/v1/factory/projects` 를 불렀더니
실제 프로젝트 대신 **테스트 잔여물 1개**만 나왔습니다. 서버 프로세스의 cwd 가 저장소 루트가
아니면 다른 디렉터리를 읽습니다.

⚠️ **«목록이 비어 보인다» 로 끝나지 않습니다.** 이 경로 중 일부는 `os.makedirs(..., exist_ok=True)`
를 합니다. 즉 **엉뚱한 위치에 빈 작업공간을 새로 만들고**, 그 뒤 사용자가 만든 산출물이 그쪽에
쌓입니다. 원래 위치에서 보면 «사라진» 상태입니다. DB 에서 빈 파일이 열리던 것과 같은 유형이고,
이쪽은 **사용자가 만든 산출물**이라 더 아픕니다.
⚠️ `core/org_seed.py:reconcile_ownership` 의 기본값도 상대경로였습니다. 이 함수는 **소유권 미러를
재구축**하므로 잘못된 디렉터리를 읽으면 미러가 0건이 되고, 미러가 비면 목록 API 의 부서 필터가
«소유권 미기록» 으로 전부 통과시킵니다(하위호환 규칙). **경로 하나가 틀리면 권한 필터가 조용히
열립니다.**

**테스트 격리에 관한 발견**: 절대경로로 고치자 테스트 20건이 깨졌습니다. 원인은
`monkeypatch.chdir(tmp_path)` — 즉 **테스트가 «제품 코드는 cwd 에 의존한다» 를 전제로 격리**하고
있었습니다. 격리가 잘못된 것이 아니라 격리 지점이 cwd 였던 것입니다. 그래서 지점을
`core.paths.PROJECTS_DIR` 하나로 모으고(호출부는 `workspace_path()` 만 부릅니다), 각 테스트에
다음 한 줄을 넣었습니다.
...
⚠️ 이 한 줄이 없으면 **테스트가 제품 `projects/` 에 프로젝트를 만듭니다.** 실측으로 확인했습니다 —
워크트리에서 스위트를 돌리는 동안 `P_ALL`·`P_CTX`·`P_DUP`·`P_IDEA`·`P_KEEP`·`P_NOWBS`·`P_OK`·
`P_PLAIN`·`P_WBS` 9개가 생겼고 **정리되지 않았습니다.** 제품 `projects/` 에도
`__audit_probe__`(빈 디렉터리, 2026-08-05 17:33)가 남아 있습니다 — 지우지 않았습니다(제품
데이터이므로 사용자 판단 사안입니다).

### ★★★ 그리고 병합과 **무관한** 유출 하나를 검증 중에 잡았다

`GET /api/v1/planning/facts` 가 **익명에게 경영계획 사실 20건(금액 포함)** 을 줬습니다.
`dev` 원본에서도 같았습니다 — 병합 회귀가 아니라 **기존 결함**입니다. 같은 라우터의 다른 GET
5개는 404(범위 은폐)로 막히는데 **가장 민감한 하나만** 열려 있었습니다.

원인이 두 겹입니다.

1. **`_assert_identified` 를 부르지 않았습니다.** 30개 라우트 중 17개가 그랬습니다. 대부분은
   범위 판정에 걸려 **우연히** 막혀 있었습니다 — 「불러 보니 401이다」는 「통제가 있다」가 아닙니다.
2. **빈 범위 요청이 통제를 우회합니다.** `org_id`·`scope_node_id` 를 비우면
   `assert_scope_allowed` 는 «전사 요청» 으로 보고 정규화할 대상이 없어 통과시키고, 저장소로
   넘어가는 범위도 빈 값이라 행 필터가 걸리지 않습니다.
   → **파라미터를 주지 않는 것이 가장 넓은 조회입니다.** 「범위를 명시하면 거부된다」만
     확인하면 이 경로를 영원히 못 봅니다. 봉합 당시 테스트가 정확히 그랬습니다.

왜 기존 테스트가 놓쳤나: `test_planning_control_gate.py` 의 `READ_PATHS` 목록에 **`/facts` 가
없었습니다.** 목록을 손으로 관리했고 「새 라우트를 추가하면 여기 넣어야 한다」는 주석은 지켜지지
않았습니다. → 17개 전부에 식별 확인을 넣고, **라우터를 읽어 누락을 잡는 테스트**를 더했습니다.

### 병합 후 익명 실측 (파라미터 없는 GET 109개)

차단 38 · 빈응답 28 · **자료 있음 26**(`dev` 29 → 26) · 그 외 17(주로 422 — **차단이 아닙니다**).
26개는 성격이 갈립니다. `/org/me` 는 익명에게 **자기 자신의 빈 권한 상태**를 주고, 화면이
로그인 안내를 띄우려면 필요합니다. `/ledger/event-types`·`/copy-policy`·`/evidence-kinds`·
`/jarvis/context-contract` 는 스키마·열거형입니다. 업무 자료가 나가는 쪽은 `/advisor/*`·
`/benchmark/scenarios`·`/enterprise-context/entities`·`/shadow/*`·`/workspace/*`·
`/telemetry/*`·`/reference/summary` 이고, **분류가 트랙 G 의 첫 작업**입니다.

### 그래서 병합 검증은 네 가지를 따로 본다

1. **같은 이름이 두 번 정의됐는가** (AST 로 최상위 `def`/`class`/상수 대조) — 스위트 통과만으로는
   못 잡습니다. 테스트가 그 경로를 부르지 않으면 조용합니다.
2. **한쪽에서 통제되던 라우트가 무방비가 됐는가** (37개 라우트 파일 전수 → **0건**)
3. **한쪽의 정의·라우트가 사라졌는가** (병합으로 바뀐 16개 비테스트 `.py` 전수 → **0건**)
4. **테스트를 실제로 돌린다** — 위 셋이 «흔적» 을 보는 데 반해 이것만이 «동작» 을 봅니다.
   4번·5번은 오직 테스트가 잡았습니다.

### 결과

- **무방비 라우트: `dev` 94 → 병합 후 53** (16개 파일). 트랙 G 분모가 그만큼 줄었습니다.
- 백필 dry-run **「바꿀 것 0건」** — 병합이 D-018 정본화를 되돌리지 않았습니다.
- `scripts/backfill_scope_node_ids.py` 의 출력 인코딩을 스크립트가 스스로 고정하게 했습니다.
  종전에는 한국어 콘솔(cp949)에서 `→`·`⚠️` 를 찍다 죽었고, **그 print 가 표별 UPDATE 사이에
  있어서 `--commit` 중에 죽으면 앞 표만 갱신된 부분 적용 상태**가 됩니다. 그 상태는 백필 전도
  후도 아니어서 무엇을 되돌릴지조차 모호합니다.

### 결정 대기 2건 (`PROGRESS.md` 에 상세)

1. **에이전트 제어판의 저장을 지금 아무도 할 수 없습니다.** `PUT /api/v1/factory/agents` 는
   `SYSTEM_DEFAULT_EDIT` 를 요구하는데 그 능력을 주는 역할·플래그가 하나도 없고(설계상 플랫폼
   관리자 전용), 운영 DB 의 플랫폼 관리자는 **0명**입니다.
   → 통제를 임의로 넓히지 않았습니다. 선택은 ⓐ 플랫폼 관리자 임명 ⓑ 요구 자격 하향입니다.
2. **역할이 없는 활성 사용자는 구성 화면이 빕니다.** 지금은 22명 전원이 역할을 갖고 있어
   드러나지 않습니다.
...
**① 화면 이관 결과물은 이제 `dev` 에 있습니다** — ~~`dev` 에 없습니다~~.
`claude/nice-hawking-f8b4bf` 의 커밋 7개(화면 10개 + 런처 + 공용 셸 `HubShell`·`afs.css` +
용어사전 `terms.ts` + 헤드리스 캡처 스크립트)를 **2026-08-05 에 병합했습니다** — 위 «병합»
섹션을 먼저 읽으십시오. 예상대로 충돌이 났고(4건), 예상하지 못한 쪽에서 회귀 5건이 나왔습니다.
남은 화면은 8개이며 진척 정본 `PROGRESS.md` 의 **트랙 F** 로 셉니다.
