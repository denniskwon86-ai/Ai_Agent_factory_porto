...
async def asset_hygiene(p: Principal = Depends(current_principal)):
    """[D-017 §9 P4-4] 자산 정리 제안 — 중복(동일·유사)과 **사용 기록 없음**.

    ★ 확신의 차이를 화면에서 지우면 사람은 목록 전체를 같은 무게로 읽고, 한 번 잘못 지우면
      다시는 이 목록을 쓰지 않는다(사용자 결정 2026-08-07 안 B).

    ★★ `unused` 는 **「미사용」이 아니라 「관측 이후 사용 기록 없음」** 이다. 서버가 아는 것은
      자산이 해석되는 것을 본 적이 없다는 것뿐이고, 관측 이전의 사용은 여기 없다.
      화면도 그 이름을 써야 한다 — 「미사용」으로 적으면 사람은 지워도 되는 것으로 읽는다.

    ⚠️⚠️ **`unused.available` 을 무시하지 말 것.** 관측 기간이 `MIN_OBSERVATION_DAYS` 에
      못 미치면 서버는 **목록을 만들지 않고** `available=False` 와 사유를 낸다. 관측을 켠
      다음 날이면 자산 전부가 목록에 오르고, 그것을 본 사람은 전부 지운다.
      `available=False` 일 때 빈 목록을 그리면 그것은 「정리할 것이 없다」는 **거짓**이다.

    ⚠️ 자동 삭제 경로를 만들지 않는다. 스킬 파일을 지우면 그것을 이름으로 참조하던 에이전트가
