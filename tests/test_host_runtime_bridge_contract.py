...
# ── ④ 브리지는 «켬» 이 기본이고, 끄는 스위치가 남아 있다 ─────────────────

def test_브리지는_명시적으로_꺼야만_꺼진다(bridge):
    """★★★ 2026-08-20 전환 — 교차검토 지시의 전제(「격리 카나리에서 종단 검증한 뒤
    전환한다」)가 충족돼 기본값을 뒤집었다.

    ⚠️ **끄는 스위치가 사라지면 안 된다.** 사고가 났을 때 되돌릴 방법이 코드 수정뿐이면
      그 전환은 되돌릴 수 없는 전환이다.
    ⚠️ 저장소가 `.env*` 를 전부 무시하므로(루트 `.gitignore`) env 로는 켤 수 없었다 —
      그래서 코드 기본값을 바꿨다. 이 시험이 그 판단을 못박는다."""
    assert "VITE_AFS_HOST_RUNTIME" in bridge
    #: ★ 정확히 `'0'` 일 때만 꺼진다 — 오탈자를 «꺼짐» 으로 읽으면 아무 데서도 안 켜진다.
    assert "!== '0'" in bridge, "끄는 스위치가 없다 — 되돌릴 방법이 코드 수정뿐이 된다"
    assert "=== '1'" not in bridge, \
        "「'1' 일 때만 켬」으로 되돌아갔다 — env 를 못 쓰는 이 저장소에서는 아무 데서도 안 켜진다"
