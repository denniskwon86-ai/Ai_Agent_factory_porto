...
def test_현재_판본을_불러온다():
    o = kit_defs.load("1.7.0")
    assert o.version == "1.7.0"
...
def test_1_0_0_은_재생성을_거부한다():
    ...
    assert "1.7.0" in msg and "fingerprint" not in msg.lower() or "verify" in msg
...
def test_use_version_이_경로와_정의를_함께_바꾼다(tmp_path):
    ...
    before = gen.KIT_ROOT
    try:
        gen.use_version("1.7.0", tmp_path / "여기")
        assert gen.KIT_VERSION == "1.7.0"
        assert gen.KIT_ROOT == tmp_path / "여기"
        assert gen.OVERLAY.version == "1.7.0"
    finally:
...
def test_available_이_판본을_나열한다():
    ...
    assert {"1.0.0", "1.1.0", "1.2.0", "1.3.0", "1.4.0", "1.5.0", "1.6.0", "1.7.0"} <= set(kit_defs.available())
...
def test_현재_판본은_다시_만들어도_같다(tmp_path):
    ...
    ⚠️ 1.0.0~1.2.0 에는 이 시험을 걸 수 없다. 생성기 로직이 바뀌어 **재생성 자체가
      거부되기** 때문이고, 그 판본들은 파일과 지문이 지킨다(`test_kit_freeze.py`).
    """
    import importlib
    from core.data_preparation import kit_freeze as kf

    gen = importlib.import_module("generate_sample_company_starter_kit")
    orig = os.path.join(REPO, "starter_kits", "KIT-MFG-NONFERROUS-PROCUREMENT", "1.7.0")
