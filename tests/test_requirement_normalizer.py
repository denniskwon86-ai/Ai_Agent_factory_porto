...
def test_확정_사실이_붙는다():
    ...
    assert "이미 제공" in out and "구현 대상 아님" in out


def test_출구를_열어_준다():
    """★★★ 「지우라」가 아니라 「호스트가 제공(구현 대상 아님)으로 적으라」다.

    ⚠️ 출구가 없으면 모델은 완성도를 택한다 — 그것이 이번 사고의 기전이다."""
    clause = render_clause()
    assert "호스트가 제공" in clause
    assert "누락이 아니라" in clause
