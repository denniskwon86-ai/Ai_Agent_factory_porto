...
- ⚠️ `release` 정상 경로는 미확인이다. 스냅샷이 `library` 를 제외해 실제 릴리스 자산이 없다.
...
- 다음/담당/조건: **Codex 결정 대기** — 설계안 §6 의 다섯 가지(mega 부모-자식 서버 확인, kit_app releaseId 범위, draft 방식, 공통 헬퍼 시점, 담당 분담). 합의 전 구현하지 않는다.
