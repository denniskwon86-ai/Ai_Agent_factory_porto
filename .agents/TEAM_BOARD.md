...
- 다음/담당/조건: project 진입 연결은 `App.tsx` 한 곳, URL 직접 진입 경로만 Gate 경유하는 최소 변경으로 설계했다(문서 §6). **Gate4 가 완전히 닫히지 않았다** — 다른 조직·Resolver 장애 미확인. 담당 문서 충돌(통합계획·B6 는 「App 은 Codex 단독」, 시작 안내 §6-A5 는 「Claude Code 로 이관」)을 문서 §7 에 명시했다.
