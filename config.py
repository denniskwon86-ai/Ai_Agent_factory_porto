...
# ★ [2026-07-27] 리뷰어 프롬프트에 주입할 실제 코드의 최대 문자 수.
#   리뷰어가 코드를 못 보면 명세만 보고 '존재하지 않는 파일'을 근거로 반려한다(실측 test_a1_v8).
#   Gemini 2.5 Flash 컨텍스트가 1M 이라 6만자는 여유롭다.
REVIEWER_CODE_BUDGET_CHARS = 60000

# ★ [2026-07-27] 동일 지적이 이 횟수만큼 반복되면 REWORK_DEV 를 멈추고 ESCALATE_PM 으로 승격.
#   ⚠️ 같은 지적의 반복은 '개발자가 게을러서'가 아니라 **그 지시를 수행할 수 없다**는 신호다.
#     실측: 시스템에 파일 삭제 기능이 없는데 "React 파일을 삭제하라"를 8회 반복하다 죽었다.
#     설계상 ESCALATE_PM → Master_PM 경로가 있는데 **발동 조건이 없어** 한 번도 쓰이지 않았다.
#     (QA·Supervisor 는 리뷰어가 PASS 해야만 실행되므로, 리뷰어에서 막히면 그 위의
#      에스컬레이션 사다리에는 도달 자체가 불가능하다.)
REPEAT_FEEDBACK_ESCALATE_AFTER = 2
...
# ★ [2026-07-27] WBS 기준 미달 시 자동 재분할 상한.
#   WBS 는 이진이다 — 내용이 빠졌거나 배분이 잘못됐으면 다시 짜야 한다. 다만 무한 재분할은
#   막아야 하므로 상한을 두고, 도달하면 사람이 보도록 표면화한 뒤 진행한다(HOTL 게이트가 뒤따름).
WBS_MAX_RESPLIT_ATTEMPTS = 3

# ==========================================
# 조직·권한 (설계서 Phase 1)
# ==========================================
ORG_MAX_DEPTH = 8        # 조직 계층 최대 깊이 — 순환/무한 중첩 방어
# ⚠️ 하위호환 스위치. False 면 권한 필터가 전부 no-op 이다.
#   `departments` 가 비어 있으면 이 값과 무관하게 무제한으로 동작한다(조직 미도입 = 지금과 동일).
#   조직을 등록한 뒤에도 이 값이 False 면 강제하지 않는다 — 도입을 단계적으로 하기 위한 안전판.
ORG_ENFORCE = False

# ── 사용자 식별 (Phase 2) ────────────────────────────────────────────────
# ⚠️ 헤더 신뢰는 **경량 전환용**이다. 브라우저가 임의 헤더를 보낼 수 있으므로 이것만으로는
#   인증이 아니다. SSO 이행 시 `main.py` 에 미들웨어로 `request.state.principal_user_id` 를
#   세팅하고 이 값을 False 로 내리면 **라우트를 하나도 안 고치고** 전환된다.
#
# ★★★ [P0-1C · 2026-08-09] 기본값을 **False 로 내렸다.** 그 «SSO 이행» 을 기다리지 않는다.
#
#   2026-08-09 실서버 실측: 로그인 화면을 붙인 뒤에도
#       curl -H 'X-Factory-User: <아무개>' .../api/v1/admin/scope-policy   → HTTP 200
#   이었다. 즉 **로그인은 정문일 뿐이고 옆문이 열려 있었다.** 세션을 발급하고 서버가 그것을
#   먼저 보게 만들어 놓고도(`api/deps._extract_user_id` ①-b), 헤더 경로를 함께 남겨 두면
#   공격자는 언제나 더 쉬운 쪽을 고른다.
#
#   ⚠️ 지우지 않고 **스위치로 남긴다.** 개발 스크립트와 하위호환 테스트가 아직 이 경로를 쓴다.
#     다만 켜는 것은 **의도적인 한 동작**이어야 한다 — 환경변수를 명시해야만 켜진다.
#         AFS_DEV_TRUST_HEADER=1 python main.py     ← 개발 전용
#     운영에서 이 변수를 설정하면 그것은 로그인 통제를 스스로 여는 것이다.
#
#   ⚠️⚠️ 이 한 줄이 «꺼졌다» 고 해서 SSE 까지 닫힌 것이 아니다. EventSource 는 헤더를 못 붙여
#     별도 경로(1회용 접속표, P0-1B)로 닫았다. 둘은 다른 구멍이다.
ORG_TRUST_HEADER = os.getenv("AFS_DEV_TRUST_HEADER", "") == "1"
ORG_USER_HEADER = "X-Factory-User"
...
# ── 관문 A: 범위 미지정의 해석 (M2 §2.2 / §5.3) ──────────────────────────
# True(기본)  = 범위 미지정은 **비노출**이다(fail-closed). 2026-07-30 관문 A 로 전환됐다.
# False       = 종전 규칙("미지정 = 전사 공용")으로 즉시 되돌린다.
#
# ⚠️ 이 스위치를 남겨 두는 이유는 설계서 §5.3 의 요구다 — "한 번에 전환하면 무엇이 안 보이게
#   됐는지 아무도 모른다." 도입 중 현업이 막히면 **코드 배포 없이** 되돌릴 수단이 있어야 한다.
#   되돌리기 전에 `coverage()` 로 몇 건이 어떤 이유로 가려졌는지 먼저 세는 것이 순서다.
#
# ⚠️⚠️ False 는 **유출 상태**다. 2026-07-29 에 이 규칙으로 기준정보 26건이 전 조직에 노출되고
#   LS전선 프롬프트에 MnM 기준정보가 들어갔다. 임시로만 쓰고, 켤 때는 기한을 정해서 켜라.
#   ★ 단 **주입 경로(프롬프트)에는 적용되지 않는다.** 주입은 되돌릴 수 없다(이미 LLM 이 읽고
#     산출물에 반영된다) — 화면에서 되돌릴 수 있는 것과 성질이 다르므로 미바인딩 비주입은
#     이 스위치와 무관하게 유지된다.
SCOPE_FAIL_CLOSED = True

# ── 지식팩 검색의 조직 범위 강제 (2026-07-30) ────────────────────────────
# True  = 프로젝트의 조직 범위로 지식팩 검색을 필터한다(청크의 `owner_org_id` 기준).
# False = 필터하지 않는다(종전 동작).
#
# ⚠️ **기본이 False 인 이유는 데이터 상태다.** 관문 A 이전에 업로드된 청크에는 `owner_org_id`
#   가 없고, 켜는 순간 그것들이 **전부 검색에서 제외된다**(fail-closed). 실측: `core-m3-standards`
#   팩의 청크 641건이 전부 범위 미기재였다 — 지금 켜면 그라운딩이 조용히 0건이 된다.
#
# 켜는 순서(설계서 §5.3 이 관문 A 에 요구한 절차와 같다):
#   ① `knowledge_base.pack_scope_report()` — 켜면 몇 건이 사라지는지 먼저 센다
#   ② `knowledge_base.set_pack_scope(pack_id, owner_org_id)` — 소유 조직을 소급 부여
#      (또는 등록부 경유 재색인 — 그쪽은 색인 시점에 범위가 심긴다)
#   ③ 보고서의 `unscoped` 가 0 이 된 뒤 이 값을 True 로
#
# ✅ [2026-07-30] 그 절차를 완료해 **True 로 전환했다.**
#   · 사용자 결정: `core-m3-standards` 팩의 소유 조직은 **LS MnM(LS_MNM)**
#   · 실측: 청크 641건 전부에 소유 부여 완료 → `pack_scope_report().unscoped == 0`
#     ("범위 강제를 켜도 사라지는 것이 없습니다")
#   · 이후 업로드는 팩 매니페스트의 선언 소유를 물려받는다(`add_document`)
#
# ⚠️ 범위가 심기지 않은 청크를 **새로** 만들면(등록부·매니페스트를 우회한 직접 업로드 등) 그
#   청크는 조회에서 제외된다. `pack_scope_report()` 가 그 건수를 상시 보고하므로, 0 이 아니면
#   원인을 찾아야 한다. 문제가 생기면 이 값을 False 로 내려 **코드 배포 없이** 되돌릴 수 있다.
KB_SCOPE_ENFORCE = True

# ── 과거사례 RAG 방어 (Phase 5) ──────────────────────────────────────────
# ⚠️ 최대 유출 경로였다: 전역 `project_releases` 컬렉션을 필터도 임계값도 없이 검색해
#   상위 N건을 **모든 프롬프트에 주입**했다. 3중으로 막는다.
RAG_PAST_CASES_ENABLED = True    # ③ 전역 킬스위치 — 의심되면 통째로 끈다
RAG_PAST_CASES_CUTOFF = 0.65     # ② 거리 임계값(get_grounding_context 와 동일 기준)
