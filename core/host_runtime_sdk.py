...
FORBIDDEN_SURFACE: Tuple[str, ...] = (
    ...
    "afs.session",
    #: ⚠️⚠️ [2026-08-26 실측] 생성된 앱이 `window.afs.auth` 를 **지어냈다.** 로그인 태스크가
    #:   있었고 호스트가 무엇을 주는지 아무도 알려 주지 않아서다. 그 검사가 실패해 앱은
    #:   「AFS 런타임 환경을 사용할 수 없습니다」로 첫 줄에서 멈췄다 — 완주는 했는데
    #:   화면이 안 뜨는 상태였다. 이름을 적어 두어야 시험이 되살아남을 잡는다.
    "afs.auth",
    "afs.login",
