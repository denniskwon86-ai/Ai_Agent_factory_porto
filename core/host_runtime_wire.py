...
#: ★ 아직 서버에 없는 경로. **I-3 착수 전에 만들어야 한다** — 없다고 해서 브리지가
#:   데이터셋 없는 경로로 우회하면 위의 혼동된 대리인이 그대로 열린다.
ROUTES_OWED: Tuple[str, ...] = ("data.get", "data.update", "data.remove")
...
def idempotency_slot(sid: str, op: str, dataset: str, key: str) -> str:
    ...
    | | |
    |---|---|
    | 막는다 | 같은 화면에서 저장을 두 번 누름 · `UNAVAILABLE` 후 재시도 |
