...
★★★ **이 정책은 「누가 서명권자인가」를 정하지 않는다.** B0-B부터 실제 서명은
  `certification_authority`의 승인된 회사 정책과 현재 조직/PDP가 결정한다.
  일반 표준 관리권은 서명권이 아니다. 이 JSON은 안내·제안 기본값이며 실제
  필요 서명·증거 최소 길이는 승인된 불변 회사 정책 판본에 명시한다.
  JSON을 변경해도 이미 승인된 회사 정책을 자동 수정하지 않는다.
