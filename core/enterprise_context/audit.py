...
# ★ [G1-B03] 생성 앱 데이터 접근 증명. **가상 문맥 전용인 위의 샌드박스 토큰과 다른 사건**이다 —
#   이름을 나누지 않으면 「가상 실험용 임시 권한」과 「실제 앱이 실제 데이터를 다룬 기록」이
#   한 통계에 섞이고, 그러면 어느 쪽이 늘었는지 아무도 답할 수 없다.
APP_TOKEN_ISSUED = "APP_TOKEN_ISSUED"
#: ★ 성공 사용도 남긴다. 발급과 거부만 남기면 「그 증명으로 실제로 무엇을 했나」에 답할 수 없고,
#:   유출 조사에서 가장 필요한 것이 그 기록이다(교차검토 [G1-B-P0-REVIEW-75] 지적 5).
APP_TOKEN_USED = "APP_TOKEN_USED"
...
# 기준정보 신규 정본은 제안과 승인을 분리한다. 단순 개정 사건과 섞으면 누가 새 전사 기준값을
# 만들었는지, 누가 검토했는지를 복원할 수 없다.
MASTER_DATA_PROPOSED = "MASTER_DATA_PROPOSED"
MASTER_DATA_APPROVED = "MASTER_DATA_APPROVED"
MASTER_DATA_REJECTED = "MASTER_DATA_REJECTED"
MASTER_DATA_REVISED = "MASTER_DATA_REVISED"
EXTERNAL_INDICATOR_PROPOSED = "EXTERNAL_INDICATOR_PROPOSED"
EXTERNAL_INDICATOR_APPROVED = "EXTERNAL_INDICATOR_APPROVED"
EXTERNAL_INDICATOR_REJECTED = "EXTERNAL_INDICATOR_REJECTED"
# [식별자 Wave 4-6] 계산 내부 진단은 관계·계약·원장 식별자를 포함한다. 일반 조회와
# 섞지 않고, 시스템 관리자가 명시적으로 펼치거나 복사한 사건만 별도로 남긴다.
CALCULATION_DIAGNOSTIC_REVEALED = "CALCULATION_DIAGNOSTIC_REVEALED"
CALCULATION_DIAGNOSTIC_COPY_ISSUED = "CALCULATION_DIAGNOSTIC_COPY_ISSUED"
...
EVENTS = (
    ...
    APP_TOKEN_ISSUED, APP_TOKEN_USED, APP_TOKEN_EXPIRED, APP_TOKEN_REVOKED,
    ...
    MASTER_DATA_PROPOSED, MASTER_DATA_APPROVED, MASTER_DATA_REJECTED, MASTER_DATA_REVISED,
    EXTERNAL_INDICATOR_PROPOSED, EXTERNAL_INDICATOR_APPROVED, EXTERNAL_INDICATOR_REJECTED,
    CALCULATION_DIAGNOSTIC_REVEALED, CALCULATION_DIAGNOSTIC_COPY_ISSUED,
)
