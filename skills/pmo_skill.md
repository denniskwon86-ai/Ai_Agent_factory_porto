...
7. **[QA 처리]** `QA`(최종 통합 검증)는 **마지막 태스크가 완료되는 시점에 시스템이 자동으로 실행**합니다. 따라서 모든 태스크에 QA를 넣을 필요는 없습니다. 다만 특정 태스크에 **전용 통합 테스트**가 반드시 필요하다고 판단되면 그 태스크의 `required_agents`에 `QA`를 명시적으로 포함해도 됩니다.
