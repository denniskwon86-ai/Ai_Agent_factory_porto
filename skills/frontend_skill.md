...
# 🚨 [필수] 백엔드 없이 단독 렌더 — mock 우선
- 프리뷰엔 실행 중인 서버가 없다(API 100% 실패 가정). **`useState` 초기값을 현실적 mock 데이터로 충분히 채워** 마운트 즉시 완결된 화면이 보이게 하라.
- `fetch` 는 선택적 보강: `try/catch` 로 감싸고 **성공할 때만** mock 을 교체. 실패 시 아무 것도 하지 말고 mock 유지. 🚫 "불러오기 실패" 에러 배너를 기본 화면으로 띄우지 마라. 흰 화면·크래시 금지.
